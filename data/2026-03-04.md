<div id=toc></div>

# Table of Contents

- [quant-ph](#quant-ph) [Total: 66]
- [astro-ph.IM](#astro-ph.IM) [Total: 8]


<div id='quant-ph'></div>

# quant-ph [[Back]](#toc)

### [1] [Stairway Codes: Floquetifying Bivariate Bicycle Codes and Beyond](https://arxiv.org/abs/2603.00228)
*Shoham Jacoby,Alex Retzker,Fernando Pastawski*

Main category: quant-ph

TL;DR: Introduction of Stairway codes as a new family of high-rate Floquet codes improving encoding efficiency and error thresholds in fault-tolerant quantum computing.


<details>
  <summary>Details</summary>
Motivation: To develop Floquet codes that achieve high encoding rates and thresholds for fault-tolerant quantum protocols, especially when dealing with limitations in available hardware.

Method: The paper constructs Stairway codes by Floquetifying Abelian two-block group algebra codes, utilizing a foliated ZX-calculus representation to manage stabilizers and measurements in a structured way.

Result: Stairway codes exhibit competitive parameters and demonstrate logical error rates outperforming existing Floquet codes, requiring significantly fewer physical qubits for similar performance.

Conclusion: Stairway codes represent a significant advancement in Floquet code efficiency and practicality, enabling better performance with reduced quantum hardware requirements.

Abstract: Floquet codes define fault-tolerant protocols through periodic measurement sequences that drive a dynamically evolving stabilizer group. They provide a natural framework for hardware supporting two-qubit parity measurements but no unitary entangling gates. However, few known constructions achieve both high encoding rates and high thresholds. We close this gap by introducing Stairway codes, a family of high-rate Floquet protocols obtained by Floquetifying Abelian two-block group algebra codes, a class that includes the bivariate bicycle codes. By representing the static code as a foliated ZX-calculus network within a $(w{-}1)$-dimensional space-time lattice and rotating the time axis, we decompose its weight-$w$ stabilizers into a periodic sequence of pairwise measurements. This reduces the design of new codes within this family to the selection of favorable periodic boundary conditions. We identify instances with competitive parameters, analyze their distance under circuit-level noise, and demonstrate logical error rates surpassing those of other Floquet codes at comparable encoding rates. Remarkably, our construction requires fewer than 300 physical qubits to match the distance and encoding rate of semi-hyperbolic Floquet codes that use over 1300 qubits.

</details>


### [2] [Theory of Magic Phase Transitions in Encoding-Decoding Circuits](https://arxiv.org/abs/2603.00235)
*Piotr Sierant,Xhek Turkeshi*

Main category: quant-ph

TL;DR: This study elucidates the relationship between quantum magic resources and phase transitions in quantum circuits, providing clarity on previous inconsistencies in the literature regarding their universality classes.


<details>
  <summary>Details</summary>
Motivation: To understand how magic resources and errors interact in quantum circuits and clarify previous conflicting results in the literature.

Method: The authors analytically examined encoding-decoding quantum circuits, focusing on the effects of fixed measurement protocols and realistic sampling methods on magic resource behavior and phase transitions.

Result: The study finds that magic phase transitions are influenced by measurement protocols, exhibiting universal features under fixed syndromes, while realistic sampling introduces fluctuations that affect critical behavior.

Conclusion: Magic phase transitions reflect error-resilience thresholds, altering our understanding of how quantum computational power is affected by measurements and errors.

Abstract: Quantum magic resources, or nonstabilizerness, are a central ingredient for universal quantum computation. In noisy many-body systems, the interplay between these resources and errors leads to sharp magic phase transitions. However, the microscopic mechanism behind these critical phenomena is still an open question, especially since early empirical evidence showed conflicting results regarding their universality classes. In this work, we provide a comprehensive picture of magic phase transitions for the class of encoding-decoding quantum circuits to resolve these ambiguities. We analytically show that the behavior of magic resources is fundamentally dictated by the chosen measurement protocol. When we fix, or post-select, the class of measurement syndromes, the magic transition inherits the universal features of the error-resilience phase transition in the circuits. Interestingly, this clean transition survives even for fully random Haar encoders showing that it is a consequence of initial's state retrieval, and not an artifact of the Clifford encoders. On the other hand, if we consider realistic Born-rule sampling, the intrinsic statistical fluctuations of a given syndrome measurement act as a relevant perturbation. This brings in strong finite-size drifts and an apparent multifractality, which end up altering the critical behavior of the system. We reveal that magic phase transitions are actually direct manifestations of error-resilience thresholds, rather than independent critical phenomena, reconciling conflicting observations from the earlier literature. Ultimately, our framework clarifies how the quantum computational power can survive, or be irreversibly destroyed, due to the competition between scrambling, measurements, and errors.

</details>


### [3] [Scaling Quantum Machine Learning without Tricks: High-Resolution and Diverse Image Generation](https://arxiv.org/abs/2603.00233)
*Jonas Jäger,Florian J. Kiwit,Carlos A. Riofrío*

Main category: quant-ph

TL;DR: This paper presents a novel approach to quantum generative modeling that allows for generating high-resolution images without resorting to dimensionality reduction, achieving state-of-the-art results using quantum Wasserstein GANs on classical datasets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current quantum machine learning methods that are restricted to low-resolution data and require excessive preprocessing techniques.

Method: The authors train quantum Wasserstein GANs directly on full MNIST and Fashion-MNIST datasets, utilizing innovations in classical image loading and variational circuit architectures for improved performance.

Result: They achieve state-of-the-art image generation across all ten classes in MNIST and Fashion-MNIST, and extend the approach to color images with promising results on the Street View House Numbers dataset, even under quantum shot noise.

Conclusion: The findings demonstrate the potential of advanced inductive biases and noise enhancement techniques in enhancing the quality and diversity of image generation in quantum models.

Abstract: Quantum generative modeling is a rapidly evolving discipline at the intersection of quantum computing and machine learning. Contemporary quantum machine learning is generally limited to toy examples or heavily restricted datasets with few elements. This is not only due to the current limitations of available quantum hardware but also due to the absence of inductive biases arising from application-agnostic designs. Current quantum solutions must resort to tricks to scale down high-resolution images, such as relying heavily on dimensionality reduction or utilizing multiple quantum models for low-resolution image patches. Building on recent developments in classical image loading to quantum computers, we circumvent these limitations and train quantum Wasserstein GANs on the established classical MNIST and Fashion-MNIST datasets. Using the complete datasets, our system generates full-resolution images across all ten classes and establishes a new state-of-the-art performance with a single end-to-end quantum generator without tricks. As a proof-of-principle, we also demonstrate that our approach can be extended to color images, exemplified on the Street View House Numbers dataset. We analyze how the choice of variational circuit architecture introduces inductive biases, which crucially unlock this performance. Furthermore, enhanced noise input techniques enable highly diverse image generation while maintaining quality. Finally, we show promising results even under quantum shot noise conditions.

</details>


### [4] [Low-entropy arrays of microwave-shielded molecules prepared by interaction blockade](https://arxiv.org/abs/2603.00400)
*Tijs Karman,Sebastian Will,Zoe Yan*

Main category: quant-ph

TL;DR: The paper proposes a method for loading single ultracold molecules into optical tweezers with high fidelity, addressing state preparation challenges for quantum applications.


<details>
  <summary>Details</summary>
Motivation: Ultracold molecules are critical for advancements in quantum technologies but face challenges in state preparation for large-scale applications.

Method: The proposed method involves loading single molecules into optical tweezer arrays from thermal or degenerate gases, ensuring that they occupy the motional ground state through strong repulsion among microwave-shielded molecules.

Result: The method achieves expected fidelities exceeding 99 percent when dealing with small trap volumes and highly polar species, preventing multiparticle occupancy.

Conclusion: This robust scheme can be scaled to thousands of traps, enabling the creation of large, low-entropy arrays of polar molecules for various quantum technologies.

Abstract: Ultracold molecules are becoming an increasingly important technology for quantum simulation, computation, and sensing, but their state preparation in large, low-entropy arrays remains a key challenge. We propose to deterministically load single molecules into optical tweezer arrays or lattices from either thermal or degenerate gases, with a high probability of occupying the tweezer's motional ground state. Strong repulsion between microwave-shielded molecules prevents multiparticle occupancy. Our proposal represents a robust scheme for deterministic single molecule preparation directly in the motional ground state with expected fidelities exceeding 99 percent for small trap volumes and highly polar species. This method can be scaled to thousands of traps limited by the reservoir molecule number, opening the door to large, low-entropy polar molecule arrays for quantum computation, quantum simulation, and precision measurement.

</details>


### [5] [Ergotropy from Geometric Phases in a Dephasing Qubit](https://arxiv.org/abs/2603.01129)
*Fernando C. Lombardo,Paula I. Villar*

Main category: quant-ph

TL;DR: The study connects the geometric and dynamic phases in qubits under pure dephasing to ergotropy, showing a distinct relationship and implications in quantum systems.


<details>
  <summary>Details</summary>
Motivation: To clarify the relationship between phase acquisition in qubits and energetic measures in the context of open quantum systems, particularly under decoherence.

Method: The authors utilize perturbative expansion in qubit-environment coupling strength, exploring the effects of incoherent and coherent ergotropy in phase accumulation.

Result: Dynamic phase is linked to incoherent ergotropy, while geometric phase involves both coherent and incoherent contributions; under weak-coupling, geometric phase aligns with incoherent ergotropy.

Conclusion: The findings distinguish between dynamic and geometric phases, suggesting geometric phases as effective indicators of energetic resources, relevant for practical quantum technologies.

Abstract: We analyze the geometric phase and dynamic phase acquired by a qubit coupled to an environment through pure dephasing, establishing a direct connection between phase accumulation and ergotropy. We show that the dynamic phase depends solely on the incoherent ergotropy, reflecting its purely energetic origin. In contrast, the geometric phase exhibits a nontrivial dependence on both the coherent and incoherent contributions to the total ergotropy, encoding the interplay between coherence, dissipation, and energy extraction. By performing a perturbative expansion in the qubit-environment coupling strength, we demonstrate that, in the weak-coupling and long-time regime, the geometric phase becomes determined exclusively by the incoherent ergotropy, which coincides with the asymptotic value of the total ergotropy reached under decoherence. These results provide a clear physical distinction between dynamic and geometric phases in open quantum systems and establish geometric phases as sensitive probes of energetic resources. Furthermore,~in superconducting circuit implementations, our findings suggest that the ergotropy of a two-level system could be inferred indirectly from geometric-phase measurements using standard techniques such as quantum state tomography.

</details>


### [6] [Universal Nested Quantum Switch](https://arxiv.org/abs/2603.00236)
*Jorge Miguel-Ramiro,Maria Flors Mor-Ruiz,Wolfgang Dür*

Main category: quant-ph

TL;DR: The paper presents a novel approach to quantum networking that eliminates the need for a central node by utilizing evenly distributed resources, improving resilience to node failures and achieving efficient Bell pair distribution.


<details>
  <summary>Details</summary>
Motivation: To develop a quantum switch functionality without relying on large central units, thereby increasing resilience and resource efficiency in quantum networks.

Method: A nested construction is introduced with logarithmically many qubits per node and a resource allocation of $O(n\log n)$ Bell pairs, compared to conventional methods that use $O(n^2)$ resources.

Result: The proposed approach achieves flexible pairwise connectivity, generating $n/2$ arbitrarily distributed Bell states, and includes a graph state variant using only one qubit per node, yielding $O(n/\log^2 n)$ Bell pairs.

Conclusion: This novel distributed quantum networking strategy improves efficiency and resilience, presenting a significant advancement over traditional centralized methods.

Abstract: The quantum switch is a basic network primitive that allows one to connect multiple nodes in a quantum network via a central node. We show that the same functionality can be achieved with a different geometry that does not rely on a powerful and large central unit, but instead utilizes evenly distributed resources. This approach is resilient against node failures. We provide a nested construction with logarithmically many qubits per node and a total of $O(n\log n)$ Bell pairs, in contrast to other distributed approaches based on pre-shared entanglement that scale as $O(n^2)$. The construction achieves fully flexible pairwise connectivity, where the shared resource state can be locally transformed into $n/2$ arbitrarily distributed Bell states. We also present a graph state variant with just one qubit per node, which allows one to generate $O(n/\log^2 n)$ Bell pairs.

</details>


### [7] [Mapping g-factors and complex intervalley coupling in Si/SiGe by conveyor-mode shuttling](https://arxiv.org/abs/2603.01844)
*Mats Volmer,Tom Struck,Arnau Sala,Jhih-Sian Tu,Stefan Trellenkamp,Davide Degli Esposti,Giordano Scappucci,Łukasz Cywiński,Hendrik Bluhm,Lars R. Schreiber*

Main category: quant-ph

TL;DR: This study maps the electron g-factor in Si/SiGe quantum dots with high precision, revealing two distinct g-factor values per quantum dot related to valley states, which enhance understanding of spin-valley dynamics in quantum computing applications.


<details>
  <summary>Details</summary>
Motivation: To address the increasing complexity of silicon spin qubit chips and the need for precise characterization of material parameters affecting qubit behavior.

Method: Two-dimensional mapping of the electron g-factor in quantum dots using nanometer lateral resolution and analyzing valley states using techniques like conveyor-belt shuttling of entangled electron spins.

Result: The study identifies two g-factor values corresponding to valley states, showing a bimodal distribution and symmetry across the mapping area, alongside extracted intervalley coupling parameters.

Conclusion: The findings provide critical insights into spin-valley dynamics crucial for qubit manipulation and engineering of Si/SiGe heterostructures for advanced quantum chips.

Abstract: As silicon spin qubit chips are increasing in qubit number and area, methods for the screening of qubit related material parameters become vital. Here we demonstrate the two-dimensional mapping of small variations of the electron g-factor of quantum dots formed in planar Si/SiGe quantum wells with precision better than $10^{-3}$ and with nanometer lateral resolution. We scan the electron g-factor across a 40 nm $\times$ 400 nm area and observe two g-factors per QD site which obey a striking symmetry and bimodal distribution across the area. These two g-factors relate to valley states of the electron in the quantum dot in agreement with a recent theoretical model. Using conveyor-belt shuttling of entangled electron spin pairs, complementary to the mapping of the local valley-splitting, we map the g-factor. We compare g-factor and valley splitting maps measured on the same device, and extract the complex intervalley coupling parameter along the shuttle trajectories applying a theoretical model of g-factor dependence on intervalley coupling. These maps will allow unprecedented insights into the spin-valley dynamics during qubit manipulation, readout and shuttling and serve as a benchmark for the engineering of Si/SiGe heterostructures for large-scale quantum chips.

</details>


### [8] [Lévy-index control of spectral singularities and coherent perfect absorption in non-Hermitian space-fractional quantum mechanics](https://arxiv.org/abs/2603.00256)
*Vibhav Narayan Singh,Mohammad Umar,Mohammad Hasan,Bhabani Prasad Mandal*

Main category: quant-ph

TL;DR: The paper analyzes scattering in a non-Hermitian potential using fractional quantum mechanics, highlighting spectral singularities and coherent absorption.


<details>
  <summary>Details</summary>
Motivation: The aim is to understand the impact of fractional quantum dynamics on non-Hermitian scattering phenomena.

Method: Utilizing the Riesz fractional derivative, the authors derive geometric locus equations for spectral singularities and coherent perfect absorption in a complex parameter space.

Result: Findings include a lower threshold for gain-loss strength required for spectral singularities and coherent perfect absorption as the Lévy index decreases, with energy shifts correlating with changes in $α$.

Conclusion: The Lévy index $α$ serves as a control parameter for tuning spectral singularities and absorption in fractional non-Hermitian systems, suggesting broader applications in waveguides and metamaterials.

Abstract: We investigate the scattering features of a non-Hermitian rectangular potential within the framework of space-fractional quantum mechanics. Using the Riesz fractional derivative, we analytically derive locus equations for spectral singularities (SSs) and their time-reversed counterparts, coherent perfect absorption (CPA), in a dimensionless complex-potential parameter space. This geometric locus formulation provides a transparent representation of the SS and CPA conditions and enables direct visualization of how fractional quantum dynamics modifies non-Hermitian scattering. We show that reducing the Lévy index $α$, which enhances nonlocal transport associated with Lévy-flight dynamics, systematically lowers the gain-loss strength required for the emergence of SSs and CPAs, while increasing the mode index further suppresses this threshold. In addition, for fixed potential parameters, we demonstrate that decreasing $α$ induces a blue shift of the SS energy, in direct agreement with earlier studies. From this perspective, the Lévy index $α$ emerges as a tunable control knob for SS-CPA settings in fractional non-Hermitian quantum systems. Beyond its quantum-mechanical setting, this study may find applications in fractional waveguides and metamaterials governed by fractional wave equations. This work also bridges the gap between non-Hermitian quantum mechanics and space-fractional quantum mechanics.

</details>


### [9] [Minimal-backaction work statistics of coherent engines](https://arxiv.org/abs/2603.01962)
*Milton Aguilar,Franklin L. S. Rodrigues,Eric Lutz*

Main category: quant-ph

TL;DR: A dynamic Bayesian network-based measurement scheme preserves quantum coherence and provides accurate energy exchange statistics for quantum engines, overcoming the limitations of standard measurement protocols.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of measuring work statistics in quantum engines affected by measurement backaction.

Method: Utilizing a dynamic Bayesian network-based measurement scheme that minimizes invasiveness and maintains quantum coherence during engine cycles.

Result: The proposed scheme allows accurate determination of energy exchange statistics, unlike traditional measurement approaches which may fail in coherent systems.

Conclusion: Universal fluctuation bounds may not apply to coherent machines, highlighting the specificity needed in understanding quantum engine behavior.

Abstract: Determining the work statistics of quantum engines is challenging due to measurement backaction. We here show that a dynamic Bayesian network-based measurement scheme, which preserves quantum coherence within an engine cycle, is minimally invasive, in the sense that the averaged measured state over one cycle exactly coincides with the unmeasured state. It therefore provides a general framework to investigate energy exchange statistics in quantum machines. This stands in contrast to the standard two-point measurement protocol, whose backaction can be so strong that it generally fails to reproduce the average work output of a coherent motor. It may even alter its mode of operation, causing it to cease functioning as an engine under observation. We further demonstrate that recently proposed universal fluctuation bounds do not necessarily apply to coherent machines.

</details>


### [10] [Constrained Quantum Optimization at Utility Scale: Application to the Knapsack Problem](https://arxiv.org/abs/2603.00260)
*Naeimeh Mohseni,Julien-Pierre Houle,Ibrahim Shehzad,Giorgio Cortiana,Corey O'Meara,Adam Bene Watts*

Main category: quant-ph

TL;DR: The paper demonstrates a hardware-efficient approach for solving constrained combinatorial optimization problems using copula-QAOA (cop-QAOA), particularly applied to the Unit Commitment problem in energy systems. It outperforms classical methods like Gurobi in certain instances with a few QAOA rounds on quantum hardware with up to 150 qubits.


<details>
  <summary>Details</summary>
Motivation: Constrained combinatorial optimization problems are significant in industry but challenging for quantum computing at utility-relevant scales, necessitating efficient solutions on near-term hardware.

Method: The paper introduces copula-QAOA (cop-QAOA), utilizing constant-depth mixers and biased initial states, to optimize single-period Unit Commitment problems reduced to one-dimensional knapsack formulations.

Result: The results indicate that cop-QAOA identifies solutions superior to a lazy greedy baseline and often matches or exceeds those provided by the classical solver Gurobi, using only a few QAOA rounds.

Conclusion: This work marks a significant advancement in demonstrating constrained combinatorial optimization on quantum hardware, particularly enforcing constraints through shallow mixers on larger scales.

Abstract: Constrained combinatorial optimization problems are challenging for quantum computing, particularly at utility-relevant scales and on near-term hardware. At the same time, these problems are of practical significance in industry; for example, the Unit Commitment (UC) problem in energy systems involves complex operational constraints. To address this challenge, we apply copula-QAOA (cop-QAOA), a hardware-efficient approach for constrained optimization to a single-period UC that can be reduced to a one-dimensional knapsack. Cop-QAOA biases the quantum state toward feasible solutions using constant-depth mixers and appropriately biased initial states. We implement our benchmark on problem instances that are confirmed to be hard for classical solvers such as Gurobi. Our results show that cop-QAOA often finds solutions better than a lazy greedy baseline and very close to, and in some instances surpasses, those obtained by Gurobi, with only a few QAOA rounds. This work presents the largest successful demonstration of the knapsack problem on IBM Quantum hardware using up to 150 qubits, and more generally, the largest demonstration of constrained combinatorial optimization where constraints are enforced via shallow mixers.

</details>


### [11] [Single-photon emitters and spin-photon interfaces in silicon](https://arxiv.org/abs/2603.02201)
*Kilian Sandholzer,Ian Berkman,Peter Deák,Carlos Errando-Herranz,Petros Filippatos,Adam Gali,Andreas Gritsch,Andreas Reiserer*

Main category: quant-ph

TL;DR: This review discusses the potential of silicon-based single-photon emitters and spin-photon interfaces for advancing quantum information technologies, highlighting their manufacturing advantages and addressing current challenges.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and practical sources of single photons for quantum technologies like communication and computing.

Method: Examining silicon as a material for single-photon emitters and spin-photon interfaces, particularly focusing on color centers and erbium dopants in nanophotonic structures.

Result: Silicon's advanced nanofabrication and high isotopic purity offer superior spin coherence, making it a strong candidate for quantum networks and distributed processors.

Conclusion: The review outlines the state-of-the-art technologies in silicon-based interfaces and the ongoing challenges to develop coherent single-photon sources.

Abstract: Single photons enable the distribution of quantum information over large distances and thus play a major role in quantum technologies such as communication and computing. Solid-state emitters are practical and efficient sources of single photons that can be manufactured in large numbers. When combined with a spin, the resulting spin-photon interfaces can store quantum states for extended periods and serve as the basis for quantum networks and repeaters. Among the many host materials explored over the past few decades, silicon stands out for its advanced nanofabrication, the maturity of its integrated photonics and microelectronics, and its high isotopic purity, which leads to exceptionally long spin coherence. These properties position silicon single-photon emitters and spin-photon interfaces among the most promising hardware platforms for implementing quantum networks and distributed quantum information processors. This review summarizes the current state of the art and open challenges towards coherent single-photon sources and scalable spin-photon interfaces based on color centers and erbium dopants in nanophotonic silicon structures.

</details>


### [12] [Characterization of Inner Control Electrode Shapes for Multi-Layer Surface-Electrode Ion Traps](https://arxiv.org/abs/2603.00348)
*Florian Ungerechts,Brigitte Kaune,Christian Ospelkaus*

Main category: quant-ph

TL;DR: This paper discusses the optimization of microfabricated surface-electrode traps for trapped-ion quantum processors through the use of asymmetric inner control electrode shapes, enhancing efficiency and scalability.


<details>
  <summary>Details</summary>
Motivation: To improve the design and efficiency of microfabricated surface-electrode traps for trapped-ion quantum processors by exploring asymmetric control electrode shapes.

Method: A systematic analysis and characterization of various asymmetric inner control electrode shapes to achieve simultaneous axial and radial control, validated in realistic use-case scenarios.

Result: Asymmetric inner electrodes increase compactness, reduce the need for outer electrodes, and improve voltage efficiency, allowing for a single DC supply from integrated Cryo-CMOS circuits.

Conclusion: The proposed design enhances the scalability and efficiency of surface-electrode traps, making them more effective for ion transport and finalizing micromotion compensation.

Abstract: Microfabricated surface-electrode traps are a scalable platform for trapped-ion quantum processors. Recent advances in fabrication techniques have enabled the design of increasingly complex multi-layer structures. Yet the control electrodes remain mostly unchanged and of rectangular shape. We systematically analyze asymmetric inner control electrode shapes for simultaneous axial and radial control in multi-layer surface traps, characterize and compare a selection of different shapes, and verify their capabilities in realistic use-case scenarios for ion transport and micromotion compensation. Eliminating the need for the commonly used additional outer control electrodes, asymmetric inner control electrodes increase the compactness and space efficiency of surface-electrode traps while concurrently reducing the number of control signals. The improved control voltage efficiency of using solely inner electrodes enables the device's entire direct-current (DC) supply to be provided by integrated Cryo-CMOS circuits, further enhancing the scalability of the processor.

</details>


### [13] [Genuine certifiable randomness from a black-box](https://arxiv.org/abs/2603.00401)
*Liam P. McGuinness*

Main category: quant-ph

TL;DR: This paper presents a method for black-box randomness certification in quantum mechanics, ensuring genuine randomness without assuming data generation methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing randomness certification methods that require assumptions about data generation, potentially certifying deterministic data as random.

Method: The authors demonstrate a black-box approach for randomness certification, where no deterministic adversary can successfully fool the certification process, employing measurements on single particle states.

Result: The proposed method guarantees the certification of genuine randomness, enabling the provable generation of random numbers without reliance on a random seed.

Conclusion: The success of this approach enhances the reliability of quantum-derived randomness in various applications, ensuring robustness against deterministic manipulation.

Abstract: Randomness is intrinsic to quantum mechanics; the outcome of a measurement on a quantum state is a random variable. This feature has been applied to randomness certification, where one party must decide whether the data they receive is truly random. However, existing demonstrations are not black-box, to avoid falsely certifying deterministic data, assumptions must be made on how the data was generated. Here we demonstrate genuine randomness certification in the black-box setting -- one in which no deterministic adversary, even with unlimited computational power, will succeed in getting their data certified. We use it to provably generate random numbers using only measurements on single particle states and without a random seed.

</details>


### [14] [Axiomatic Foundation of Quantum-Inspired Distance Metrics](https://arxiv.org/abs/2603.00406)
*Maryam Bagherian*

Main category: quant-ph

TL;DR: This paper presents an axiomatic framework for quantum-inspired distance metrics in projective Hilbert spaces, leading to a unified theory that establishes the Fubini-Study metric's uniqueness.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive structure that organizes and generalizes existing quantum distance measures, enhancing understanding in quantum information theory.

Method: The authors develop an axiomatic framework based on five fundamental axioms that govern the behavior of distance metrics in the context of quantum states.

Result: The study shows that all admissible distances are determined by state overlap and establishes the Fubini-Study metric as the unique canonical geodesic distance, along with producing a hierarchy of various distance metrics.

Conclusion: This work rigorously formalizes the geometric structure of quantum state spaces and connects abstract metric theory with operational principles in quantum mechanics.

Abstract: We develop a comprehensive axiomatic framework for quantum-inspired distance metrics on projective Hilbert spaces, providing a unified foundation that organizes and generalizes existing measures in quantum information theory. Starting from five fundamental axioms, projective invariance, unitary covariance, superposition sensitivity, entanglement awareness, and measurement contextuality, we show that any admissible distance depends solely on state overlap and establish the uniqueness of the Fubini-Study metric as the canonical geodesic distance. Our framework further yields a hierarchy of comparison results relating the Fubini-Study metric, Bures distance, Euclidean distance, measurement-based pseudometrics, and entanglement-sensitive distances. Key contributions include an entanglement-geometry complementarity principle, high-dimensional concentration bounds, and operational interpretations connecting distances to state discrimination and quantum metrology. This work places the geometry of quantum state spaces on a rigorous axiomatic footing, bridging abstract metric theory, information geometry, and operational quantum principles.

</details>


### [15] [Optimizing CMOS-compatible, superconducting Titanium Nitride Resonators: Deposition Conditions and Structuring Processes](https://arxiv.org/abs/2603.00441)
*Simon J. K. Lang,Alexandra Schewski,Ignaz Eisele,Johannes Weber,Carla Moran-Guizan,Zhen Lou,Moritz Singer,Benedikt Schoof,Marc Tornow,Thomas Mayer,Daniela Zahn,Rui N. Pereira,Christoph Kutter*

Main category: quant-ph

TL;DR: This study explores the fabrication and characteristics of TiN-based superconducting CPW resonators, focusing on how various parameters influence their performance, particularly the impact of crystal orientation and processing on dielectric losses. The results reveal optimized conditions yielding extremely low TLS losses, significant for qubit applications.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of superconducting coplanar waveguide resonators for quantum computing applications by exploring the influence of material properties and fabrication processes.

Method: Fabrication of TiN thin films on high-resistivity Si substrates, systematic variation of sputtering conditions, etch power, and resist strip temperatures, along with characterizing losses in the films through experimental measurements.

Result: Achieved distinct crystal orientations in TiN films, with TiN111 showing the lowest TLS losses and improved robustness. Found that structuring processes significantly influenced TLS loss more than crystal orientation; lowest loss values reached $9.67 \times 10^{-7}$ with a resulting quality factor near one million.

Conclusion: Process-induced oxide formation critically limits resonator performance, and optimized processing techniques can yield low-loss TiN resonators, making them suitable for scalable CMOS qubit fabrication.

Abstract: We report on the fabrication and characterization of superconducting coplanar waveguide (CPW) resonators based on titanium nitride (TiN) thin films deposited on 200\,mm diameter high-resistivity Si(100) substrates. We systematically investigate how deposition conditions, dry-etch power and in-situ resist strip temperature affect morphology, superconducting properties and dielectric losses. By tuning reactive sputtering conditions, three distinct preferred crystal orientations - (111), (200), and mixed are achieved. Our results demonstrate that all films exhibiting similar minimal two-level system (TLS) losses, with TiN111 exhibit the lowest median TLS losses $\tildeδ_\mathrm{TLS}$, and greater robustness against reoxidation. The applied structuring process, in contrast, had a far greater influence on the TLS loss than the crystal orientation of the TiN film and, consequently, the intrinsic material properties of the superconducting layer. The lowest TLS losses for all TiN depositons were achieved with a low power etch and low temperature resist strip. An additional buffered oxide etch (BOE) treatment could remove high-loss interfacial oxides at the metal-air (MA) and substrate-air (SA) interface and recover the etch-induced TLS losses. Consequently, TiN resonators exhibiting $\tildeδ_\mathrm{TLS}$ values as low as $9.67 \times 10^{-7}$ were realized. The corresponding median low-power loss, $\tildeδ_\mathrm{LP}$, amounts to $11.04 \times 10^{-7}$, which translates to an internal quality factor approaching one million. These findings highlight the critical role of process induced oxide formation at the MA and SA interfaces in limiting the performance of TiN resonators and provide a scalable, low-loss process compatible with industry-grade 200 mm CMOS qubit fabrication workflows.

</details>


### [16] [Nonclassical Many-Body Superradiant States with Interparticle and Spin-Momentum Entanglement](https://arxiv.org/abs/2603.00463)
*Jarrod T. Reilly,Gage W. Harmon,John Drew Wilson,Murray J. Holland,Simon B. Jäger*

Main category: quant-ph

TL;DR: The paper describes a cross-cavity system achieving steady-state superradiance through collective dissipative dynamics, highlighting significant entanglement outcomes.


<details>
  <summary>Details</summary>
Motivation: To explore superradiance and nonclassical properties in a coupled cavity-atom system influenced by collective dynamics.

Method: The system employs two coupled cavities with four-level atoms, focusing on collective atomic decay and off-resonant Raman transitions.

Result: The findings reveal steady-state superradiant states exhibiting nonclassical photon statistics and substantial hybrid entanglement between atomic spin and motional states.

Conclusion: The research demonstrates potential for quantum-enhanced applications, particularly in sensing, through heralded measurements facilitating particle-particle entanglement.

Abstract: We present a cross-cavity system in which steady-state superradiance is achieved using solely collective dissipative dynamics. Two cavities symmetrically couple an ensemble of four-level atoms by driving transitions between two electronic states and two motional states along perpendicular cavity axes. Both cavities operate in the bad-cavity regime: one cavity mediates collective atomic decay, while the other cavity, together with a coherent drive, mediates collective pumping via an off-resonant Raman transition. With this, we find steady-state superradiant states that possess nonclassical properties, such as super-Poissonian photon statistics. The system thus requires a beyond mean-field description, and so we develop an exact master equation simulation technique utilizing strong symmetries of the system's jump operators. Because superradiant decay is accompanied by a momentum impulse along the corresponding cavity axis, the system exhibits substantial hybrid entanglement between the atoms' spin and motional degrees of freedom at steady state. We also demonstrate that heralded measurements of the two cavity outputs prepare a state with significant particle-particle entanglement with prospects for quantum-enhanced acceleration sensing.

</details>


### [17] [Efficient Polynomial-Scaled Determination of Algebraic Entanglement Entropy Between Collective Degrees of Freedom](https://arxiv.org/abs/2603.00464)
*John Drew Wilson,Jarrod T. Reilly,Murray J. Holland*

Main category: quant-ph

TL;DR: This paper presents a method to calculate algebraic entanglement entropy in physical systems with multipartite and intraparticle entanglement, using symmetries and irreducible representations to achieve polynomial complexity in calculations.


<details>
  <summary>Details</summary>
Motivation: To explore systems with complex entanglement structures involving multipartite and intraparticle entanglement, aiming to simplify calculations of entanglement entropy.

Method: The method involves leveraging permutation symmetries and irreducible representations of Lie groups to diagonalize reduced density matrices in a block-wise fashion, leading to polynomial complexity calculations.

Result: The procedure enables the calculation of algebraic entanglement entropy that scales linearly with particle number despite being in systems that would normally require exponentially-scaled Hilbert space for similar results.

Conclusion: This approach opens avenues for exact simulations of entangled systems with manageable computational resources, highlighting the potential of using symmetries for efficient entanglement analysis.

Abstract: In this work, we explore physical systems which support not only multipartite interparticle entanglement, but also intraparticle entanglement between different degrees of freedom of the constituent particles and entanglement between different degrees of freedom of different particles, i.e., algebraic entanglement. We derive a simple method for calculating the algebraic entanglement entropy between two of the particles' degrees of freedom from collective states of the whole ensemble. Our procedure makes use of underlying symmetries in these systems, in particular permutation symmetry of the particle indices, and shows a connection between the algebraic entanglement entropy in these systems and the irreducible representations of Lie groups which describe the particles' degrees of freedom. Namely, we use the direct sum over irreducible representations to diagonalize the reduced density matrices in a block-by-block manner, then utilize the multiplicity of these irreducible representations to reproduce the results from an exponentially-scaled Hilbert space in only polynomial complexity. We use this to explore a variety of systems where the constituent particles support two degrees of freedom each with two levels, such as atoms with two electronic states and two momentum states. Notably, these systems may be exactly simulated in a polynomial-scaled Hilbert space, yet they support an algebraic entanglement entropy that grows linearly with the particle number which typically requires an exponentially-scaled Hilbert space.

</details>


### [18] [A Stable and General Quantum Fractional-Step Lattice Boltzmann Method for Incompressible Flows](https://arxiv.org/abs/2603.00558)
*Yang Xiao,Liming Yang,Chang Shu,Yinjie Du*

Main category: quant-ph

TL;DR: This paper introduces a quantum fractional-step lattice Boltzmann method (FS-LBM) to enhance stability and precision in simulations of fluid dynamics, outperforming previous models.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations in existing quantum lattice Boltzmann methods that restrict simulations to a single Reynolds number and suffer from instability.

Method: The FS-LBM employs a quantum circuit for the predictor step and a classical method for the corrector step, maintaining a relaxation time of $τ$ = 1 for compatibility.

Result: The FS-LBM demonstrates accuracy and convergence consistent with classical models, significantly improving precision and stability compared to earlier quantum methods.

Conclusion: The proposed framework successfully executes the first quantum LBM simulation of three-dimensional incompressible thermal flows, showcasing its potential in advancing quantum computing applications in fluid dynamics.

Abstract: Quantum computing shows substantial potential in accelerating simulations and alleviating memory bottlenecks in computational fluid dynamics (CFD), owing to its inherent properties of superposition and entanglement. The lattice Boltzmann method (LBM), being largely algebraic in nature, has inspired the development of various quantum LBMs. However, most existing approaches fix the relaxation time at $τ$ = 1, thereby confining a given mesh resolution to simulations at a single Reynolds number. Although our earlier quantum lattice kinetic scheme (LKS) lifted this restriction, it suffers from instability at high Reynolds numbers. To address this challenge, we propose a quantum fractional-step LBM (FS-LBM). In this framework, the predictor step is implemented on a quantum circuit using the standard LBM formulation, while the corrector step is performed classically. The relaxation time is retained at $τ$ = 1 to ensure seamless compatibility with existing quantum LBMs. Benchmark simulations of representative two- and three-dimensional incompressible isothermal and thermal flows demonstrate that the quantum FS-LBM achieves accuracy and convergence orders consistent with its classical counterpart, while significantly outperforming the quantum LKS in both precision and stability. Notably, this work presents the first quantum LBM simulation of three-dimensional incompressible thermal flows.

</details>


### [19] [Closing the Loop: Resource-aware Hybrid NAS Guided by Analytical and Hardware-Calibrated Quantum Cost Modeling](https://arxiv.org/abs/2603.00625)
*Muhammad Kashif,Alberto Marchisio,Muhammad Shafique*

Main category: quant-ph

TL;DR: The paper presents a hybrid quantum-classical neural network framework that optimizes resource estimation and performance through an analytical cost model.


<details>
  <summary>Details</summary>
Motivation: Current methods for estimating hardware resources in hybrid quantum-classical neural networks often overlook crucial quantum-specific factors, making evaluations inaccurate on real devices.

Method: An analytical quantum cost model incorporating gate durations, routing overheads, and noise is developed, alongside a classical cost model that translates FLOPs into device-specific throughput. This forms the basis for Hyb-HANAS, a framework that optimizes accuracy, hardware cost, and parameter count using NSGA-II.

Result: Hyb-HANAS identifies Pareto-optimal trade-offs and optimally co-adapts classical and quantum components, improving the efficiency of HQNNs on real quantum hardware.

Conclusion: The proposed models are essential for accurate resource estimation and have broader applications in quantum hardware benchmarking and compiler evaluation.

Abstract: Hybrid quantum-classical neural networks (HQNNs) integrate quantum circuits with classical layers, each operating under fundamentally different computational paradigms, which makes hardware resource estimation challenging. The training of quantum circuits on real devices requires thousands of circuit executions, which is impractical on current NISQ devices. Therefore, most HQNNs are evaluated on classical simulators, with hardware cost approximated using floating-point operations (FLOPs). However, FLOPs and existing quantum resource estimation methods (e.g., gate counts) overlook key quantum hardware-specific factors such as gate durations, limited qubit connectivity, and noise, all of which ultimately determine the true cost and scalability of quantum circuits.
  In this paper, we propose an analytical quantum cost model that estimates quantum hardware resources using real backend calibration data, incorporating gate durations, routing overheads, and noise-induced sampling inefficiencies. To complement this, we develop a classical cost model that converts FLOPs into device-specific throughput, enabling a unified time-based representation of hardware resource cost for both subsystems of HQNNs. Building on these analytical models, we present Hyb-HANAS, a hardware-aware hybrid neural architecture search framework, which jointly optimizes accuracy, hardware cost, and parameter count using NSGA-II. Hyb-HANAS identifies Pareto-optimal trade-offs and cross-domain co-adaptation between classical and quantum components of HQNNs. Beyond NAS, the proposed analytical quantum cost model is broadly applicable to quantum hardware benchmarking, compiler evaluation, and training-time estimation of quantum circuits on NISQ devices.

</details>


### [20] [Instantons In A Symmetric Quartic Potential](https://arxiv.org/abs/2603.00652)
*Pervez Hoodbhoy,M. Haashir Ismail,M. Mufassir*

Main category: quant-ph

TL;DR: The paper analyzes tunneling in a quartic potential with four minima using semi-classical methods and Feynman path integrals, identifying various instanton configurations and deriving results on energy levels and oscillations.


<details>
  <summary>Details</summary>
Motivation: To extend semi-classical tunneling analysis in potential systems with multiple minima, specifically focusing on quartic potential with four degenerate minima.

Method: Utilizing Feynman path integrals in imaginary time to identify instanton configurations, transforming to a rotating frame for zero-mode handling, and generalizing the dilute instanton gas approximation.

Result: Derived coherent Rabi-type oscillations and energy splittings of the lowest states, validating results against numerical methods, particularly at deep semi-classical limits.

Conclusion: Identified a critical coupling regime demonstrating a transition from discrete $D_4$ symmetry to continuous $O(2)$ symmetry, indicating limitations of the localized instanton description.

Abstract: We extend the semi-classical analysis of the double-well potential to a quartic system featuring four degenerate minima. Utilizing the Feynman path integral in imaginary time, we identify longitudinal, transverse, and diagonal instanton configurations that mediate tunneling between minima. The zero mode is handled by transforming to a rotating frame whose origin lies on the classically determined path. By generalizing the dilute instanton gas approximation to account for these distinct pathways, we derive the coherent Rabi-type oscillations and the energy splittings of the four lowest-lying states. These semi-classical results are validated against high-precision numerical diagonalization, showing excellent agreement in the deep semi-classical limit. We further identify a critical coupling regime where the discrete $D_4$ symmetry undergoes a `melting' transition into a continuous $O(2)$ rotational symmetry, signaling a fundamental breakdown of the localized instanton description.

</details>


### [21] [Toward Quantum-Optimized Flow Scheduling in Multi-Beam Digital Satellites](https://arxiv.org/abs/2603.00701)
*Qiben Yan,John P. T. Stenger,Daniel Gunlycke*

Main category: quant-ph

TL;DR: This paper presents a hybrid quantum-classical framework to optimize data flow scheduling for multibeam satellites, improving efficiency and addressing NP-hard challenges by leveraging QUBO formulations and a novel training strategy.


<details>
  <summary>Details</summary>
Motivation: The need for high-throughput scheduling in multibeam satellites while overcoming limitations of traditional methods.

Method: A hybrid quantum-classical approach reformulates the scheduling problem as a QUBO, incorporating throughput-maximization and constraints, and a layer-wise training strategy for variational quantum algorithms.

Result: The framework demonstrates improved scheduling efficiency with better solution quality and runtime on quantum hardware as compared to classical and hybrid methods under realistic satellite traffic.

Conclusion: The proposed method offers a significant advancement in scheduling for multibeam satellites, showcasing the potential of hybrid quantum-classical techniques in solving complex optimization problems.

Abstract: Data flow scheduling for high-throughput multibeam satellites is a challenging NP-hard combinatorial optimization problem. As the problem scales, traditional methods, such as Mixed-Integer Linear Programming and heuristic schedulers, often face a trade-off between solution quality and real-time feasibility. In this paper, we present a hybrid quantum-classical framework that improves scheduling efficiency by casting Multi-Beam Time-Frequency Slot Assignment (MB-TFSA) as a Quadratic Unconstrained Binary Optimization (QUBO) problem. We incorporate the throughput-maximization objective and operational constraints into a compact QUBO via parameter rescaling to keep the formulation tractable. To address optimization challenges in variational quantum algorithms, such as barren plateaus and rugged loss landscapes, we introduce a layer-wise training strategy that gradually increases circuit depth while iteratively refining the solution. We evaluate solution quality, runtime, and robustness on quantum hardware, and benchmark against classical and hybrid baselines using realistic, simulated satellite traffic workloads.

</details>


### [22] [Compile-once block encodings for masked similarity-transformed effective Hamiltonians](https://arxiv.org/abs/2603.00761)
*Bo Peng,Yuan Liu,Karol Kowalski*

Main category: quant-ph

TL;DR: COMPOSER is an advanced computational architecture for simulating electronic structures using efficient Hamiltonian reductions and parametric variations.


<details>
  <summary>Details</summary>
Motivation: To enhance the efficiency and accuracy of electronic-structure operator simulations by reducing the complexity of Hamiltonians and operators.

Method: Utilizes low-rank factorizations to transform Hamiltonians into manageable forms, compiles a two-qubit architecture, employs a PREP-SELECT-PREP template, and a singular QSP polynomial for spectral transformations.

Result: Achieves near-linear scaling in resource usage and allows for adjustable algorithmic accuracy while maintaining computational stability and resource efficiency.

Conclusion: COMPOSER offers a robust framework for effective Hamiltonian construction and execution, balancing algorithmic accuracy with flexibility based on user-defined parameters.

Abstract: We present COMPOSER, a compile-once modular parametric oracle for similarity-encoded effective reduction of electronic-structure operators (e.g., Schrieffer-Wolff-type constructions). Low-rank factorizations compress Hamiltonians and anti-Hermitian generators into rank-one bilinear and projected-quadratic ladders with near-linear scaling at fixed thresholds; each ladder admits deterministic, number-conserving preparation and a block encoding using constant number of signal ancillas. A fixed PREP-SELECT-PREP template multiplexes these ladders, and one QSP polynomial performs the spectral transformation with degree set by operator norms. For a fixed orbital pool and qubit register, the two-qubit fabric is compiled once; geometry, active-space (mask) updates, and truncations are absorbed by re-dialed single-qubit rotations. We introduce a mask-aware similarity-sandwich effective-Hamiltonian construction and benchmark stability under low-rank and second-order-perturation-guided screening. COMPOSER is an execution architecture: algorithmic errors (block-encoding and QSP approximation) are tunable for any supplied parameters, while physical accuracy depends on how those parameters are obtained if not refined.

</details>


### [23] [QANTIS: A Hardware-Validated Quantum Platform for POMDP Planning and Multi-Target Data Association](https://arxiv.org/abs/2603.00785)
*Bayram Yüksel Eker,Suayb S. Arslan,Özgür Nazlı,Mustafa Serhat Demirgil,Furkan Deligöz*

Main category: quant-ph

TL;DR: The paper introduces QANTIS, a modular platform that integrates quantum techniques for enhancing multi-target data association and belief updates in autonomous navigation, demonstrating significant computational efficiencies and characterizing the operating limits of current superconducting quantum hardware.


<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of planning and sensor measurement assignment in autonomous navigation using POMDPs, particularly the high complexity of data association.

Method: QANTIS integrates quantum amplitude amplification for belief updates and QUBO-based optimization for data association, evaluated through a series of hardware experiments on IBM's superconducting quantum processors.

Result: The study shows that quantum techniques significantly enhance belief update efficiencies, amplifying rare observation probabilities and maintaining Bayesian posterior accuracy, while identifying practical scalability boundaries for hardware utilization.

Conclusion: Results emphasize the importance of characterizing operational limits on current quantum architectures, highlighting that while quantum advantages are evident at certain scales, they do not yet translate to a universal wall-clock speedup for all problem sizes.

Abstract: Autonomous navigation under uncertainty requires solving partially observable Markov decision processes (POMDPs) for planning and assigning sensor measurements to tracked targets--a task known as multi-target data association (MTDA). Both problems become computationally demanding at scale: belief conditioning costs $\mathcal{O}(P(e)^{-1})$ per node under rare evidence, while MTDA is NP-hard. Quantum amplitude amplification can quadratically reduce the belief-update query cost to $\mathcal{O}(P(e)^{-1/2})$, while QUBO reformulations expose MTDA to quantum and quantum-inspired optimisation heuristics. We present QANTIS, a modular platform that integrates quantum belief update (Grover amplitude amplification and BIQAE), QUBO-based data association via FPC-QAOA, and composable error mitigation, and we report a 45-experiment hardware study on three IBM Heron backends. On hardware, a single Grover iterate applied to a Tiger belief oracle amplifies a rare observation probability from $0.179$ to $0.907$ ($5.1\times$; ISA 18) while preserving the Bayesian posterior (Hellinger $0.0015$), increasing usable-shot yield from 1,463 to 7,429. We interpret this as a hardware validation of the quadratic query-complexity mechanism at $k=1$ with posterior preservation, rather than a wall-clock advantage claim. We further demonstrate, to our knowledge, the first closed-loop hybrid quantum-classical Tiger POMDP on superconducting hardware ($T=8$, max Hellinger below $0.015$), and empirically characterise NISQ feasibility boundaries: ZNE-based error mitigation is beneficial below ISA $\approx 100$ and harmful above ISA $\gtrsim 1{,}000$; FPC-QAOA is meaningful at $\leq 15$ QUBO variables (ISA $\lesssim 450$). These results characterise practical operating regimes on current superconducting hardware rather than wall-clock quantum advantage at today's problem scales.

</details>


### [24] [Lissajous coherent states via projection](https://arxiv.org/abs/2603.00788)
*Errico J. Russo,James Schneeloch,Edwin E. Hach,Richard J. Birrittella,Wanda Vargas,Christopher C. Gerry*

Main category: quant-ph

TL;DR: The paper constructs stationary coherent states for harmonic oscillators, detailing their relationships to classical motion and defining vortex states, culminating in a resolution of identity for SU(2) coherent states.


<details>
  <summary>Details</summary>
Motivation: To explore the properties of coherent states and their connections to classical motions in harmonic oscillators, particularly focusing on Lissajous figures and quantum interference.

Method: States are constructed by projecting products of coherent states onto sets of degenerate states in order to derive stationary states linked to the dynamics of two-dimensional harmonic oscillators.

Result: A clear connection between probability current flow and quantum interference is established, and a quantifiable definition for vortex states in the 2D harmonic oscillator is provided.

Conclusion: The constructed stationary states are validated as true coherent states through the resolution of identity operators, also revealing that isotropic oscillator states correspond to SU(2) coherent states.

Abstract: We construct stationary coherent states concentrated on Lissajous figures of the isotropic and anisotropic harmonic oscillators, the latter having coprime frequencies, by projecting products of ordinary coherent states (one coherent state for each degree of freedom) onto sets of degenerate states. By performing these projections, we are deriving our states from sets of coherent states that are known to follow the classical motion of a two-dimensional harmonic oscillator for arbitrary frequencies. We clarify the nature of any singularities present in the phase of the wavefunction for each of the states we derive, and we establish a rigorous connection between the laminar flow of probability current and the emergence of quantum interference. Through this analysis, we are able to provide a clear and quantifiable definition for a vortex state of the two-dimensional harmonic oscillator (2DHO). In an appendix, we show that our stationary states are true coherent states as they can be used to resolve the relevant identity operators (the above mentioned projection operators) on their respective degenerate subspaces. In the special case of the isotropic oscillator, the states obtained are the SU(2) coherent states, and we derive from our formalism the familiar resolution of unity for these states.

</details>


### [25] [ReloQate: Transient Drift Detection and In-Situ Recalibration in Surface Code Quantum Error Correction](https://arxiv.org/abs/2603.00837)
*Maxwell Poster,Jason Chadwick,Jonathan Mark Baker*

Main category: quant-ph

TL;DR: The paper presents a method for real-time prediction of logical error rates in quantum error correction, addressing noise drift in quantum hardware by leveraging detector fire rate data, allowing timely responses and spatial remapping of logical qubits.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of variable noise rates in real quantum hardware that can degrade quantum error correction performance.

Method: The authors develop a real-time prediction model for logical error rates based on transient data from detector fire rates, allowing for immediate error handling without extensive prior characterization.

Result: The proposed method effectively predicts logical error rates and facilitates the remapping of logical qubits in a patch-based architecture, significantly enhancing practical implementation of quantum error correction.

Conclusion: By combining DFR-based predictions with spatial remapping, this approach provides a timely and efficient way to manage error rates, advancing the field of quantum error correction.

Abstract: Quantum error correction (QEC) promises to exponentially suppress qubit noise, but typically assumes spatially-uniform and temporally-constant noise rates. However, real quantum hardware exhibits variation in noise levels over time, which will be amplified by QEC if not addressed. To mitigate this drift in error rates, we leverage transient information readily available in surface code quantum error correction to predict logical error rates (LER) in real time. We infer a prediction model by sampling physical error rates from real hardware, and mapping detector fire rate (DFR), or parity of stabilizer measurements across QEC rounds, to LER. This allows for on-the-fly LER predictions without the typical characterization overhead required to determine LER. This method can easily be extended to other stabilizer codes. Importantly, we observe that this prediction should be accurate yet conservative (i.e. give an upper estimate) to enable appropriately fast responses to real-time physical error changes. That is, responses should be executed marginally ahead of time to allow for their execution to complete, and minimize time spent (ideally none) above intolerable error rates. More importantly, we pair this predictor with a scheme which remaps drifted logical qubits to fresh tiles in a patch-based architecture while their original tiles are recalibrated. Our results demonstrate DFR-based prediction to be an effective LER predictor, and remapping as a spatially efficient and timely mitigation response for small code distances, both of which are significant steps in furthering practical QEC.

</details>


### [26] [Three-Qubit State Preparation: Classification and Explicit Circuits](https://arxiv.org/abs/2603.00845)
*Yonghae Lee,Taewan Kim*

Main category: quant-ph

TL;DR: The paper presents a systematic method for preparing arbitrary three-qubit pure states, classifying them by entanglement structure and providing explicit circuit templates that optimize gate count and depth.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and practical quantum circuits for three-qubit state preparation in quantum information applications.

Method: States are classified into five types based on a $1|2$ bipartition. The authors derive circuit templates using elementary gates and Schmidt decomposition, providing clear procedural steps.

Result: The approach improves practicality by reducing gate counts and circuit depth compared to previous methods, creating a direct algorithm for instantiation.

Conclusion: The presented framework is a significant advancement for preparing three-qubit states efficiently, with clear implications for practical quantum circuit design.

Abstract: We present a deterministic framework for preparing an arbitrary three-qubit pure state. To leverage entanglement structure in the state-preparation task, we classify three-qubit pure states into five types with respect to a $1|2$ bipartition. Given a target state specified by its amplitudes, we provide concrete criteria and concurrence-based tests that determine its type. For each type, we derive an explicit circuit template composed of elementary single-qubit rotations and CNOT gates, with gate parameters determined systematically from the Schmidt decomposition. The full construction is described step by step from the target amplitudes, with no procedural ambiguity. As an application, we further group frequently encountered three-qubit pure states in quantum information into four classes and provide an explicit circuit for each class. Compared with prior approaches, our circuits are designed for practical use: they admit a direct algorithmic instantiation, use only CNOT gates between adjacent qubits, and for certain classes achieve smaller gate counts and circuit depth.

</details>


### [27] [Multipartite device-independent quantum key distribution using W states](https://arxiv.org/abs/2603.00937)
*Makoto Ishihara,Wojciech Roga,Masahiro Takeoka*

Main category: quant-ph

TL;DR: The paper explores multipartite device-independent quantum key distribution (DI-QKD) using W states instead of GHZ states, demonstrating its feasibility and proposing new protocols for secure key distribution over longer distances.


<details>
  <summary>Details</summary>
Motivation: To investigate the possibility of multipartite DI-QKD using W states rather than the traditionally used GHZ states, focusing on enhancing practical implementation.

Method: The authors construct Bell inequalities that are significantly violated by W states and analyze various implementation scenarios, including detection efficiency and long-distance protocols with single-photon interference.

Result: The proposed W state-based multipartite DI-QKD shows the potential for longer-distance key distribution than existing protocols based on GHZ states, with practical implementation analyses provided.

Conclusion: This work offers new insights into the link between multipartite entanglement and device-independent quantum processing and suggests a viable alternative for achieving long-distance multipartite DI-QKD.

Abstract: Multipartite device-independent quantum key distribution (DI-QKD), also known as device-independent conference key agreement, enables more than two remote parties to share a common key with information-theoretic security even without trusting the devices. So far, several multipartite DI-QKD protocols have been proposed where Greenberger-Horne-Zeilinger (GHZ) states are used as multipartite entanglement. A natural question is then whether one can construct multipartite DI-QKD with the other type of multipartite entanglement. W state is of particular interest since it is intrinsically different from GHZ state and in some cases, easier to optically implement. In this paper, we show that multipartite DI-QKD is possible with W states. To this end, we construct Bell inequalities largely violated by W states, which can be used for the multipartite DI-QKD. Furthermore, we consider several different implementation scenarios. First, we analyze the minimum required detection efficiencies to extract finite amount of keys. Then we propose a long-distance multipartite DI-QKD protocol with single-photon interference and make detailed analyses with several physical implementation scenarios. We show that the protocol enables secret key distribution over longer distances than the existing multipartite DI-QKD protocols based on GHZ states. This study provides new insight about the relationship between multipartite entanglement and device-independent quantum information processing as well as opens an alternative path toward long-distance multipartite DI-QKD.

</details>


### [28] [Nonlocality distillation can outperform entanglement distillation](https://arxiv.org/abs/2603.00940)
*Peter Høyer,Jibran Rashid,Razeen ud Din*

Main category: quant-ph

TL;DR: The paper compares entanglement and nonlocality distillation strategies for maximizing CHSH violation, showing that nonlocality distillation is more effective, especially with fewer copies of the shared state.


<details>
  <summary>Details</summary>
Motivation: To explore efficient strategies for maximizing CHSH violation in quantum resources.

Method: A comparative analysis of entanglement and nonlocality distillation with differing numbers of shared state copies.

Result: Nonlocality distillation achieves a higher CHSH value than entanglement distillation for small state copies and shows superior resource efficiency.

Conclusion: Nonlocality distillation is more effective than entanglement distillation, especially in low-copy scenarios.

Abstract: Given the goal of maximizing CHSH violation, we compare the optimal strategies of entanglement and nonlocality distillation. In the limit of the number of copies of the shared state, entanglement distillation is guaranteed to work by generating a Bell state. For a small number of copies of the state, we show that nonlocality distillation can achieve a higher CHSH value, even though optimal entanglement distillation requires communication. Nonlocality distillation not only outperforms entanglement distillation but also demonstrates superior resource efficiency across multiple metrics for quantum resource estimation.

</details>


### [29] [Measuring Bell non-locality in the presence of signaling](https://arxiv.org/abs/2603.00984)
*Mark Broom,Talel Naccache,Emmanuel M. Pothos,Christoph Gallus,Pawel Blasiak*

Main category: quant-ph

TL;DR: The paper introduces a new method for quantifying Bell non-locality, allowing analysis in scenarios where signaling occurs, while preserving locality as much as possible.


<details>
  <summary>Details</summary>
Motivation: To address how Bell non-locality can be understood and quantified in the presence of signaling, as traditional analyses typically assume non-signaling conditions.

Method: A linear-programming framework is employed to find an optimal convex decomposition of observed correlations, separating local and genuinely non-local components.

Result: The method provides a closed-form solution for quantifying correlations and evaluates a measure of signaling, demonstrating broad applicability and meaningful insights.

Conclusion: This framework extends the definition of non-locality, offering new tools for experimental analysis in physics and beyond.

Abstract: Scientific inquiry seeks causal explanations of observed phenomena. The Bell experiment provides a paradigmatic case, revealing correlations between spatially separated systems that no local model can reproduce. Such correlations, known as Bell non-locality, are typically analyzed under the non-signaling assumption, which requires that local statistics be independent of distant measurement choices. Yet real experiments, as well as applications beyond physics, often involve signaling, raising the question of how non-locality should be characterized without this constraint. We introduce a general method for quantifying Bell non-locality in the presence of signaling, designed to relax locality as little as necessary. Our approach is guided by the question: how often can locality be preserved across repeated trials in explaining the observed correlations? The task reduces to finding the optimal convex decomposition of the observed correlations into local and genuinely non-local components. We solve this problem within the linear-programming framework, obtaining a closed-form solution valid for arbitrary correlations. We further evaluate a corresponding measure of signaling, demonstrating the generality of the method and the non-trivial character of the results. By extending the notion of non-locality beyond the non-signaling regime, our framework reshapes the basis for experimental analysis in physics and offers tools applicable outside physics.

</details>


### [30] [Power Network SCADA Quantum Communications: A Comparison of BB84, B92, E91, and SGS04 Quantum Key Distribution Protocols](https://arxiv.org/abs/2603.01060)
*Hillol Biswas,Kyriakos Zoiros*

Main category: quant-ph

TL;DR: This paper explores the integration of quantum communication, particularly quantum key distribution (QKD), into power system SCADA networks to enhance security amidst large data volumes and real-world complexities.


<details>
  <summary>Details</summary>
Motivation: To optimize the use of optical fiber-based power network SCADA systems with quantum communication, addressing trends and practical challenges.

Method: Assessment and comparison of QKD protocols (BB84, E91, B92, SARG04) applied to large multivariate power-system datasets.

Result: Simulation outcomes indicate varying performance of QKD protocols when applied to power system SCADA, providing insights into their effectiveness in real-world scenarios.

Conclusion: Leveraging QKD protocols with quantum electronics hardware presents a viable path for enhancing SCADA/PMU network security in actual power systems.

Abstract: The current state, emerging trends, and practical challenges of optical fiber-based power network SCADA quantum communication must be addressed to fully utilise the technological platform's potential in real-world power system SCADA communications involving massive volumes of real-time data, as well as in managing, encoding, and applications such as quantum cryptography. Quantum key distribution (QKD) is an essential part of the cybersecurity paradigm for quantum communication. Even though quantum computing with individual circuits yields probabilistic outcomes for the problem at hand, real-world datasets are complex and challenging to handle, even with telemetry. When using the cybersecurity triad of availability, confidentiality, and integrity (CIA) in reverse order (AIC), availability is given priority in electric power networks. This research assesses the use of the BB84, E91, B92, and SARG04 cryptographic protocols by applying them to large, multivariate power-system SCADA datasets and comparing the outcomes. By leveraging the variety of QKD protocols available with quantum electronics hardware, this simulation work provides a promising avenue for developing frameworks and deploying SCADA/PMU networks in actual power systems.

</details>


### [31] [Multipartite parity bounds and total correlation](https://arxiv.org/abs/2603.01105)
*James Tian*

Main category: quant-ph

TL;DR: The paper analyzes multipartite observables formed from sums of local self-adjoint contractions, demonstrating how their parity structure results in norm bounds and correlations within tensor product Hilbert spaces.


<details>
  <summary>Details</summary>
Motivation: To explore the characteristics and limits of multipartite observables in quantum systems, particularly under local interactions.

Method: The authors decompose local product observables into commutator and anticommutator components, derive norm bounds using pairwise defect weights, and establish connections to information theory and total correlation.

Result: The square of the multipartite sum exhibits a parity structure that cancels odd terms; a norm bound is established alongside a lower bound on total correlation contingent on local norms.

Conclusion: The findings provide an explicit threshold for observable expectations related to total correlation, highlighting a decay mechanism under local noise in quantum systems.

Abstract: This paper studies multipartite observables formed from sums of local self-adjoint contractions on tensor product Hilbert spaces. The square of such a sum has a parity structure: after decomposing each local product into commutator and anticommutator parts, the odd parity terms cancel and only even parity contributions remain. This yields a norm bound in terms of a family of pairwise defect weights built from local commutator and anticommutator norms. These defect weights also control an information theoretic estimate. The excess of the observable expectation above the product state threshold is shown to necessarily carry a definite amount of total correlation. Under a natural $\ell^{2}$-type bound on each local family, this product state threshold becomes explicit, which leads to a fully explicit lower bound on total correlation. A simple depolarizing example illustrates the resulting decay mechanism under local noise.

</details>


### [32] [A high-performance quantum memory for quantum interconnects](https://arxiv.org/abs/2603.01156)
*H. -X Luo,C. Li,J. -L. Ren,Y. Yuan,Y. -L. Wen,J. -F. Li,Y. -F. Wang,S. -C. Zhang,H. Yan,S. -L. Zhu*

Main category: quant-ph

TL;DR: The paper presents a high-performance quantum memory that enhances quantum communication by optimizing multiple metrics, achieving over 80% efficiency and 99% fidelity, enabling efficient storage of high-dimensional qubits.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of transmission loss and enhance the performance of quantum communication through optimized quantum memory in quantum repeaters.

Method: The authors introduce a quantum interconnect rate to quantify quantum memories and develop a memory that integrates multimode capacity, efficiency, and fidelity, operating on 11-dimensional spatial modes.

Result: The memory achieves over 80% efficiency and above 99% fidelity, capable of storing high-dimensional qudits and facilitating the distribution of 3.56 bits of quantum information over a 1000-km repeater link in one minute.

Conclusion: This work paves the way for scalable quantum interconnects and networks by optimizing quantum memory performance metrics.

Abstract: Single photons are the flying qubits of choice for distributing entanglement in a quantum internet. Quantum memories embedded in quantum repeaters are crucial to overcome transmission loss and enhance the rate of quantum communication. A multimode memory can further boost the channel capacity. However, benchmarking and building a practical quantum memory that simultaneously optimizes multiple performance metrics poses two key challenges. Here, we introduce quantum interconnect rate to comprehensively quantify quantum memories, and further demonstrate a high-performance quantum memory that simultaneously integrates three essential criteria at once: large multimode capacity, high efficiency, and high fidelity. Operating on 11-dimensional spatial modes, our memory achieves a uniform efficiency exceeding 80% and qubit storage fidelities above 99%, enabling the efficient storage of high-dimensional qudits. Based on these capabilities, we estimate a distribution of 3.56 bits of quantum information over a 1000-km repeater link in one minute, highlighting a practical pathway toward scalable quantum interconnects and quantum networks.

</details>


### [33] [On Utility-optimal Entanglement Routing in Quantum Networks](https://arxiv.org/abs/2603.01197)
*Sounak Kar,Arpan Mukhopadhyay*

Main category: quant-ph

TL;DR: This paper develops a Mixed-Integer Convex Program for optimizing entanglement routing in quantum networks, surpassing previous models by identifying optimal paths and maintaining high accuracy across various entanglement measures.


<details>
  <summary>Details</summary>
Motivation: To enhance quantum networks by optimizing the allocation of communication resources and identifying optimal routing paths rather than relying on predetermined routes.

Method: Formulated the routing problem as a Mixed-Integer Convex Program (MICP), incorporating negativity as an entanglement measure, and developed randomized heuristics for improved efficiency.

Result: The proposed formulation achieves over 99.99% accuracy in real-world examples, with heuristics demonstrating faster computational times and improved performance on real networks.

Conclusion: The work extends classical routing concepts to quantum networks, enabling better resource allocation and utility maximization.

Abstract: Quantum networks are envisioned to enable reliable distribution and manipulation of quantum information across distances, forming the foundation of a future quantum internet. The fair and efficient allocation of communication resources in such networks has been addressed through the quantum network utility maximization (QNUM) framework, which optimizes network utility under the assumption of predetermined routes for competing user demands. In this work, we relax this assumption and aim to identify optimal routes that correspond to the maximum achievable network utility. Specifically, we formulate the single-path utility-based entanglement routing problem as a Mixed-Integer Convex Program (MICP). The formulation is exact when negativity is chosen as the entanglement measure for utility quantification or the network supports sufficiently high entanglement generation rates across demands. For other entanglement measures considered, the formulation approximates the problem with over 99.99% accuracy on evaluated real-world examples. To improve computational tractability, we propose a randomized rounding-based heuristic and an upper bound via the relaxation of the MICP. Furthermore, based on min-congestion routing, we introduce an alternative randomized heuristic and upper bound. This heuristic is computationally faster, while both the heuristic and the upper bound often outperform their counterparts on considered real-world networks. Our work provides the framework for extending classical flow-based and quality of service-aware routing concepts to quantum networks.

</details>


### [34] [Bounding the classical cost of simulating quantum behaviors in the prepare-and-measure scenario](https://arxiv.org/abs/2603.01255)
*Sebastian Schlösser,Matthias Kleinmann*

Main category: quant-ph

TL;DR: This paper explores the communication costs in a prepare-and-measure scenario for quantum state transmission, revealing techniques to reduce classical communication required for simulating qubit and qutrit transmissions.


<details>
  <summary>Details</summary>
Motivation: To understand the communication requirements for the simulation of quantum states in the prepare-and-measure paradigm, specifically aiming to reduce classical communication bits for transmitting quantum information.

Method: The authors analyze various sets of quantum states and measurements, demonstrating that shared randomness and classical bits can be optimized to lower communication costs, detailing specific protocols under certain constraints.

Result: They succeeded in reducing the average communication cost to 1.89 bits for qubit transmission, while exploring protocols requiring fewer state preparations and measurements.

Conclusion: The findings suggest innovative methods to understand and optimize communication costs in quantum information transfer, highlighting distinctions between qubit and qutrit communication requirements.

Abstract: We study the prepare-and-measure scenario in which Alice transmits a quantum system to Bob, who then performs a quantum measurement. The quantum state of the system is unknown to Bob, and the measurement is unknown to Alice. It has recently been shown that shared randomness and two bits of classical communication are necessary and sufficient to simulate the transmission of a qubit. We show that the communication cost can be reduced to an average of $1.89$ bits. We then study restricted sets of state preparations: First, for a restriction to real-valued qubit states, if the communication of a classical trit is sufficient, we show that the corresponding protocol must have a convoluted form. We then reduce the smallest qubit scenario requiring two bits of classical communication to only $6$ state preparations and $5$ measurements. For a qutrit, it is not known whether the communication cost is finite; we identify a scenario that requires at least $5$ classical messages, already for the simulation of the real qutrit. Finally, we develop a method for restricted sets of states, that allows us to lower bound the classical communication cost based solely on the set of quantum states.

</details>


### [35] [Intermodal entanglement in a quantum optical model of HHG due to the back-action on the driving field](https://arxiv.org/abs/2603.01315)
*Ákos Gombkötő,Péter Ádám,David Theidel,Tamás Kiss*

Main category: quant-ph

TL;DR: This paper investigates entanglement in high-harmonic generation (HHG), showing that inter-harmonic correlations are linked to quantum properties, suggesting universality in HHG.


<details>
  <summary>Details</summary>
Motivation: The need for nonclassical light with unique quantum properties for advancements in quantum technologies drives this research.

Method: A theoretical quantum optical model is developed, simplifying material interactions into susceptibilities for a general HHG description.

Result: The model reveals that entanglement arises from back-action effects and can replicate experimentally observed nonclassical behaviors.

Conclusion: Intermodal entanglement in HHG may be a universal phenomenon, not limited to specific materials, highlighting broader implications for quantum optics.

Abstract: Preparation of nonclassical light with special quantum properties is essential for quantum technologies. High-harmonic generation (HHG) is a process which not only enables the creation of attosecond pulses but also has the potential to generate light with intricate quantum properties. In a recent experiment [1], nonclassical inter-harmonic correlations have been measured from a HHG source. In this work, we theoretically investigate entanglement between different harmonics within an effective quantum optical model. This model implements a signifcant degree of simplifcation regarding the processes within the target material, treating the material through susceptibilities, as it is usual in quantum optics. Such an approach yields a general description of HHG, permitting the implications that can be derived within it to hold broadly. We find that entanglement is produced as a result of the often neglected back-action. We can qualitatively reproduce experimentally measured nonclassicalities, which suggests that intermodal entanglement can, to an extent, be considered a universal phenomenon associated with HHG, rather than a result of using specific material targets.

</details>


### [36] [Remote state preparation of single-partite high-dimensional states in complex Hilbert spaces](https://arxiv.org/abs/2603.01323)
*Jun-Hai Zhao,Si-Qi Du,Wen-Qiang Liu,Dong-Hong Zhao,Hai-Rui Wei*

Main category: quant-ph

TL;DR: This paper presents methods for remotely preparing high-dimensional quantum states, showing feasibility with current technology.


<details>
  <summary>Details</summary>
Motivation: Explores the potential of high-dimensional quantum systems for enhancing quantum information applications.

Method: Proposes schemes for remotely preparing four- and eight-level equatorial states using orthogonal measurement bases, considering both maximally and non-maximally entangled states.

Result: Demonstrates that three-, five-, six-, and seven-level states can be achieved by adjusting parameters, with feasibility confirmed by evaluations.

Conclusion: Highlights the practicality of high-dimensional RSP schemes without collection operations by using spatial encoding in single-photon systems.

Abstract: High-dimensional quantum systems offer a new playground for quantum information applications due to their remarkable advantages such as higher capacity and noise resistance. We propose potentially practical schemes for remotely preparing four- and eight-level equatorial states in complex Hilbert spaces exactly by identifying a set of orthogonal measurement bases. In these minimal-resource-consuming schemes, both pre-shared maximally and non-maximally entangled states are taken into account. The three-, five-, six-, and seven-level equatorial states in complex Hilbert spaces can also be obtained by adjusting the parameters of the desired states. The evaluations indicate that our high-dimensional RSP schemes might be possible with current technology. The collection operations, necessary for our high-dimensional RSP schemes via partially entangled channels, can be avoided by encoding the computational basis in the spatial modes of single-photon systems.

</details>


### [37] [Quantum framework for parameterizing partial differential equations via diagonal block-encoding](https://arxiv.org/abs/2603.01358)
*Hiroshi Yano,Yuki Sato*

Main category: quant-ph

TL;DR: A quantum framework is proposed for simulating parameterized PDEs using diagonal representations for efficient computation, especially in optimization scenarios.


<details>
  <summary>Details</summary>
Motivation: To enhance the simulation of PDEs with complex, spatially varying coefficients and support optimization in quantum computing.

Method: Utilizes block-encodings of diagonal matrices to represent parameter fields, enabling efficient quantum simulations in both forward and parameter-dependent contexts.

Result: Demonstrated effective forward simulation and parameter design for the 2D wave equation with a Gaussian parameter profile.

Conclusion: This quantum algorithmic approach offers a promising avenue for tackling PDE-constrained optimization problems with intricate parameters.

Abstract: We study a quantum-algorithmic framework for parameterizing partial differential equations (PDEs). For a broad class of problems in which the discretized parameter field admits a diagonal representation, block-encodings of diagonal matrices, or diagonal block-encodings, can be used to represent spatially varying coefficients with structured, potentially complicated profiles. This encoding enables efficient quantum simulation of forward PDEs and extends naturally to parameter-dependent settings. Such simulations are a key primitive for quantum algorithms for PDE-constrained optimization, where the goal is to identify optimal design parameters. We illustrate the framework numerically through forward simulation and parameter design for the two-dimensional wave equation with a Gaussian parameter profile.

</details>


### [38] [Nonreciprocal entanglement in exciton optomechanics with an optical parametric amplifier](https://arxiv.org/abs/2603.01397)
*Zhen-Sen Lin,Rui Zhang,Zi-Wei Jiang,Wen-Quan Yang,Ya-Feng Jiao,Hui Jing,Le-Man Kuang*

Main category: quant-ph

TL;DR: This study investigates nonreciprocal entanglement in a spinning exciton-optomechanical system, demonstrating that entanglement can be achieved and regulated under feasible conditions.


<details>
  <summary>Details</summary>
Motivation: To explore nonreciprocal entanglement in quantum systems and its applications in quantum technologies.

Method: The study utilizes a spinning exciton-optomechanical system with an optical parametric amplifier to examine nonreciprocal entanglement among photons, excitons, and phonons.

Result: Nonreciprocal entanglement is achieved with enhanced photon-exciton entanglement and tripartite entanglement, while photon-phonon and exciton-phonon entanglement are weakened.

Conclusion: This research enables manipulation of room-temperature nonreciprocal entanglement, paving the way for advancements in nonreciprocal quantum technologies.

Abstract: We study nonreciprocal bipartite and tripartite entanglement in a spinning exciton-optomechanical system (EOMS) with an optical parametric amplifier (OPA). We demonstrate that nonreciprocal entanglement among photons, excitons, and phonons can be achieved under experimentally feasible parameters. We find that the nonreciprocal entanglement induced by Sagnac effects can be regulated through the OPA. Particularly, We show that the OPA significantly enhances photon-exciton entanglement and tripartite entanglement but weakens photon-phonon and exciton-phonon entanglement. Moreover, we find that the photon-exciton nonreciprocal entanglement not only can be generated at room temperature and even higher temperature but also exhibits highly robustness to cavity dissipation. Our works open a way to manipulate the room-temperature nonreciprocal entanglement, which may be useful for developing nonreciprocal quantum technologies.

</details>


### [39] [Applicability and Limitations of Quantum Circuit Cutting in Classical State-Vector Simulation](https://arxiv.org/abs/2603.01443)
*Mitsuhiro Matsumoto,Shinichiro Sanji,Takahiko Satoh*

Main category: quant-ph

TL;DR: The paper explores circuit cutting, a method for partitioning large quantum circuits into smaller subcircuits for efficient simulation, demonstrating conditions under which this method improves runtime and offering practical guidelines for maximizing qubit counts in simulations.


<details>
  <summary>Details</summary>
Motivation: Circuit cutting is aimed at optimizing the simulation of large quantum circuits by enabling independent execution of smaller subcircuits, potentially reducing computation time.

Method: The authors derive threshold conditions for effective cutting of circuits and validate these with state-vector experiments, measuring speedup and assessing runtime breakdowns.

Result: Experiments confirm the predicted speedup for circuit cutting, revealing critical points where merging techniques become more efficient, thereby extending feasible qubit counts.

Conclusion: Under a constrained time budget, two-way circuit cutting enhances simulation capacity, allowing for 4 to 6 additional qubits compared to non-cutting methods.

Abstract: Circuit cutting partitions a large quantum circuit into smaller subcircuits that can be executed independently and recombined by classical post-processing. In classical state-vector simulation with full-state reconstruction, the runtime is governed by a trade-off between reduced subcircuit size and the overheads of exponentially many subcircuits and full-state reconstruction. For equal partitioning, we derive threshold conditions on the number of cuts below which cutting reduces the wall-clock time. State-vector experiments validate the predicted speedup boundary up to 24 qubits, and a runtime breakdown up to 30 qubits identifies crossovers at $q \approx 18$ and $q \approx 22$ where merging overtakes first preprocessing and then subcircuit simulation. As a practical guideline, we show that under a 10-minute wall-clock budget, two-way cutting extends the maximum feasible qubit count by 4 to 6 qubits relative to simulation without cutting.

</details>


### [40] [Exact bounds on quantum partial search algorithm and improving the parallel search](https://arxiv.org/abs/2603.01462)
*Yan-Bo Jiang,Xiao-Hui Wang,Kun Zhang,Vladimir Korepin*

Main category: quant-ph

TL;DR: The paper demonstrates the optimality of the Grover-Radhakrishnan-Korepin (GRK) algorithm in partial search problems, provides bounds on success probability and query complexity, and explores hybrid strategies for enhanced parallel efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the optimality of the GRK algorithm in partial search problems and its implications for parallel quantum search efficiency.

Method: The authors exhaustively examine all operator sequences with a fixed number of oracle queries to show that the GRK structure maximizes success probability, deriving upper and lower bounds on query complexity and success probability.

Result: The GRK sequence is shown to universally maximize success probability, and a hybrid strategy combining partial and full search protocols demonstrates improved parallel efficiency compared to direct GRK-based parallelization.

Conclusion: The findings elucidate the fundamental constraints of quantum partial search, highlighting its importance in optimizing parallel quantum algorithms.

Abstract: Grover's algorithm provides a quadratic speedup over classical algorithms for searching unstructured databases and is known to be strictly optimal in oracle query complexity, with tight bounds on its success probability. Although the standard Grover search cannot be further accelerated in the full-search setting, a trade-off between accuracy and query complexity gives rise to the partial search problem. The Grover-Radhakrishnan-Korepin (GRK) algorithm is widely regarded as the optimal protocol for this task. In this work, we provide strong evidence for the strict optimality of the GRK operator sequence among all admissible compositions of global and local Grover operators. By exhaustively examining all operator sequences with a fixed number of oracle queries, we show that the GRK structure universally maximizes the success probability. Building on this result, we derive an asymptotically tight upper bound on the maximal success probability for partial search and establish a matching lower bound on the minimal expected number of oracle queries. Furthermore, we investigate parallel quantum search within the partial-search framework. While a direct GRK-based parallelization does not outperform established parallel Grover schemes, we demonstrate that a hybrid strategy combining partial and full search protocols achieves a strictly improved parallel efficiency. Our results clarify the fundamental limits of quantum partial search and its role in optimizing parallel quantum search algorithms.

</details>


### [41] [Efficient Learning Algorithms for Noisy Quantum State and Process Tomography](https://arxiv.org/abs/2603.01521)
*Chenyang Li,Shengxin Zhuang,Yukun Zhang,Jingbo B. Wang,Xiao Yuan,Yusen Wu,Chuan Wang*

Main category: quant-ph

TL;DR: A new learning framework is introduced that efficiently characterizes noisy quantum states and processes with polynomial resource requirements, applicable to large-scale quantum systems.


<details>
  <summary>Details</summary>
Motivation: Conventional tomography methods for quantum systems are resource-intensive, growing exponentially with system size.

Method: A sample-efficient learning algorithm for unital noisy quantum states is developed, extended to a unified protocol for quantum process tomography that is input-agnostic.

Result: The proposed method requires only polynomial samples and classical processing, achieving high success probability even for complex circuits.

Conclusion: This framework provides a scalable approach for characterizing large noisy quantum devices, addressing significant challenges in quantum technology advancement.

Abstract: Efficiently characterizing large quantum states and processes is a central yet notoriously challenging task in quantum information science, as conventional tomography methods typically require resources that grow exponentially with system size. Here, we introduce a provably efficient and structure-agnostic learning framework for noisy $n$-qubit quantum circuits under generic noise with arbitrary noise strength. We first develop a sample-efficient learning algorithm for unital noisy quantum states. Building on this result, we extend the framework to quantum process tomography, obtaining a unified protocol applicable to both unital and non-unital channels. The resulting approach is input-agnostic and does not rely on assumptions about specific input distributions. Our theoretical analysis shows that both state and process learning require only polynomially many samples and polynomial classical post-processing in the number of qubits, while achieving near-unit success probability over ensembles generated by local random circuits. Numerical simulations of two-dimensional Hamiltonian dynamics further demonstrate the accuracy and robustness of the approach, including for structured circuits beyond the random-circuit setting assumed in the theoretical analysis. These results provide a scalable and practically relevant route toward characterizing large-scale noisy quantum devices, addressing a key bottleneck in the development of quantum technologies.

</details>


### [42] [Theory of anomalous Landau-Zener tunneling induced by nonlinear coupling](https://arxiv.org/abs/2603.01523)
*Wen-Yuan Wang,Hong-Juan Meng*

Main category: quant-ph

TL;DR: The paper presents a theory of Landau-Zener tunneling that incorporates nonlinear coupling, revealing a breakdown of traditional behaviors and introducing new dynamics, including a universal attractor.


<details>
  <summary>Details</summary>
Motivation: To explore the effects of amplitude-dependent, sign-reversible nonlinear coupling on Landau-Zener tunneling in two-level systems, contrasting it with standard nonlinearity.

Method: Combining analytical methods and phase-space analysis to study the effects of nonlinear coupling on the adiabatic energy landscape and tunneling behavior.

Result: Discovery of a black-hole-like fixed point that acts as a universal attractor, leading to a breakdown of the standard exponential LZ formula and a power-law dependence on coupling strength.

Conclusion: The findings establish a framework for understanding anomalous adiabaticity and provide insights for controlling quantum states in nonlinear systems.

Abstract: We develop a general theory of Landau-Zener (LZ) tunneling in a two-level system with amplitude-dependent, sign-reversible nonlinear coupling, distinguishing it fundamentally from conventional on-site nonlinearity. Through a combination of analytical and phase-space analysis, we show that beyond a critical interaction strength, the nonlinear coupling fundamentally reshapes the adiabatic energy landscape, introducing a topological twisted and knotted structure. This structure leads to a complete breakdown of the standard exponential LZ formula, even in the adiabatic limit. Central to this anomalous behavior is the emergence of a black-hole-like fixed point, which acts as a universal attractor: upon traversing the critical region, all quantum trajectories converge to this fixed point, irreversibly erasing any memory of the initial state. From this fixed-point picture, we derive an exact analytical expression for the adiabatic tunneling probability, revealing a characteristic power-law dependence on both linear and nonlinear coupling strength. Our work establishes a paradigmatic framework for nonlinear-coupling-induced anomalous adiabaticity breaking and offers a universal mechanism for state control in driven quantum and wave systems.

</details>


### [43] [Quantum Thermal Machines Improved by Internal Coupling: From Equilibrium to Non-equilibrium Limit Cycles](https://arxiv.org/abs/2603.01567)
*Jingyi Gao,Naomichi Hatano*

Main category: quant-ph

TL;DR: This paper explores the impact of internal coupling on the performance of various quantum Otto cycles and demonstrates the advantages of coupling in enhancing efficiency and operational regimes.


<details>
  <summary>Details</summary>
Motivation: To understand how internal coupling affects the operational capabilities and performance metrics of quantum Otto cycles in different regimes.

Method: Analysis of the quantum Otto cycle as Gibbs-state limit cycle, equilibrating limit cycle, and non-equilibrating limit cycle, employing the GKSL master equation for validation.

Result: Internal coupling broadens the operational regime and improves efficiency and performance in GSLC; coupled systems can operate as engines or refrigerators where uncoupled ones cannot.

Conclusion: While NELC shows lower efficiency and COP compared to ELC, it can achieve higher power output with shorter interaction times, highlighting a power-efficiency trade-off.

Abstract: We investigate how internal coupling influences the operation and performance of a quantum Otto cycle operating as the Gibbs-state limit cycle (GSLC), equilibrating limit cycle (ELC), and non-equilibrating limit cycle (NELC). We show that the internal coupling significantly broadens the operational regime of the cycle. In particular, in parameter regimes where the uncoupled Otto cycle fails to operate as any thermal machine, the coupled system can function as an engine or a refrigerator.
  For the GSLC, in which we assume that the system quickly equilibrates during the isochoric processes, the internal coupling not only shifts and enlarges the operational regime but also enhances the efficiency and the coefficient of performance (COP), allowing the performance to exceed the standard Otto bounds while remaining below the Carnot limit.
  For ELC and NELC, we validate the global approach of the Gorini--Kossakowski--Sudarshan--Lindblad (GKSL) master equation by comparison with the GSLC, and examine the NELC for finite interaction time and the ELC for infinite interaction time. Although the efficiency and COP of NELC are lower than those of ELC, shorter interaction times yield higher power output, consistent with the power--efficiency trade-off.

</details>


### [44] [Intersubjectivity as a principle determining physical observables and non-classicality](https://arxiv.org/abs/2603.01575)
*Shun Umekawa,Koki Ono,Hayato Arai*

Main category: quant-ph

TL;DR: This paper identifies conditions that distinguish Projection-Valued Measures (PVMs) from Positive Operator-Valued Measures (POVMs), focusing on intersubjectivity in measurement outcomes, and establishes criteria for classical systems.


<details>
  <summary>Details</summary>
Motivation: The paper aims to bridge contemporary quantum measurement theory with traditional projective measurement frameworks, emphasizing the importance of inter-observer agreement.

Method: The authors reformulate Ozawa's intersubjectivity condition quantitatively within generalized probabilistic theories, proving links between PVMs, POVMs, and classical systems through coarse-graining analysis.

Result: It is shown that a POVM is a PVM if its coarse-graining maintains intersubjectivity, and a system is classical if this property holds under all coarse-grainings.

Conclusion: Intersubjectivity is crucial for state tomography and discrimination tasks, highlighting its significance in both quantum information and classical theories.

Abstract: We identify an operational principle that singles out Projection-Valued Measures (PVMs) among general Positive Operator-Valued Measures (POVMs), bridging the modern quantum measurement theory and the traditional formulation based on projective measurements of physical observables. We reformulate Ozawa's intersubjectivity condition, which requires inter-observer agreement of the measurement outcomes, in a quantitative manner within the framework of generalized probabilistic theories. We prove that (i) a POVM is a PVM if and only if its every coarse-graining is intersubjective, and (ii) a system is classical if and only if intersubjectivity is preserved under any coarse-graining, establishing a complete characterization of the physical observables and the classical theory. Furthermore, measurements with intersubjectivity are sufficiently rich for the informational tasks of state tomography and state discrimination, testifying to its operational significance in quantum and beyond information processing.

</details>


### [45] [QuMeld: A Modular Framework for Benchmarking Qubit Mapping Algorithms](https://arxiv.org/abs/2603.01578)
*Gabrielius Keibas,Linas Petkevičius*

Main category: quant-ph

TL;DR: QuMeld is an open-source framework aimed at addressing the qubit mapping problem in quantum computing, supporting various mapping algorithms and topologies.


<details>
  <summary>Details</summary>
Motivation: To provide a unified framework for evaluating and comparing qubit mapping algorithms across diverse quantum computer configurations.

Method: The framework offers six qubit mapping algorithms and supports sixteen different quantum topologies, coupled with multiple evaluation metrics.

Result: Facilitates systematic assessment and adaptation of qubit mapping methods for various quantum circuits and hardware.

Conclusion: QuMeld enhances the extensibility and adaptability of quantum computing solutions in terms of qubit mapping.

Abstract: The qubit mapping problem is a challenge in quantum computing that is related to mapping logical qubits to the physical ones on the quantum computer. Due to the diversity of quantum computer topologies and circuits, numerous approaches solving this problem exist. Finding the best solution for specific combination of topology and circuit remains difficult and no unified framework currently exists for systematically evaluating and comparing qubit mapping algorithms across different cases. We present QuMeld, an open-source framework that is designed for solving this issue. The framework currently supports six qubit mapping algorithms, sixteen quantum computer topologies and multiple evaluation metrics. The modular design of the framework allows integration of new mapping algorithms, quantum circuits, hardware topologies, and evaluation metrics, ensuring extensibility and adaptability to future developments.

</details>


### [46] [Single-ion phonon laser in the quantum regime](https://arxiv.org/abs/2603.01585)
*Dong Yuanzhang,He Siwen,Deng Zhijiao,Li Peidong,Chen Liang,Feng Mang*

Main category: quant-ph

TL;DR: The paper investigates the generation of quantum phonon laser states with a single trapped ion using a three-level model, addressing the challenges of practical applications in quantum information technologies.


<details>
  <summary>Details</summary>
Motivation: To explore new methods for generating quantum phonon lasers, surpassing classical limitations and enhancing quantum information capabilities.

Method: The authors employ a three-level model to analyze steady-state characteristics by solving the quantum master equation. An experimental setup with a 40Ca+ ion and bichromatic lasers is proposed for generating quantum phonon laser states, with a focus on quantum statistics and state tomography.

Result: The study reveals significant differences in phonon laser thresholds between the two-level and three-level models, leading to more accurate descriptions of the quantum phonon laser's behavior.

Conclusion: The three-level model effectively captures the physical mechanisms of quantum phonon lasers, highlighting its potential in quantum technologies despite challenges in practical implementation.

Abstract: The quantum phonon laser state is a vibrational state generated by phonon coherent amplification based on quantum mechanics. Its core is coherent excitation and manipulation of phonon quantum states by controlling phonon dynamics. This technology breaks classical limits of traditional phonon lasers, offering new methods for quantum information. Previous research on quantum phonon lasers focused on quantum van der Pol oscillators. As typical nonlinear quantum systems, they show significant value in trapped-ion systems. These breakthroughs extend nonlinear dynamics into the quantum domain and provide platforms for exploring quantum nonlinear phenomena. Although realized in two-ion systems, practical applications remain challenging. This paper explores how a single trapped ion generates quantum phonon laser states using a three-level model. By solving the quantum master equation numerically, steady-state characteristics are analyzed, focusing on quantum statistics including the Wigner function and second-order correlation function. An experimental scheme is proposed based on a single trapped 40Ca+ ion, using bichromatic blue-sideband and red-sideband lasers to generate quantum phonon laser states. By introducing the characteristic function of motional states, precise quantum state tomography is achieved. Additionally, a two-level model discusses the phonon laser threshold effect. However, the three-level model shows significantly different thresholds and more accurately describes the quantum phonon laser's physical mechanisms.

</details>


### [47] [Sustaining high-fidelity quantum logic in neutral-atom circuits via mid-circuit operations](https://arxiv.org/abs/2603.01612)
*Rui Lin,You Li,Le-Tian Zheng,Tai-Ran Hu,Si-Yuan Chen,Hong-Ming Wu,Yu-Chen Zhang,Hao-Wen Cheng,Yu-Hao Deng,Zhan Wu,Ming-Cheng Chen,Jun Rui,Chao-Yang Lu,Jian-Wei Pan*

Main category: quant-ph

TL;DR: The paper presents a neutral-atom framework that maintains high gate fidelity in quantum circuits through innovative mid-circuit operations, achieving sustained performance for quantum error correction.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of ensuring high gate fidelity in deep quantum circuits for fault-tolerant quantum computation, particularly in the context of neutral-atom technologies.

Method: The authors implemented hardware-efficient mid-circuit operations, including non-destructive erasure detection, in-circuit Raman sideband cooling, and qubit re-initialization to enhance and maintain gate fidelity throughout multiple operational rounds.

Result: Achieved a two-qubit controlled logic gate fidelity of 99.81(1)% and maintained this fidelity at ~99.8% across several operational rounds, thereby demonstrating a robust fault-tolerant mechanism for quantum circuits.

Conclusion: The in-situ refreshable architecture enables effective management of internal and motional entropy, crucial for large-scale, continuous quantum error correction processes.

Abstract: The realization of fault-tolerant quantum computation hinges on the ability to execute deep quantum circuits while maintaining gate fidelities consistently above error-correction thresholds. Although neutral-atom arrays have recently demonstrated high-fidelity two-qubit gates and early-stage logical quantum processors, sustaining such high performance across deep, repetitive circuits remains a formidable challenge due to cumulative motional heating and atom loss. Here we demonstrate a sustainable neutral-atom framework that overcomes these limitations by integrating a suite of hardware-efficient mid-circuit operations. We report a two-qubit controlled logic gate with a raw fidelity of 99.60(1)%, which is further increased to a fidelity of 99.81(1)% via non-destructive erasure detection. Crucially, by implementing in-circuit Raman sideband cooling and qubit re-initialization, we demonstrate that gate fidelities can be maintained at the ~99.8% level across multiple operational rounds without observable degradation. By actively managing the internal and motional entropy of the system mid-stream, our in-situ refreshable architecture provides a critical pathway for executing the repeated syndrome-extraction cycles required for large-scale, continuous quantum error correction.

</details>


### [48] [No More Hooks in the Surface Code: Distance-Preserving Syndrome Extraction for Arbitrary Layouts at Minimum Depth](https://arxiv.org/abs/2603.01628)
*Yuga Hirai,Shota Ikari,Yosuke Ueno,Yasunari Suzuki*

Main category: quant-ph

TL;DR: The paper introduces ZX interleaving syndrome extraction to enhance fault tolerance in surface code implementations by maintaining full fault distance at minimal circuit depth.


<details>
  <summary>Details</summary>
Motivation: Hook errors compromise logical operations by reducing fault distance. Improved syndrome extraction techniques are needed to mitigate these issues without increasing complexity.

Method: ZX interleaving syndrome extraction intersperses Z and X stabilizer tiles, effectively shortening hook-error edges in the decoding graph while keeping circuit depth to four layers of CNOT gates.

Result: Numerical simulations show that the proposed method achieves full fault distance $d$, outperforming existing methods that achieve only $d-1$ under similar conditions.

Conclusion: The method is applicable to all regular tiling layouts of the surface code, making it vital for effective fault-tolerant quantum computing.

Abstract: Hook errors are a major challenge in implementing logical operations with the surface code, because they can reduce the fault distance below the code distance. This motivates syndrome-extraction circuits that suppress hook-error effects for the stabilizer layouts that appear during logical operations. However, the existing methods either increase circuit depth or require simultaneous execution of measurements and CNOT gates, both of which introduce additional overheads and degrade the threshold. We propose the ZX interleaving syndrome extraction, which preserves the full fault distance $d$ for any surface-code layout with regular stabilizer tiles at minimum depth, i.e., four layers of CNOT gates, without requiring additional circuit depth or simultaneous execution of measurements and CNOT gates. The key idea is to interleave the Z and X stabilizer tiles so that hook-error edges in the decoding graph are shortened and effectively eliminated. Numerical simulations under uniform depolarizing noise for memory and lattice-surgery experiments confirm that the proposed method achieves a full fault distance of $d$, whereas the best existing minimum-depth approach achieves $d-1$. Since the full fault distance is achievable for any regular tiling layout of the surface code, the proposed method may serve as an indispensable technique for practical fault-tolerant quantum computation.

</details>


### [49] [Shaping frequency-tunable single photons for quantum networking in waveguide QED](https://arxiv.org/abs/2603.01649)
*Álvaro Pernas,Álvaro Gómez-León,Ricardo Puebla*

Main category: quant-ph

TL;DR: Developed controls for frequency-tunable single photons improve quantum information exchange in superconducting waveguide QED networks.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of quantum information exchange in networks with frequency-detuned nodes to enhance scalability.

Method: Derived control techniques for shaping single photons to match different frequency nodes, providing a theoretical framework and analyzing photon properties.

Result: Enabled frequency-selective quantum state transfer and remote entanglement generation in a realistic quantum network, validated by numerical simulations.

Conclusion: The proposed methods enhance the practical implementation of distributed quantum computation through improved photon shaping and control.

Abstract: The exchange of quantum information among nodes in a quantum network is one of the main challenges in modern technologies. Superconducting waveguide QED networks hold great potential for realizing distributed quantum computation, where distinct nodes communicate via itinerant single photons. Yet, different frequencies among the nodes restrict their applicability and limit scalability. Here we derive the controls required to shape single photons arbitrarily detuned with respect to their natural frequency, allowing thus for an on-demand and deterministic exchange of quantum information among frequency detuned nodes. We provide a theoretical framework, analyzing the properties of the controls for typical photon shapes, identifying operation regimes amenable for experimental realization. We then show how these controls enable frequency-selective quantum state transfer among non-resonant and distant nodes of a realistic network. In addition, we also provide a simple extension for remote entanglement generation between these nodes. The suitability and high-fidelity of these protocols is supported by numerical simulations, highlighting the novel networking possibilities unlocked when shaping frequency-tunable single photons.

</details>


### [50] [Gain-induced spectral non-degeneracy in type-II parametric down-conversion](https://arxiv.org/abs/2603.01656)
*Behnood Taheri,Denis Kopylov,Manfred Hammer,Torsten Meier,Jens Förstner,Polina Sharapova*

Main category: quant-ph

TL;DR: The study reveals how gain-induced spectral shifts in type-II parametric down-conversion lead to a transition from degenerate to non-degenerate PDC, enhancing photon pair distinguishability in high-gain scenarios.


<details>
  <summary>Details</summary>
Motivation: Understanding the behavior of parametric down-conversion under high-gain conditions is crucial for advancing quantum optics applications.

Method: A rigorous theoretical model is developed through a system of coupled integro-differential equations to analyze the PDC process.

Result: The analysis shows that increasing parametric gain alters PDC properties, specifically enhancing distinguishability of photon pairs, contrary to predictions from simpler models.

Conclusion: The study highlights the inadequacy of spatially-averaged models in capturing the complexities of gain-induced effects in PDC processes.

Abstract: We demonstrate the novel effect of gain-induced spectral shifts in the type-II parametric down-conversion (PDC) process, which results in a transition from degenerate to non-degenerate PDC with increasing parametric gain. This effect, originating from the second-order dispersion terms, significantly alters the properties of PDC in the high-gain regime, where it leads to increased distinguishability of the generated photon pairs. The effect is established by evaluating a rigorous theoretical model, which is based on solving a system of coupled integro-differential equations for monochromatic operators. The widely used spatially-averaged approximate model fails to reproduce this important effect.

</details>


### [51] [High-Performance Quantum Frequency Conversion from Ultraviolet to Telecom Band](https://arxiv.org/abs/2603.01745)
*Yi Yang,Bin Wang,Ji-Chao Lin,Yang Gao,Xin Li,Jiu-Peng Chen,Lei Hou,Ye Wang,Yong Wan,Xiu-Ping Xie,Ming-Yang Zheng,Qiang Zhang,Jian-Wei Pan*

Main category: quant-ph

TL;DR: This paper presents a novel quantum frequency conversion technique using thin-film lithium niobate, achieving high efficiency and low noise for connecting ultraviolet and telecom communication bands.


<details>
  <summary>Details</summary>
Motivation: To bridge the spectral gap for efficient communication between stationary qubits and optical channels, essential for scalable quantum networks.

Method: Utilization of a short-wavelength-pumping QFC with a quasi-phase matching period of 3.07 um, alongside a theoretical model correlating conversion efficiency with domain defects, and a noise suppression strategy using counter-tuning behaviors.

Result: Attained a theoretical limit conversion efficiency of 839%/(W*cm^2) and a record external efficiency of 28.8% with ultra-low noise of 35 counts per second.

Conclusion: The demonstrated QFC technique meets the stringent needs for remote ion-ion entanglement in quantum networks.

Abstract: Quantum frequency conversion (QFC) is essential for bridging the spectral gap between stationary qubits and low-loss optical communication channels. In this work, we demonstrate a short-wavelength-pumping QFC with the first-order quasi-phase matching period of 3.07 um on thin-film lithium niobate, converting ultraviolet photons to the telecom C-band. By constructing a theoretical model that correlates the normalized conversion efficiency with domain defects in the short-period phase-matched waveguide, we found the critical tolerance of domain defects along the waveguide should be $\le 2$ (excluding the ends). Based on this, we achieved a theoretical limit normalized conversion efficiency of 839%/(W*cm^2) for the fundamental guided mode through fabrication optimization. Furthermore, we propose a robust noise suppression strategy for short-wavelength pumping by utilizing the counter-tuning behaviors of difference-frequency generation and spontaneous parametric down-conversion. By combining these advances with ultra-narrowband filtering, we achieve a record-high external efficiency of 28.8% and an ultra-low noise of 35 counts per second. This high-performance QFC connecting ultraviolet and telecom bands satisfies the stringent requirements for long-lived remote ion-ion entanglement in scalable quantum networks [W.-Z. Liu et al., Nature (2026)].

</details>


### [52] [Experimental realization and self-testing of semisymmetric informationally complete measurements via a one-dimensional photonic quantum walk](https://arxiv.org/abs/2603.01802)
*Xu Xu,Han-Yu Cheng,Meng-Yun Ma,Chao-Jie Sun,Yan Wang,Li-Jiong Shen,Zhe Sun,Qi-Ping Su,Chui-Ping Yang,Yong-Nan Sun*

Main category: quant-ph

TL;DR: This paper explores the experimental realization and self-testing of semi-symmetric informationally complete POVMs (semi-SIC POVMs) using quantum walks and linear optics, addressing the challenge of demonstrating SIC-POVMs in finite dimensions.


<details>
  <summary>Details</summary>
Motivation: The existence of SIC-POVMs in every finite dimension is unknown, driving research into alternative measurement frameworks.

Method: The paper experimentally realizes four-outcome semi-SIC POVMs through a one-dimensional discrete-time quantum walk, utilizing single photons and linear optics for self-testing in a semi-device-independent manner.

Result: The experiment successfully implements and verifies semi-SIC POVMs, contributing to the understanding and application of generalized quantum measurements.

Conclusion: The findings enhance the potential for quantum certification and further explorations into the utility of generalized POVMs.

Abstract: Generalized quantum measurements play a crucial role in quantum mechanics, and symmetric informationally complete positive operator-valued measurements (SIC POVMs) provide a powerful and flexible framework for extracting information from quantum systems. However, the existence of SIC-POVMs in every finite dimension remains an open question, which has stimulated extensive research into alternative classes of POVMs. Recently, Geng $et$ $al$. [Phys. Rev. Lett. 126, 100401 (2021)] proposed a broader class of SIC POVM, called semisymmetric informationally complete POVM (semi-SIC POVM), which extends beyond SIC POVM. In this work, we focus on the four-outcome POVMs and experimentally realize the semi-SIC POVMs using a one-dimensional discrete-time quantum walk. Additionally, employing single photons and linear optics, we perform an experimental self-testing of semi-SIC POVMs in the semi-device-independent manner. Our results pave the way for exploring quantum certification with generalized quantum measurements.

</details>


### [53] [Finite-Depth, Finite-Shot Guarantees for Constrained Quantum Optimization via Fejér Filtering](https://arxiv.org/abs/2603.01809)
*Chinonso Onah,Kristel Michielsen*

Main category: quant-ph

TL;DR: This paper investigates the CE-QAOA under finite-layer constraints, establishing success probability bounds and proposing future hardware implementations.


<details>
  <summary>Details</summary>
Motivation: To enhance the optimization capabilities of the CE-QAOA by ensuring feasibility and optimality in problem-solving.

Method: By applying harmonic constraints to the cost angles and leveraging a wrapped phase-separation condition, the authors analyze the success probabilities mathematically.

Result: They derived a dimension-free lower bound for the success probability of sampling optimal solutions and established a specific ratio-form guarantee for single-shot success probabilities.

Conclusion: The paper highlights the need for practical implementations of spectral filters to fully realize the potential of their theoretical findings.

Abstract: We study finite-layer alternations of the \emph{Constraint--Enhanced Quantum Approximate Optimization Algorithm} (CE--QAOA), a constraint-aware ansatz that operates natively on block one-hot manifolds. Our focus is on feasibility and optimality guarantees. We show that restricting cost angles to a harmonic lattice exposes a positive Fejér filter acting on the cost-phase unitary $U_C(γ)=e^{-iγH_C}$ \emph{in a cost-dephased reference model (used only for analysis)}. Under a wrapped phase-separation condition, this yields \emph{dimension-free} finite-depth and finite-shot lower bounds on the success probability of sampling an optimal solution. In particular, we obtain a ratio-form guarantee \[ q_0 \;\ge\; \frac{x}{1+x}, \qquad x \;=\; (p{+}1)^2 \sin^2(δ/2)\,C_β, \] where $q_0$ is the single-shot success probability, $C_β$ is the mixer-envelope mass on the optimal set, $δ$ is a phase-gap proxy, and $p$ is the number of layers. Riemann--Lebesgue averaging extends the discussion beyond exact lattice normalization. We conclude by outlining coherent realizations of hardware-efficient positive spectral filters as a main open direction.

</details>


### [54] [Nature abhors macroscopic superpositions](https://arxiv.org/abs/2603.01811)
*Filippus S. Roux*

Main category: quant-ph

TL;DR: Macroscopic mass distributions resist forming superpositions due to an energy dip, modeled as a Schrödinger cat state, which acts as an opposing force.


<details>
  <summary>Details</summary>
Motivation: To explore the relationship between mass distributions and entanglement in spacetime.

Method: Modeling macroscopic superpositions as Schrödinger cat states to analyze energy dynamics based on separation distance.

Result: Identified a dip in total energy that acts as a force against the formation of macroscopic superpositions.

Conclusion: This reluctance can be generalized to address the measurement problem in quantum mechanics.

Abstract: Superpositions of mass distributions can potentially lead to entanglement with the geometry of spacetime. Here we show that there exists a natural reluctance for macroscopic mass distributions to form such superpositions. The macroscopic superposition is modeled as a Schr{ö}dinger cat state. The reluctance manifests as a dip in the total energy of the Schr{ö}dinger cat state as a function of the separation distance between the terms in the superposition. The dip in the energy provides an opposing force preventing the formation of the superposition. A generalization of this phenomenon addressing the measurement problem is also discussed.

</details>


### [55] [Time-dependent adiabatic elimination in matter-wave optics](https://arxiv.org/abs/2603.01826)
*Samuel Böhringer,Alexander Bott,Eric P. Glasbrenner*

Main category: quant-ph

TL;DR: The paper presents a method for adiabatically eliminating certain quantum states using a time-dependent projector approach, accommodating non-commuting Hamiltonian elements and explicit coupling.


<details>
  <summary>Details</summary>
Motivation: To separate the dynamics of specific quantum states from the total quantum state in systems with explicit time-dependent coupling and non-commuting Hamiltonians.

Method: A projector-based formalism of adiabatic elimination is developed that accounts for center-of-mass degrees of freedom and time-dependent interactions between subsystems.

Result: The formalism successfully facilitates the adiabatic elimination of relevant states within the given constraints, applicable to scenarios like matter-wave optics.

Conclusion: This method broadens the applicability of adiabatic elimination in complex quantum systems where traditional methods may fail due to non-commuting Hamiltonians.

Abstract: We show how the dynamics of a specific subset of states can be separated from the dynamic of the total quantum state via a time-dependent projector-based formalism of adiabatic elimination. Within our formalism, we assume explicit time dependency in the coupling between both subsystems. Additionally, we do not assume that the elements of the Hamiltonian commute, as in matter-wave optics this not given in general. Here the center-of-mass degrees of freedom frequently need to be taken into account. Our formalism allows to perform the adiabatic elimination in such a setting.

</details>


### [56] [Local approach to entropy production in the nonequilibrium dynamics of open quantum systems](https://arxiv.org/abs/2603.01861)
*Irene Ada Picatoste,Alessandra Colla,Heinz-Peter Breuer*

Main category: quant-ph

TL;DR: The paper explores entropy production rates in nonequilibrium quantum dynamics, highlighting connections to Markovian and non-Markovian behaviors.


<details>
  <summary>Details</summary>
Motivation: To understand the interplay between entropy production, memory effects, and the underlying dynamics in open quantum systems.

Method: Defines the entropy production rate in terms of relative entropy, analyzes eigenvalues of the time-local generator, and explores P-divisibility of the quantum dynamical map.

Result: Positivity of the entropy production rate implies negative eigenvalues for Markovian dynamics, while demonstrating exceptions in non-Markovian scenarios.

Conclusion: Negative entropy production rates indicate non-Markovianity but are not a necessary condition; likewise, a newly defined entropy production approach aligns positivity with Markovianity.

Abstract: We discuss fundamental features of the local expression for the entropy production rate of the nonequilibrium quantum dynamics of open systems and its relations to memory effects and the spectrum of the generator of the dynamics. Defining the entropy production rate as negative rate of change of the relative entropy with respect to an instantaneous fixed point, it is shown that positivity of the entropy production rate for all possible initial states implies that the real parts of the eigenvalues of the time-local generator for the quantum master equation are always negative. It is further demonstrated that Markovian dynamics, identified as P-divisibility of the quantum dynamical map, implies positivity of entropy production rate, thus providing a kind of generalized second law in the nonequilibrium regime. We also prove by means of the counterexample of a phase covariant quantum master equation that the converse of this statement is not true, i.e., there are non-Markovian dynamics for which the entropy production rate is always positive. Thus, we conclude that the emergence of negative entropy production rates is a sufficient but not necessary condition for non-Markovianity of the quantum dynamics. Finally, we also consider a recently introduced map-based notion of entropy production and show the equivalence between its positivity and Markovianity for general finite-dimensional systems.

</details>


### [57] [Theory of the Uhlmann Phase in Quasi-Hermitian Quantum Systems](https://arxiv.org/abs/2603.01908)
*Xu-Yang Hou,Xin Wang,Hao Guo*

Main category: quant-ph

TL;DR: This paper develops a theory of Uhlmann phases in quasi-Hermitian systems, revealing new topological features and stability of nontrivial phases at finite temperatures.


<details>
  <summary>Details</summary>
Motivation: To address challenges in extending the Uhlmann phase to non-Hermitian systems, focusing on parameter-dependent inner product structures.

Method: Develop a theory for quasi-Hermitian systems using generalized purification, derive parallel transport conditions and Uhlmann connection, and analyze solvable two-level models.

Result: Discovered rich finite-temperature topological phase diagrams with nontrivial phases stabilized by quasi-Hermitian parameters, along with a framework for measuring geometric phases via Loschmidt fidelity.

Conclusion: This work provides a comprehensive understanding of mixed-state geometric phases in non-Hermitian systems, enhancing experimental investigation possibilities.

Abstract: Geometric phases play a fundamental role in understanding quantum topology, yet extending the Uhlmann phase to non-Hermitian systems poses significant challenges due to parameter-dependent inner product structures. In this work, we develop a comprehensive theory of the Uhlmann phase for quasi-Hermitian systems, where the physical Hilbert space metric varies with external parameters. By constructing a generalized purification that respects the quasi-Hermitian inner product, we derive the corresponding parallel transport condition and Uhlmann connection. Our analysis reveals that the dynamic metric induces emergent geometric features absent in the standard Hermitian theory. Applying this formalism to solvable two-level models, we uncover rich finite-temperature topological phase diagrams, including multiple transitions between trivial and nontrivial phases driven by thermal fluctuations. Crucially, the quasi-Hermitian parameters are shown to profoundly influence the stability of topological regimes against temperature, enabling nontrivial phases to persist within finite-temperature windows. Furthermore, by extending established interferometric protocols originally developed for Hermitian systems, the geometric amplitude can be recast as a measurable Loschmidt fidelity between purified states, providing a practical and experimentally accessible pathway to investigate quasi-Hermitian mixed-state geometric phases and their finite-temperature transitions. This work establishes a unified framework for understanding mixed-state geometric phases in non-Hermitian quantum systems and opens a practical avenue for their experimental investigation.

</details>


### [58] [Quantum Network Simulation and Emulation: A Roadmap for Quantum Internet Design](https://arxiv.org/abs/2603.01980)
*Brian Doolittle,Michael Cubeddu*

Main category: quant-ph

TL;DR: This paper reviews tools for simulating and emulating quantum networks, identifies their limitations, and proposes a roadmap for integrating quantum technologies to overcome these challenges.


<details>
  <summary>Details</summary>
Motivation: The need for efficient simulation and emulation tools to support the development of quantum networks, given the limitations of current classical methods.

Method: The paper reviews existing quantum network simulation and emulation tools, discusses their foundational principles, and analyzes their bottlenecks, followed by proposing a roadmap for integration with quantum testbeds.

Result: Identification of limitations in current classical simulation methods, highlighting issues like error rates, latency, and cost at scale, along with potential solutions through quantum technologies.

Conclusion: A structured approach is needed for the adoption of quantum simulation tools, advocating for codesign efforts with quantum network testbeds to enhance capabilities.

Abstract: Quantum networks are advancing the information technology infrastructure of society. Simulation and emulation software tools have emerged to support the design, development, and deployment of quantum networks, however, classical simulation and emulation methods have major bottlenecks in the error, latency, and cost that they can achieve at scale. In this work, we review quantum network simulation and emulation tools, including foundational principles, state-of-the-art tools, and bottlenecks. We then discuss how quantum technologies can address these challenges, and we construct a roadmap for the adoption of quantum simulation and emulation tools, emphasizing codesign with quantum network testbeds.

</details>


### [59] [Cavity-enhanced optical readout and control of nuclear spin qubits](https://arxiv.org/abs/2603.01987)
*Alexander Ulanowski,Johannes Früh,Fabian Salamon,Adrian Holzäpfel,Andreas Reiserer*

Main category: quant-ph

TL;DR: This paper presents a method for achieving efficient all-optical control and readout of nuclear spin qubits using 167-Er dopants in a cryogenic cavity, facilitating their use in quantum networks.


<details>
  <summary>Details</summary>
Motivation: To improve quantum memory capabilities in quantum networks by overcoming challenges in optical control of nuclear spin qubits.

Method: The study embeds 167-Er dopants in yttrium orthosilicate within a cryogenic Fabry-Perot cavity, enhancing frequency-selective emission to achieve high readout fidelity and coherence times.

Result: The method results in a single-shot readout fidelity of 91(2)% and coherence times exceeding 0.2 s, supported by the use of a large magnetic field to freeze impurities.

Conclusion: 167-Er is positioned as a promising platform for developing long-range, fiber-based quantum networks due to its efficient emission and coherence properties.

Abstract: Their exceptional coherence makes nuclear spins in solids a prime candidate for quantum memories in quantum networks and repeaters. Still, the direct all-optical initialization, coherent control, and readout of individual nuclear spin qubits have been an outstanding challenge. Here, this is achieved by embedding 167-Er dopants in yttrium orthosilicate in a cryogenic Fabry-Perot cavity, whose linewidth of 65 MHz is much smaller than the 0.9 GHz separation of neighboring hyperfine levels. Frequency-selective emission enhancement thus enables a single-shot readout fidelity of 91(2)%. Furthermore, a large magnetic field freezes paramagnetic impurities, leading to coherence times exceeding 0.2 s. The combination of nuclear-spin qubits with frequency-multiplexed addressing and lifetime-limited photon emission in the minimal-loss telecommunications C-band establishes 167-Er as a leading platform for long-range, fiber-based quantum networks.

</details>


### [60] [Decoherence and entropy production due to quantum fluctuations of spacetime](https://arxiv.org/abs/2603.02034)
*Thiago H. Moreira*

Main category: quant-ph

TL;DR: The thesis explores the dynamics of an open quantum system influenced by a bath of gravitons, focusing on decoherence from internal and external degrees of freedom and entropy production driven by an external agent.


<details>
  <summary>Details</summary>
Motivation: The study addresses the complex relationship between quantum mechanics and gravitational physics, particularly the effects of gravitons on quantum systems.

Method: We analyze decoherence effects caused by gravitons on systems with both external and internal degrees of freedom, as well as entropy production from the interaction with an externally driven graviton bath.

Result: Decoherence of spatial superpositions in microscopic systems occurs over time due to gravitons, even when the bath alone does not induce it; entropy production reveals universal irreversibility tied to spacetime quantum fluctuations.

Conclusion: The findings highlight the profound implications of graviton interactions on quantum systems, demonstrating a fundamental link between gravity and quantum mechanics.

Abstract: The intersection between quantum mechanics and gravitational physics has been providing challenging puzzles for decades. In this thesis, we study the dynamics of an open quantum system coupled with a bath of gravitons, the quanta of the gravitational field in the linear limit of general relativity. We focus on two main aspects. First, we analyze the decoherence induced by gravitons when we consider the open system to be described by both external and internal degrees of freedom. Since gravity is universal, the internal variables also interact with the gravitons, and here we show that this interaction leads to the decoherence of spatial superpositions of microscopic systems in the long-time regime, even when the graviton bath alone does not. We then proceed to the second main aspect, which is the entropy production that arises when an external agent drives a quantum system through the graviton bath. This irreversibility comes from quantum fluctuations of spacetime itself and, as such, has a fundamentally universal aspect.

</details>


### [61] [Using anti-squeezed Schrödinger cat states for detection of a given phase shift](https://arxiv.org/abs/2603.02038)
*V. L. Gorshenin,K. D. Dyadkin,S. D. Chikalkin*

Main category: quant-ph

TL;DR: The paper introduces antisqueezing-enhanced non-Gaussian Schrödinger cat states for better phase shift detection in optical interferometers, enhancing robustness against optical losses.


<details>
  <summary>Details</summary>
Motivation: To improve detection sensitivity in optical interferometers under conditions of optical loss.

Method: Utilization of antisqueezing in non-Gaussian Schrödinger cat states to optimize phase shift detection and assess sensitivity against conventional Gaussian squeezed states.

Result: Optimal antisqueezing levels were identified, leading to enhanced robustness and greater sensitivity compared to Gaussian squeezed states under similar conditions.

Conclusion: Antisqueezing improves phase detection sensitivity in optical interferometry, making non-Gaussian states favorable in lossy environments.

Abstract: We propose to use the antisqueezing-enhanced non-Gaussian Schrödinger cat quantum states of the probing light for the task of detection of a given phase shift in optical interferometers. We show that the antisqueezing allows to increase the robustness of the setup to optical losses. We find the optimal degrees of the antisqueezing for experimentally achievable values of the Schrödinger cat amplitude and the optical losses and compare the resulting sensitivity with the one provided by the Gaussian squeezed states.

</details>


### [62] [Levitated Ferromagnetic Torsional Oscillators for High-Precision Magnetometry and Probing Exotic Interactions](https://arxiv.org/abs/2603.02074)
*Ren Yichong,Wu Lielie,Broer Wijnand,Xue Fei,Huang Pu,Du JiangFeng*

Main category: quant-ph

TL;DR: The paper presents a high-sensitivity magnetometer utilizing a levitated ferromagnetic torsion oscillator, achieving remarkable magnetic sensitivity and potential for fundamental physics exploration.


<details>
  <summary>Details</summary>
Motivation: To harness mechanical isolation in levitated ferromagnetic systems for improved precision in magnetic field sensing and reduced noise.

Method: Implementation of a levitated ferromagnetic torsion oscillator with a centroid tracking method to enhance measurement resolution and minimize noise.

Result: Achieved a magnetic sensitivity of {$391	ext{	extpm} 59 \, m{fT	ext{	extperiodcentered} Hz^{-1/2}}$ in a compact sensor volume of $(2.5 \, m{mm})^3$ operating at room temperature.

Conclusion: The levitated torsion oscillator system is a powerful tool for high-precision magnetic sensing and shows potential for significant advancements in fundamental physics.

Abstract: Levitated ferromagnetic systems are expected to have significant potential in precision magnetic field sensing by leveraging mechanical isolation to minimize mechanical contact and associated noise. Here, we report the implementation of a high-sensitivity magnetometer based on a levitated ferromagnetic torsion oscillator, incorporating a centroid tracking method for superior measurement resolution and noises reduction. The device, featuring a compact sensor volume of $(2.5 \, \rm{mm})^3$ and operating under room temperature, attains a remarkable magnetic sensitivity of {$391\pm 59 \, \rm{fT\cdot Hz^{-1/2}}$}. This capability enables precise detection of weak magnetic fields and provides a novel platform for exploring exotic interactions beyond the Standard Model. These results demonstrate that the levitated torsion oscillator system not only serves as a powerful tool for high-precision magnetic sensing but also holds promise for advancing breakthroughs in fundamental physics.

</details>


### [63] [Transmitting Correlation for Data Transmission over the Bosonic Arbitrarily Varying Channel](https://arxiv.org/abs/2603.02078)
*Janis Nötzel,Florian Seitz*

Main category: quant-ph

TL;DR: This paper examines how to distribute shared randomness in symmetrizable communication systems to mitigate jamming attacks, using correlated thermal light or entangled states in a constrained optical communication model.


<details>
  <summary>Details</summary>
Motivation: To stabilize communication systems against variational jammers, it's crucial to investigate the distribution of shared randomness, particularly in the context of optical communication.

Method: The authors utilize classically correlated thermal light and entangled two-mode squeezed states, employing homodyne detection under power constraints to counteract energy-limited jamming.

Result: The proposed approach effectively establishes a method for sharing randomness, mitigating the jamming effects in a realistic optical communication scenario.

Conclusion: The findings suggest that both correlated and entangled light can serve as valuable resources for distributing shared randomness to enhance the robustness of communication against jamming.

Abstract: Shared randomness is the central ingredient for stabilizing symmetrizable communication systems against arbitrarily varying jammers. Given the presence of the jammer, however, the question arises how this precious resource could have been distributed. Several works discuss the use of external sources for this task. In this work, we show, based on the most standard optical communication model, how the sender and receiver can employ either classically correlated thermal light or entangled two-mode squeezed states created at and transmitted by the sender to counter the jamming attack of an energy-limited jammer during the distribution phase. Both sender and receiver are only allowed to use homodyne detection in our model, and the sender has to obey a power limit as well.

</details>


### [64] [Obstacles to Continuous Quantum Error Correction via Parity Measurements](https://arxiv.org/abs/2603.02106)
*Anton Halaski,Christiane P. Koch*

Main category: quant-ph

TL;DR: The paper examines the limitations of time-continuous quantum error correction due to measurement backaction in parity-measurement protocols, suggesting alternatives for error protection.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of protecting quantum information under time-dependent Hamiltonians using continuous quantum error correction.

Method: Analyzed the effects of continuous coupling in parity-measurement protocols within circuit quantum electrodynamics and generalized the findings to broader contexts.

Result: Demonstrated that common measurement protocols corrupt logical information due to inadequate handling of three-body interactions, leading to measurement backaction issues.

Conclusion: Identified practical limits on continuous stabilizer quantum error correction and proposed alternatives such as architectures with three-body interactions and erasure-based encodings.

Abstract: Time-continuous quantum error correction, necessary to protect quantum information under time-dependent Hamiltonians, relies on weak continuous syndrome measurements. Implementing these measurements requires a continuous coupling among at least two qubits and a meter, a demanding requirement. We show that, under continuous operation, common parity-measurement protocols in the circuit quantum electrodynamics platform corrupt the logical information. The failure arises from approximating the three-body interaction by a sum of two-body couplings to the meter, which prevents simultaneous suppression of measurement backaction on the logical and error subspaces. We argue that the same mechanism applies more generally beyond the circuit quantum electrodynamics setting. Taken together, our results impose a practical limitation on continuous stabilizer quantum error correction and point to the viable alternatives -- architectures that realize native three-body interactions, or erasure-based encodings in which the error subspace need not be protected.

</details>


### [65] [Quantum algorithm for the lattice Boltzmann method with applications on real quantum devices](https://arxiv.org/abs/2603.02127)
*Antonio Bastida-Zamora,Ljubomir Budinski,Oskari Kerppo,Valtteri Lahtinen,Ossi Niemimäki,William Steadman,Roberto Zamora-Zamora,Pierre Sagaut,Vladyslav Bohun,Maciej Koch-Janusz,Illia Lukin*

Main category: quant-ph

TL;DR: A new quantum algorithm for the lattice Boltzmann method (LBM) is introduced, enhancing flexibility and retaining computational efficiency for various physics modeling.


<details>
  <summary>Details</summary>
Motivation: To improve the flexibility and efficiency of quantum algorithms for the lattice Boltzmann method in modeling diverse physical phenomena.

Method: Introduction of a novel algorithm based on the one-step simplified LBM, allowing for adaptations in modeling while optimizing gate and qubit requirements.

Result: The new algorithm demonstrates capability in modeling linear acoustics and effectively addresses a nonlinear Navier-Stokes problem using IBM QPU in a hybrid simulation.

Conclusion: This algorithm enhances potential for practical quantum applications, particularly for linear problems in physics simulations.

Abstract: We introduce a novel quantum algorithm for the lattice Boltzmann method (LBM) based on the one-step simplified LBM. The structure of the algorithm allows for more flexibility in modelling different physics in contrast to earlier quantum algorithms for the LBM, while retaining computational efficiency in terms of the gate and qubit complexity. The new algorithm has potential for full end-to-end quantum utility especially for linear problems. We discuss the implementation of examples in linear acoustics, as well as a nonlinear Navier-Stokes problem that was solved on an IBM QPU in a hybrid simulation loop.

</details>


### [66] [Constant-Time Surgery on 2D Hypergraph Product Codes with Near-Constant Space Overhead](https://arxiv.org/abs/2603.02157)
*Kathleen Chang,Zhiyang He,Theodore J. Yoder,Guanyu Zhu,Tomas Jochym-O'Connor*

Main category: quant-ph

TL;DR: This paper introduces a method for reducing time overhead in quantum code surgery through parallel logical measurements, achieving constant time and near-constant space overhead.


<details>
  <summary>Details</summary>
Motivation: To improve the time efficiency of fault-tolerant computation in quantum low-density parity-check codes, which traditionally suffer from high time overhead.

Method: The authors develop surgery gadgets enabling parallel logical measurements on 2D hypergraph product codes, reducing time overhead to constant ($O(1)$) and space overhead to near-constant ($	ilde{O}(1)$) by leveraging amortization of multiple operations.

Result: This new approach maintains the flexibility of surgery while offering overheads similar to transversal gates, thus enhancing practical implementation.

Conclusion: These gadgets open up new avenues for designing fast logical computation in quantum error correction, making them suitable for near-term experimental applications.

Abstract: Generalized code surgery is a versatile and low-overhead technique for performing fault-tolerant computation on quantum low-density parity-check (qLDPC) codes. In many settings, surgery exhibits practical space overheads, while its time overhead remains a bottleneck at $O(d)$ syndrome rounds per operation. In this work, we construct surgery gadgets that perform parallel logical measurements on 2D hypergraph product codes in constant time overhead ($O(1)$) and near-constant space overhead ($\tilde{O}(1)$). The reduced time overhead is a result of amortization, as we show, following the formulation by Cowtan et al. (arXiv:2510.14895), that performing $d$ surgery operations in $O(d)$ time is fault tolerant. Our gadgets combine the strengths of different approaches to fault-tolerant logical operations: they partially retain the flexibility of surgery while achieving overheads comparable to transversal gates. Consequently, they are well-suited for near-term experimental realization and demonstrate new possibilities in the design of gadgets for fast logical computation.

</details>


<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [67] [A Particle Detector Array deployed to the Murchison Widefield Array in the Murchison Radio-astronomy Observatory](https://arxiv.org/abs/2603.00414)
*J. E. Dickinson,J. D. Bray,D. Kenney,T. Booler,J. Edgley,D. Emrich,A. Forouzan,T. Gould,A. McPhail,P. Roberts,R. E. Spencer,L. Verduyn,R. Watson,A. Williams,K. Grainge,A. Haungs,T. Huege,C. W. James,S. J. Tingay*

Main category: astro-ph.IM

TL;DR: The Murchison Widefield Array Particle Detector Array (MWA PDA) effectively identifies cosmic ray extensive air showers (EAS) and triggers radio data capture at the Murchison Radio-astronomy Observatory, demonstrating reliable performance and potential as a precursor for larger future instruments.


<details>
  <summary>Details</summary>
Motivation: To detect cosmic ray extensive air showers at the Murchison Radio-astronomy Observatory and provide a trigger for capturing associated radio data.

Method: Eight particle scintillation detectors deployed to monitor EAS, with calibration to confirm their performance and temperature dependence assessment, followed by a study of EAS over 13 days.

Result: Identified 35,500 EAS with reliable trigger generation for cosmic events at high energies, confirming the detectors' functionality and compliance with radio frequency requirements.

Conclusion: The MWA PDA effectively reconstructs EAS events within specified parameters and can enhance future observational capabilities in cosmic ray research.

Abstract: We report the design and functionality of the Murchison Widefield Array Particle Detector Array (MWA PDA), an array of eight particle scintillation detectors deployed to Inyarrimanha Ilgari Bundara, the Murchison Radio-astronomy Observatory (MRO). The purpose of the instrument is to identify cosmic ray extensive air showers (EAS) occurring over the core of the MWA radio telescope, and generate a trigger to allow radio data on the event to be captured and analysed. The system also acts as a pathfinder for a much larger instrument to be deployed in the core of the low-frequency component of the Square Kilometre Array, SKA-Low, by the SKA's ultra-high-energy particles science working group. Here, we describe the instrument and associated infrastructure, which has been verified to comply with the strict radio-frequency emissions requirements of the MRO, and was deployed in November 2024. We present calibration data, which demonstrates the ability of each detector to identify individual atmospheric muons at the expected rate, and we characterise the temperature dependence of the system. We describe a sample of 35,500 EAS identified using multi-detector coincidence over a 13-day period, and show how the detector data can be used to reconstruct the arrival directions and approximate energies of these events. We conclude that the PDA can reliably trigger on and reconstruct EAS contained within the $\sim 103 \times 90$ m$^2$ core region, arriving within 20$^{\circ}$ of zenith, at primary cosmic ray energies above $\sim 4$ PeV. We have also verified that the detector array can generate triggers, allowing the capture of radio data from the MWA correlator for offline analysis.

</details>


### [68] [Detection and classification of astronomical sources with Astro-RetinaNet in crowded stellar fields](https://arxiv.org/abs/2603.00473)
*Yibo Yan,Chao Liu,Jiadong Li,Feng Wang*

Main category: astro-ph.IM

TL;DR: Astro-RetinaNet, a deep learning model, effectively detects blended stars in high-density astronomical images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Next-generation surveys will find many faint celestial objects, making accurate detection challenging due to blending in crowded fields.

Method: Astro-RetinaNet, an advanced deep learning model based on the Retinanet algorithm, was developed to detect and classify blended sources in single-band images after being trained on simulated data.

Result: The model achieved high precision scores for varying degrees of star blending and outperformed traditional methods in detecting sources in both simulated and observed data.

Conclusion: Astro-RetinaNet significantly improves source recovery rates in crowded fields, making it a valuable tool for future astronomical surveys.

Abstract: Upcoming next-generation sky surveys will detect large number of faint objects with magnitudes larger than 25. When objects are crowded within a limited a field of view, blending becomes unavoidable. Blending leads to the omission of many sources during photometry in these fields, which cause an underestimates of tens of percent in crowded fields, and remains a major challenge for existing source-extraction techniques. Although artificial neural networks had shown promising results in the detection and classification in wide-field surveys, they often fail with severely blended stars. We developed a robust deep learning model, Astro-RetinaNet, based on the Retinanet algorithm to detect and classify blended sources in single-band astronomical images. After training and evaluating the performance of our network on simulated images, we find precision of 0.96, 0.89,0.70, 0.50,0.75 for single star, 2-star, 3-star, 4-star and 5-or-more star blending cases, respectively, with star number density $\sim$22000 stars per $\rm arcmin^2$. We compare our method's detection capability and completeness both on CSST simulated NGC 2298 images and HST observed M31 images. In crowded and non-crowded stellar fields of simulated NGC 2298, our results show that the model can recover $82\%$ and $95\%$ sources respectively at magnitude ($i$ band) of 25, while for SExtractor and Photutils the completeness reduces to $20\%, 59\%$ and $60\%, 88\%$ respectively. In the M31 case, as faint as 27 magnitude ($F814W$) in a crowded field, Astro-RetinaNet detects 2,224 sources, significantly outperforming Photutils and SExtractor by factors of 3.4 and 7.1, respectively.

</details>


### [69] [Deep learning-based astronomical multimodal data fusion: A comprehensive review](https://arxiv.org/abs/2603.00699)
*Wujun Shao,Dongwei Fan,Chenzhou Cui,Yunfei Xu,Shirui Wei,Xin Lyu*

Main category: astro-ph.IM

TL;DR: The paper reviews multimodal data fusion (MDF) in astronomy, highlighting its necessity due to data complexity and advancements in AI and deep learning.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of unimodal data analysis in the face of abundant astronomical data from various sources.

Method: The review covers the motivation for MDF, data sources, major modalities, deep learning models, the fusion process, strategies, and surveys existing studies.

Result: It identifies characteristics, applicability, advantages, and limitations of various deep learning models and fusion strategies in astronomical MDF.

Conclusion: The paper synthesizes key findings, discusses challenges, and suggests directions for future research in DL-based MDF in astronomy.

Abstract: With the rapid advancements in observational technologies and the widespread implementation of large-scale sky surveys, diverse electromagnetic wave data (e.g., optical and infrared) and non-electromagnetic wave data (e.g., gravitational waves) have become increasingly accessible. Astronomy has thus entered an unprecedented era of data abundance and complexity. Astronomers have long relied on unimodal data analysis to perceive the universe, but these efforts often provide only limited insights when confronted with the current massive and heterogeneous astronomical data. In this context, multimodal data fusion (MDF), as an emerging method, provides new opportunities to enhance the value of astronomical data and deepening the understanding of the universe by integrating information from different modalities. Recent progress in artificial intelligence (AI), particularly in deep learning (DL), has greatly accelerated the development of multimodal research in astronomy. Therefore, a timely review of this field is essential. This paper begins by discussing the motivation and necessity of astronomical MDF, followed by an overview of astronomical data sources and major data modalities. It then introduces representative DL models commonly used in astronomical multimodal studies, the general fusion process as well as various fusion strategies, emphasizing their characteristics, applicability, advantages, and limitations. Subsequently, the paper surveys existing astronomical multimodal studies and datasets. Finally, the discussion section synthesizes key findings, identifies potential challenges, and suggests promising directions for future research. By offering a structured overview and critical analysis, this review aims to inspire and guide researchers engaged in DL-based MDF in astronomy.

</details>


### [70] [PhysFormer: A Physics-Embedded Generative Model for Physically Self-Consistent Spectral Synthesis](https://arxiv.org/abs/2603.01459)
*Siqi Wang,Mengmeng Zhang,Yude Bu,Chaozhou Mou*

Main category: astro-ph.IM

TL;DR: PhysFormer, a generative model, ensures physical consistency in high-dimensional systems governed by PDEs by learning significant physical quantities from data without requiring known parameters, enhancing stability and fidelity in spectral generation.


<details>
  <summary>Details</summary>
Motivation: There is a significant challenge in achieving physical consistency and numerical stability in modeling high-dimensional complex systems governed by PDEs, especially using traditional methods like PINNs, which struggle with known parameters and training stability.

Method: PhysFormer utilizes a low-dimensional latent space to directly learn key physical quantities from data. It incorporates the physical process of radiative flux generation to enforce physical consistency within its generative framework, moving physical constraints from loss functions into the model mechanism itself.

Result: PhysFormer demonstrates improved spectral fidelity and inversion stability compared to existing methods, particularly in high-dimensional and degenerate inversion tasks, even as signal-to-noise ratios vary.

Conclusion: The proposed PhysFormer framework establishes a new paradigm in generative modeling for complex systems, allowing for more robust handling of unknown physical quantities while maintaining physical integrity.

Abstract: In scientific and engineering domains, modeling high-dimensional complex systems governed by partial differential equations (PDEs) remains challenging in terms of physical consistency and numerical stability. However, existing approaches, such as physics-informed neural networks (PINNs), typically rely on known physical fields or coefficients and enforce physical constraints via external loss functions, which can lead to training instability and make it difficult to handle high-dimensional or unobservable scenarios. To this end, we propose PhysFormer, a generative modeling framework that is self-consistent at both the data and physical levels. PhysFormer leverages a low-dimensional, physically interpretable latent space to learn key physical quantities directly from data without requiring known high-dimensional physical field parameters, and embeds the physical process of radiative flux generation within the network to ensure the physical consistency of the generated spectra. In high-dimensional, degenerate inversion tasks, PhysFormer constrains generation within physical limits and enhances spectral fidelity and inversion stability under varying signal-to-noise ratios (SNRs). More broadly, this approach shifts the physical processes from external loss functions into the generative mechanism itself, providing a physically consistent generative modeling paradigm for complex systems involving unknown or unobservable physical quantities.

</details>


### [71] [CosmicWeb-21cm array: A New Radio Observation Array Design for 21cm Cosmology](https://arxiv.org/abs/2603.01529)
*Jiancheng Wang,Jirong Mao,Xiangming Cheng,Yigong Zhang,Jie Su,Xiaogu Zhong,Min Wang,Zhigang Zhang,Qingwei Wang,Yonghua Xu,Zhixuan Li,Longhua Qin,Zhengjun Zhang*

Main category: astro-ph.IM

TL;DR: The CosmicWeb-21cm array is a novel radio interferometer designed to enhance 21 cm cosmology by integrating innovative geometrical, sampling, and processing techniques.


<details>
  <summary>Details</summary>
Motivation: To address challenges in 21 cm cosmology, particularly in sensitivity and data processing efficiency for cosmic structures.

Method: Utilizes a multi-scale nested geometry, adaptive frequency sampling, machine-learning processing, and advanced dipole design for data acquisition.

Result: Achieved 99.7% foreground removal efficiency, low noise temperature, and stable beam patterns, facilitating enhanced mapping of the Epoch of Reionization and cosmic structures.

Conclusion: The co-designed technological advances enhance the capabilities of 21 cm cosmology, providing high sensitivity for probing dark energy and cosmic web structures.

Abstract: This paper presents the CosmicWeb-21cm array, a novel radio interferometer designed to overcome the key challenges in 21 cm cosmology. Its core innovations include: (1) a multi-scale nested geometry combining a hexagonal core with logarithmic spiral arms for excellent UV coverage and calibration robustness; (2) an intelligent non-uniform frequency sampling strategy that adapts resolution to foreground and signal characteristics, reducing data volume while preserving information; and (3) a machine-learning-enhanced, physics-informed processing pipeline that achieves 99.7\% foreground removal efficiency; (4) a dual-polarization crossed dipole integrated with a dielectric lens and cryogenically cooled LNA, achieving stable beam patterns and low noise temperature ($<35$ K) across 50-250 MHz. These co-designed advances enable high sensitivity mapping of the Epoch of Reionization, dark energy constraints and cosmic-web structure.

</details>


### [72] [SPACE-Timers -- A Stack-Based Hierarchical Timing System for C++](https://arxiv.org/abs/2603.01618)
*Geray S. Karademir,Klaus Dolag*

Main category: astro-ph.IM

TL;DR: SPACE-Timers is a lightweight profiling framework for HPC in C++ that efficiently analyzes execution patterns, offering structured reports and integration with various tools.


<details>
  <summary>Details</summary>
Motivation: To facilitate performance analysis in high-performance computing applications with minimal overhead and robust reporting features.

Method: Utilizes a stack-based timing model and hierarchical profiling to represent runtime behavior as a tree of timing nodes, aggregating data recursively and detecting unaccounted time.

Result: Successfully demonstrated effectiveness in profiling large-scale scientific applications, specifically in OpenGadget3, showing its utility across various HPC tasks.

Conclusion: SPACE-Timers enhances the profiling process in C++ applications, supporting detailed analysis and energy-aware optimizations through integration with multiple backends.

Abstract: SPACE-Timers are a lightweight hierarchical profiling framework for C++ designed for modern high-performance computing (HPC) applications. It uses a stack-based timing model to capture deeply nested execution patterns with minimal overhead, representing runtime behaviour as a tree of timing nodes with precise attribution. The framework provides structured reports with recursive aggregation, detection of unaccounted time, and compact visual summaries of runtime distribution, supporting both quick inspection and detailed analysis. It also includes checkpointing and error detection mechanisms. SPACE-Timers supports multiple profiling backends, including NVTX, ITT, ROCtx, and Omnitrace, and integrates with the MERIC runtime system to enable energy-aware optimisation. Its successful use in OpenGadget3 demonstrates its effectiveness for large-scale scientific applications.

</details>


### [73] [Systematic errors in spectral measurements with the Tenerife Microwave Spectrometer](https://arxiv.org/abs/2603.02016)
*A. M. Arriero-Lopez,J. A. Rubiño-Martín,F. Cuttaia,L. Terenzi,R. Hoyland*

Main category: astro-ph.IM

TL;DR: This paper presents a model for the TMS radiometer to analyze and quantify the effects of instrumental non-idealities on absolute sky-temperature measurements.


<details>
  <summary>Details</summary>
Motivation: To understand and quantify the impact of non-idealities in absolute radiometric measurements, crucial for accurate sky-temperature readings.

Method: Utilized the Jones-matrix formalism for non-ideal signal effects and the Friis formalism for noise contributions, accounting for losses throughout the instrument.

Result: Identified percent-level non-ideal mixing effects and a frequency-dependent output offset averaging 6.9 K, primarily from the cryostat window, impacting thermal stability and signal variation.

Conclusion: Provided a comprehensive assessment of systematic effects in absolute radiometric measurements, offering a framework for TMS instrument calibration and performance enhancement.

Abstract: We present an analytical instrument model of the TMS radiometer, a pseudo-correlation system designed for absolute sky-temperature measurements through a continuous comparison between sky and reference-load signals. The goal of this work is to quantify and understand the impact of instrumental non-idealities that are intrinsic to absolute radiometric measurements. We used a combination of the Jones-matrix formalism, to describe non-ideal signal mixing effects in the different instrument components, and the Friis formalism, to account for noise-temperature contributions which introduce specific offset terms. The model includes all components from the cryostat entrance window onwards and allows us to propagate realistic losses, return losses, and noise figures through the system. We find that non-ideal mixing effects, such as intensity-to-polarization and sky-to-load leakage, are expected to appear at the percent level, but they could be in principle calibrated out. The total TMS intensity response exhibits a frequency-dependent offset with a band-averaged level of 6.9 K, dominated by the cryostat window, and with smaller contributions from the infrared filter and orthomode transducers. Under the required TMS thermal stability of 1 mK over one hour, the resulting variation in the output signal can reach up to 91.3 mK, which sets the fundamental limit on the absolute sky-temperature accuracy. In contrast, relative spectral measurements across the 10-20 GHz band are stable at the few-microkelvin level, consistent with the instrument target sensitivity of ~ 10 Jy/sr. This work provides a detailed and quantitative assessment of the systematic effects affecting absolute radiometric measurements and establishes a robust framework for the calibration and performance optimization of the TMS instrument.

</details>


### [74] [The statistical properties of the cross spectrum](https://arxiv.org/abs/2603.02094)
*Edward J. R. Nathan,Adam Ingram,Daniela Huppenkothen,Matteo Bachetti,Javier A. García*

Main category: astro-ph.IM

TL;DR: This paper derives the statistical distribution of the cross spectrum of two time series, showing it follows an asymmetric Laplace distribution, and relates it to power spectra, enhancing its use in astrophysical studies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of a comprehensive statistical basis for the cross spectrum in the context of astronomical applications, specifically X-ray timing.

Method: The authors derive the statistical distribution of the cross spectrum and present the probability distribution function along with marginal distributions of various quantities.

Result: The study finds that the cross spectrum follows an asymmetric Laplace distribution, contributing new insights into its statistical properties.

Conclusion: This work provides a statistical framework that enhances the use of the cross spectrum in investigating physical processes like black hole and neutron star accretion.

Abstract: The cross spectrum encodes the correlated variability between two time signals. In recent years, the cross spectrum has been used to study astronomical sources, particularly in the field of X-ray timing. In the literature, it has been common to either simultaneously fit the real and imaginary components of the cross spectrum, or fit the phase and magnitude. Until now, a full discussion of the statistical distribution of the cross spectrum has been missing from the astronomical literature. In this paper, we present a derivation of the full statistical distribution of a cross spectrum between two time series, showing that it follows an asymmetric Laplace distribution. We further provide the probability distribution function for a cross spectrum random variable, along with the marginal distributions for many quantities. We also relate the cross spectrum to the power spectra of the constituent time series. This work will enable the cross spectrum to be used more accurately as a probe of physical processes such as accretion onto black holes and neutron stars.

</details>
