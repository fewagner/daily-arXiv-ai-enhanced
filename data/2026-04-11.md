...
TL;DR: PowerSpectR is an R package for calculating and visualizing median-based radial Fourier power spectra from imaging data, enhancing the study of spatial structure.


<details>
  <summary>Details</summary>
Motivation: To provide a robust method for analyzing spatial structures in imaging data while reducing bias from localized features and artifacts.

Method: The package implements median-based radial Fourier power spectra computation using edge-effect mitigation via Hann windowing, Fourier-domain analysis, and azimuthal median statistics.

Result: PowerSpectR yields low-dimensional summaries of morphological complexity in images by estimating slopes in power spectra.

Conclusion: The tool is an effective resource for researchers aiming to analyze spatial structures in imaging data, mitigating issues of localized bias.
...
TL;DR: ASTRAFier is a new Transformer-based model for classifying stellar variability in light curves from Kepler and TESS, achieving high accuracy without complex feature engineering.


<details>
  <summary>Details</summary>
Motivation: To leverage extensive light curve data from missions like Kepler and TESS for improved stellar variability classification using machine learning.

Method: ASTRAFier integrates BiLSTM and CNNs into a Transformer-based framework that operates directly on time series data with no feature engineering.

Result: The model achieved classification accuracies of 94.26% on Kepler data and 88.22% on TESS data, demonstrating effective scalability on a large dataset of approximately 2.8 million TESS light curves.

Conclusion: The successful application of ASTRAFier illustrates its potential for efficient and accurate stellar variability classification, with a public stellar variability catalog released for further research.
...
TL;DR: The CE4 mission explored periodic technosignatures in lunar farside radio observations but found no credible signals, establishing a framework for future lunar SETI studies.


<details>
  <summary>Details</summary>
Motivation: To explore lunar farside for radio SETI, minimizing terrestrial radio interference.

Method: Utilized PCA, cross-antenna alignment, and diagnostics for periodicity and frequency structure on CE4 dynamic spectra.

Result: No periodic candidate signal was found, indicating no evidence of artificial signals in the CE4 sample.

Conclusion: The study establishes a framework for lunar radio SETI and highlights the potential of future lunar missions for expanding research.
...
TL;DR: A new frequentist method efficiently detects continuous gravitational waves in Pulsar Timing Array data, outperforming traditional Bayesian methods in speed and maintaining comparable accuracy.


<details>
  <summary>Details</summary>
Motivation: The computational cost of Bayesian methods for detecting continuous gravitational waves in large Pulsar Timing Arrays limits their scalability and feasibility for future observations.

Method: An adaptive spline fitting algorithm non-parametrically reduces red noise and optimizes pulsar subsets to streamline the detection process.

Result: The new method achieves a 1.0% strain error and 0.072% frequency error for CGW signals, proving to be nearly as accurate as Bayesian analysis but significantly faster (completed in <5 hours).

Conclusion: The proposed frequentist method offers a scalable and efficient alternative for continuous gravitational wave detection in large-scale PTA data.
...
TL;DR: The paper introduces FLARE, a framework for detecting quasar flares in the SDSS Stripe 82 dataset by modeling normal variability, scoring anomalies, and verifying flare candidates using advanced machine learning techniques.


<details>
  <summary>Details</summary>
Motivation: Quasar flares indicate significant variations in luminosity that provide insights into the dynamics of supermassive black holes and their accretion discs but have not been systematically studied in existing datasets.

Method: The FLARE framework employs a three-stage approach: 1) models baseline behaviors using a physics-informed GRU, 2) scores anomalies via EVT, and 3) verifies flare candidates with Vision Language Models, ensuring multi-band consistency.

Result: The method identified 27 quasars with notable flaring activity from the extensive SDSS Stripe 82 dataset, effectively distinguishing flares from background noise.

Conclusion: FLARE offers an effective and systematic approach to detect quasar flares, enhancing our understanding of their underlying physical processes.
...
TL;DR: This study evaluates the stability and performance of GEM chamber prototypes using a $^{55}$Fe source over 98 days, focusing on gain, efficiency, and energy resolution under different conditions.


<details>
  <summary>Details</summary>
Motivation: The research aims to ensure the reliability and performance of GEM detectors for future large-scale experiments in Micro Pattern Gaseous Detectors.

Method: Performance evaluation was conducted through continuous radiation exposure and measurement of gain, efficiency, and energy resolution, while monitoring temperature, pressure, and humidity.

Result: The study presents measured values of gain and energy resolution, along with observations on the count rate over the long-term exposure to radiation.

Conclusion: The findings are critical for assessing the performance stability of GEM detectors prior to their deployment in large-scale experiments.
...
TL;DR: SPIROS is a specialized optical simulation tool for particle physics detectors, designed to be faster and more user-friendly than general frameworks like GEANT4.


<details>
  <summary>Details</summary>
Motivation: To provide a dedicated tool for the design and analysis of optical processes in particle physics detectors, streamlining simulation tasks.

Method: SPIROS utilizes a lightweight simulation engine with an intuitive interface, allowing direct import of 3D CAD models and configuration via a single input file for various optical processes.

Result: Validation against GEANT4 indicates high accuracy in photon behaviors, with SPIROS operating over two times faster in typical use cases.

Conclusion: SPIROS has been successfully applied in multiple neutrino experiments, is open-source, and aims to enhance detector design and optimization efficiency.
...
TL;DR: The study investigates the afterpulse effect in silicon photomultipliers (SiPMs) using a single-cell readout structure, employing three analysis methods validated by simulations to characterize afterpulse dynamics.


<details>
  <summary>Details</summary>
Motivation: To understand the afterpulse effect in SiPMs and its dependence on irradiation, providing insights for improved detector performance.

Method: Three independent methods: charge integration and two multiple linear regression techniques, to analyze afterpulse events and dynamics.

Result: Measured afterpulse probabilities and time constants in three SiPM samples, noting low values that do not significantly depend on irradiation fluence.

Conclusion: Afterpulse characteristics remain stable across various conditions, suggesting robustness of SiPMs under irradiation.
...
TL;DR: This research demonstrates the application of Graph Neural Networks (GNNs) for enhancing energy measurement and particle identification in a proposed second detector for the Electron Ion Collider, achieving superior performance compared to classical methods.


<details>
  <summary>Details</summary>
Motivation: To improve energy measurement and particle identification tasks in detector systems, particularly for the upcoming EIC, using advanced machine learning techniques.

Method: Graph Neural Networks are utilized to represent particle hits as graphs for classification and prediction tasks, alongside a parameterization to accelerate scintillator optical photon simulation by 20 times.

Result: The GNN approach surpasses classical methods in performance for energy and particle classification, providing improved energy/timing resolution and identification accuracy.

Conclusion: Integrating GNNs into a Multi-Objective Optimization framework allows for efficient evaluation and tradeoff quantification in detector design parameters affecting performance metrics.
...
TL;DR: This study analyzes two binary black hole merger candidates, GW190711_030756 and GW200114_020818, revealing their mass ratios, spin characteristics, and implications for their origins.


<details>
  <summary>Details</summary>
Motivation: To analyze the astrophysical properties of significant binary black hole merger candidates and their implications for the formation scenarios of black holes.

Method: Utilization of numerical relativity surrogate models for inferring source and remnant properties of the candidates.

Result: GW190711_030756 and GW200114_020818 are identified as asymmetric-mass binaries with differing mass ratios and spin characteristics, indicating an emerging population of rapidly spinning black holes.

Conclusion: The findings suggest complex formation scenarios for these black holes, with implications for their retention in various host environments.
...
TL;DR: The paper reports the first detection of variable linear polarization from MAXI J1744−294, a potential low-mass X-ray binary near the Galactic center, revealing a significant Faraday rotation measure and implications for its magnetic environment.


<details>
  <summary>Details</summary>
Motivation: To investigate the polarization properties of MAXI J1744−294 to understand its environment and confirm its location relative to the Galactic center.

Method: The study employed observations of the Stokes parameters at 33 and 43 GHz over four epochs, measuring Faraday rotation to determine magnetic field characteristics.

Result: A Faraday rotation measure of RM = -63,606 radians m$^{-2}$ was detected, indicating MAXI J1744's location within the Galactic center; a secondary RM of approximately -6000 rad m$^{-2}$ was noted during one observation.

Conclusion: The findings support MAXI J1744's association with the Galactic center and suggest the presence of a localized magnetic field, possibly linked to a jetted outflow from an X-ray binary system.
...
TL;DR: First self-consistent simulation of subphotospheric shocks in GRBs reveals important dynamics between radiation and shock propagation, leading to significant insights about their spectral properties.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the complexities of modeling subphotospheric shocks in gamma-ray bursts due to varying timescales and the dynamic role of radiation in these phenomena.

Method: The research employs a radiation-hydrodynamic simulation tracking the evolution of forward and reverse shocks, along with the plasma and photon fields, under full Compton coupling.

Result: Findings show that reverse shocks remain radiation-mediated even at lower optical depths, with last scattering of photons occurring across various radii, not just at a single photospheric surface.

Conclusion: The study enhances understanding of radiation-mediated shocks near the photosphere, revealing their influence on the emitted photon spectra and providing potential observational predictions.
...
TL;DR: This study connects gravitational-wave observations of black hole mergers to the dynamical formation in dense stellar systems, predicting a significant second peak in merger masses.


<details>
  <summary>Details</summary>
Motivation: To explain the observed excess of binary black hole mergers with masses around 35 $M_\odot$ and to explore its implications for the formation mechanisms of black holes.

Method: Analyzed gravitational-wave data alongside cluster population-synthesis models to study the mass distributions of black hole mergers and their formation environments.

Result: Identified a potential second peak near 70 $M_\odot$ in the mass spectrum of black hole mergers, arising from dynamical processes in star clusters and providing constraints on first-generation merger rates.

Conclusion: A confirmed excess at 70 $M_\odot$ could reinforce the dynamical formation theory of black holes and substantiate the presence of a pair-instability mass gap.
...
TL;DR: This paper provides a multimessenger analysis of the neutron-star core's sound-speed profile using data from GW170817 and various NICER mass-radius posteriors, highlighting the presence of intermediate-density stiffening in the equation of state.


<details>
  <summary>Details</summary>
Motivation: To understand the sound-speed profile of neutron-star cores using multimessenger observations, which can give insights into the equation of state at high densities.

Method: The analysis combines gravitational wave data from GW170817 with mass-radius measurements from NICER to infer sound-speed variations in neutron-star matter.

Result: The study finds a significant preference for intermediate-density stiffening in the equation of state, with probabilities indicating stiffening at high densities, especially in more massive neutron stars.

Conclusion: Current data suggest that while massive neutron stars can confirm the onset of stiffening in their equations of state, they do not consistently reach the full peak of stiffening.
...
### [15] [Development of Faster and More Accurate Supernova Localization at Super-Kamiokande](https://arxiv.org/abs/2604.07702)
...
TL;DR: The paper discusses upgrades to the Super-Kamiokande neutrino observatory's SN detection capabilities, enabling faster and more accurate supernova direction reconstruction to improve multi-messenger astronomy.


<details>
  <summary>Details</summary>
Motivation: To enhance the detection and analysis of supernova emissions through neutrino observations for insights into explosive mechanisms and progenitor stars.

Method: Implemented a novel HEALPix-based approach (HP-Fitter) for rapid SN direction calculation and upgraded the ML-Fitter with Gd neutron-capture event data for greater accuracy.

Result: The upgrades enable SNWATCH to issue alerts with accurate directional information in about 90 seconds, enhancing the observatory's real-time monitoring capability.

Conclusion: The improvements effectively increase both the speed and precision of supernova alerts, facilitating better multi-messenger astronomy research.
...
### [16] [Detection of TeV emission during early afterglow from poorly localized GRBs with ground based IACTs](https://arxiv.org/abs/2604.07582)
...
TL;DR: This study explores enhanced strategies for detecting very-high-energy (VHE) emission from gamma-ray bursts (GRBs) using next-generation ground-based telescopes, addressing the challenges posed by poor localization and rapid afterglow decay.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges in detecting VHE emissions from GRBs during their prompt or early afterglow phases, which are hindered by various factors including observational latency and sky localization issues.

Method: The authors simulate a population of GRBs based on extensive data from Fermi/GBM and Swift/XRT, and evaluate detection strategies using next-generation Imaging Atmospheric Cherenkov Telescopes with larger fields of view.

Result: The proposed follow-up strategies significantly increase the detection rate of GRBs, with up to double the detection rate for telescopes like ASTRI and LACT, and four VHE detections per year for CTAO.

Conclusion: Optimizing observational strategies for GRBs can greatly enhance the potential for detecting early afterglow emissions at VHE, paving the way for increased discoveries in this field.
...
### [17] [Stochastic problems in pulsar timing](https://arxiv.org/abs/2604.08373)
*Reginald Christian Bernardo*

Main category: astro-ph.HE

TL;DR: The paper explores Langevin stochastic differential equations to analyze pulsar timing noise and gravitational wave signals, deriving analytical solutions for relevant red noise and GWB dynamics, revealing nonstationarity issues and proposing models for neutron star behavior.


<details>
  <summary>Details</summary>
Motivation: To provide a detailed mathematical framework for understanding pulsar timing noise and gravitational wave backgrounds via Langevin equations, improving analysis methods in pulsar timing array studies.

Method: The study utilizes diffusion theory to derive analytical solutions for means, covariances, and probability distributions of Langevin equations associated with red noise and gravitational wave signals.

Result: The analysis shows inconsistencies in modeling pulsar spin frequency with stationary GWB signals and introduces an enhanced model (overdamped harmonic oscillator) that supports stationary conditions; it also outlines a two-component neutron star model for timing noise which reveals nonstationary behavior.

Conclusion: The findings emphasize the importance of understanding the coexistence of damping and diffusive modes in pulsar timing noise, providing insights that can refine models of neutron star dynamics.
...
TL;DR: The study uses pGMCA to disentangle overlapping X-ray emissions from the Milky Way's Galactic Center, particularly from the SNR Sgr A East and surrounding plasma, enhancing the understanding of their properties and interactions.


<details>
  <summary>Details</summary>
Motivation: The complexity of X-ray emissions in the central Milky Way makes it difficult to derive individual component properties, particularly from the SNR Sgr A East and its confused signals with surrounding plasma.

Method: Poissonian Generalized Morphological Component Analysis (pGMCA) was applied to deep, stacked Chandra ACIS-I observations to separate overlapping X-ray components in the Galactic Center.

Result: The analysis successfully isolated components; it revealed Fe emissions characteristic of mixed-morphology remnants and identified reflection shocks in Sgr A East, with refined measurements showing lower ionization age and higher electron density.

Conclusion: The study enhances the understanding of the SNR Sgr A East's properties and its interactions with surrounding materials, providing a clearer analysis of the complex X-ray environment in the Milky Way's center.
...
### [19] [Resolving the 2024 Outburst of Magnetar 1E 1841-045 from its host Supernova Remnant with EP-FXT](https://arxiv.org/abs/2604.07734)
...
TL;DR: Investigation of the new active episode of magnetar 1E 1841-045 reveals significant changes in pulse profile and spectral characteristics, driven by X-ray bursts and enhanced emission.


<details>
  <summary>Details</summary>
Motivation: To analyze the timing and spectral behavior of 1E 1841-045 during its recent outburst, contributing to the understanding of magnetar dynamics.

Method: Data from the Einstein Probe were utilized to perform timing analysis and energy-resolved pulse profile assessments alongside phase-resolved spectroscopy.

Result: X-ray bursts led to a 20% increase in flux; pulse profiles exhibited multi-peaked structures with phase shifts, and the BB+PL model effectively captured spectral characteristics.

Conclusion: The relationship between outbursts and pulse profile/spectral dynamics is complex, highlighting the significance of high-resolution data in magnetar studies.
...
### [20] [A study of periodic nulling in PSR B0751+32 with FAST](https://arxiv.org/abs/2604.07742)
*W. M. Yan,N. Wang,F. F. Kou,Z. Y. Liu,J. P. Yuan,Z. G. Wen S. N. Sun,M. Y. Zou,Y. R. Wen,X. J. Chen*

Main category: astro-ph.HE

TL;DR: The study analyzes nulling in PSR B0751+32 using FAST, confirming a 35.1% nulling fraction and highlighting significant modulation changes and pulse profile features.


<details>
  <summary>Details</summary>
Motivation: To investigate the characteristics and underlying mechanisms of nulling in PSR B0751+32, contributing to the understanding of pulsar behavior.

Method: A mixture model method was employed to determine the nulling fraction, and three independent methods were used to estimate nulling periodicity, while observing pulse energy evolution.

Result: Confirmed periodic nulling (NF = 35.1% ± 0.6%), significant temporal evolution in modulation, and identified asymmetric pulse profile components without evidence of subpulse drifting.

Conclusion: Results challenge existing models attributing nulling to geometric effects, suggesting a more complex underlying mechanism.
...
### [21] [Does the spectral break in the IceCube diffuse neutrino spectrum originate from AGN evolution?](https://arxiv.org/abs/2604.07824)
*Caijin Xie,Zijian Qiu,Yudong Cui,Sujie Lin,Lili Yang*

Main category: astro-ph.HE

TL;DR: The study proposes that the diffuse neutrino background detected by IceCube is primarily sourced from the evolution of active galactic nuclei (AGNs), suggesting that AGNs have a significant role in cosmic ray acceleration and neutrino production.


<details>
  <summary>Details</summary>
Motivation: To address the mystery of the diffuse neutrino background and its spectral break observed by IceCube, and to explore the role of AGN evolution in this context.

Method: A unified framework is developed that factors in AGN evolution, where cosmic rays accelerated during the active phase diffuse and produce a hadronic afterglow upon central engine shutdown.

Result: The model successfully matches the spectral characteristics of the diffuse neutrino background and the emissions from specific AGN sources, indicating high cosmic ray reservoir efficiency in AGN host galaxies and supporting a lepton-dominated scenario.

Conclusion: This work implies that AGN host galaxies are significant contributors to cosmic rays and neutrinos, highlighting the need for advanced neutrino telescopes for further study.
...
### [22] [The deci-Hz gravitational wave signal from the collapse of rotating very massive stars](https://arxiv.org/abs/2604.07878)
*Bailey Sykes,Jade Powell,Bernhard Müller,Alexander Heger*

Main category: astro-ph.HE

TL;DR: The paper examines gravitational waves from the collapse of a massive rotating star, focusing on detectable signals in the deci-Hz range and their implications for future detection efforts.


<details>
  <summary>Details</summary>
Motivation: To explore the gravitational wave signals from massive stars collapsing in the pair-instability regime, particularly for strong asymmetries that enhance detectability.

Method: Calculating the gravitational wave signal from a 300 $M_	ext{odot}$ star's collapse and analyzing its waveform characteristics for template-based search methods.

Result: The collapse produces strong deci-Hz signals, identifiable out to distances of 200 Mpc, with a projected detection rate of 0.5 events per year for advanced detectors.

Conclusion: These findings enhance the understanding of gravitational waves from massive stars and support the development of deci-Hz detection strategies.
...
### [23] [Oblique Shocks at Supernova Remnants in Massive Star Clusters: A Model for the Cosmic-Ray Knee Observed by LHAASO](https://arxiv.org/abs/2604.07977)
*Luana N. Padilha,Rita C. Anjos*

Main category: astro-ph.HE

TL;DR: This study examines how oblique shocks in Massive Star Clusters accelerate cosmic rays to high energies, proposing a model that correlates observed cosmic-ray spectra with particle acceleration mechanisms.


<details>
  <summary>Details</summary>
Motivation: To understand the role of oblique shocks in accelerating cosmic rays to energies near the knee of the energy spectrum.

Method: A model is developed that factors in supernova and collective wind shocks while emphasizing the shock obliquity angle's impact on particle energy.

Result: The model shows that oblique shocks can enhance acceleration efficiency, enabling cosmic rays to reach multi-PeV energies and aligning with observed all-particle spectra by LHAASO.

Conclusion: This research offers a unified framework connecting particle acceleration in Massive Star Clusters to the observed cosmic-ray knee, predicting associated gamma-ray and neutrino emissions.
...
### [24] [Flux variability of the "10 keV feature" of 4U 0115+63](https://arxiv.org/abs/2604.08051)
...
TL;DR: The study analyzes the ''10 keV feature'' in the spectra of the Be X-ray binary 4U 0115+63, revealing its independence from other spectral components, and suggesting a distinct formation mechanism.


<details>
  <summary>Details</summary>
Motivation: To investigate the spectral behavior of the ''10 keV feature'' in the context of cyclotron resonant scattering features and quasiperiodic oscillations.

Method: Flux-resolved analysis using two NuSTAR observations during a 2015 outburst, comparing spectral components across different fluxes.

Result: A distinct change in spectral shape related to the ''10 keV feature'' was observed, indicating it behaves independently of harmonic CRSFs, with a constant centroid energy.

Conclusion: The findings support the notion that the ''10 keV feature'' has a different formation mechanism than the continuum, though originating from the same environment.
...
### [25] [How many VHE gamma-ray binaries with young pulsars can be observed?](https://arxiv.org/abs/2604.08175)
*A. M. Bykov,A. G. Kuranov,A. E. Petrov,K. A. Postnov*

Main category: astro-ph.HE

TL;DR: The study analyzes Galactic gamma-ray binaries as potential cosmic ray accelerators by simulating interactions between massive stars and their relativistic companions, focusing on particle acceleration and emission patterns.


<details>
  <summary>Details</summary>
Motivation: Ever-increasing gamma-ray observatory sensitivity has led to the emergence of Galactic gamma-ray binaries, prompting the need to understand their role as cosmic ray accelerators.

Method: Population synthesis calculations are performed on Galactic binaries with massive stars and pulsars, considering various factors like binary eccentricities, orbital periods, and pulsar braking energy losses to estimate the number of gamma-ray binaries.

Result: The study finds that strong magnetic fields and conditions in the interaction zone can accelerate particles up to PeV energies, with the anisotropic structure influencing gamma-ray emission and absorption.

Conclusion: The anisotropic radiation patterns from pulsar outflows could significantly impact the detection and interpretation of gamma-ray loud systems.
...
### [26] [The Stack Search Tests on FAST Data: Discovery of Six Faint Isolated Millisecond Pulsars in NGC 6517 and NGC 7078 (M15)](https://arxiv.org/abs/2604.08268)
...
TL;DR: This study reports the discovery of six new faint millisecond pulsars in two globular clusters using a novel stacking technique to improve detection sensitivity.


<details>
  <summary>Details</summary>
Motivation: To identify faint millisecond pulsars (MSPs) in globular clusters, particularly focusing on enhancing sensitivity for detection.

Method: Utilized the Five-hundred-meter Aperture Spherical radio Telescope (FAST) and a stacking technique on power spectra from multiple observations to boost the signal-to-noise ratio.

Result: Discovered six new MSPs in NGC 6517 and M15, with spin periods and dispersion measures varying across the pulsars. Phase-coherent timing was achieved for one pulsar, while others remain undetected by standard methods.

Conclusion: The findings indicate that all six MSPs are isolated, aligning with expected pulsar populations in globular clusters, and emphasize the utility of the stacking technique in enhancing detection of faint pulsar signals.
...
TL;DR: This study investigates how plasma instabilities affect the spectrum of TeV gamma-ray cascades, providing constraints on the intergalactic magnetic field (IGMF) using observational data from the blazar 1ES 0229+200.


<details>
  <summary>Details</summary>
Motivation: The research aims to understand the role of plasma instabilities in electromagnetic cascades induced by TeV gamma rays in the intergalactic medium, and to constrain the weak IGMF in cosmic voids.

Method: A parameterized model for plasma instabilities is introduced within the Monte Carlo framework CRPropa, analyzing extended-emission observations of the blazar 1ES 0229+200 to derive IGMF constraints under varying field-of-view angles.

Result: The study finds that plasma instabilities with a characteristic length scale of approximately 100 kpc can match the observed photon spectrum and imply a weak IGMF strength around 10^{-17} G.

Conclusion: The presence of plasma instabilities significantly influences the emission characteristics of TeV gamma-ray cascades, providing crucial insights into cosmic void magnetic fields.
...
TL;DR: The paper explores shadow and polarization images of Kerr-Sen black holes under different accretion disk models, highlighting effects of black hole charge, spin, and observer angle on image characteristics.


<details>
  <summary>Details</summary>
Motivation: To understand how the parameters of Kerr-Sen black holes impact observable features such as shadows and polarization images.

Method: Utilizes radiative transfer theory with two accretion models—RIAF and BAAF—to analyze photon rings and brightness asymmetries based on varying parameters.

Result: Increased black hole charge leads to smaller photon rings and darker central regions, with frame dragging affecting brightness asymmetry. BAAF model shows narrower higher order images compared to RIAF.

Conclusion: Polarization images are influenced by gravitational lensing and frame dragging, with intensity around the photon ring varying significantly with charge.
...
### [29] [Multiplicity dependence of prompt and non-prompt J/$ψ$ production at midrapidity in pp collisions at $\sqrt{s} = 13$ TeV](https://arxiv.org/abs/2604.07968)
*ALICE Collaboration*

Main category: hep-ex

TL;DR: The paper measures prompt and non-prompt J/$ψ$ yields as a function of charged-particle multiplicity in pp collisions at 13 TeV, finding a stronger-than-linear increase in self-normalized yields across azimuthal regions.


<details>
  <summary>Details</summary>
Motivation: To investigate J/$ψ$ production mechanisms in high-energy pp collisions relative to charged-particle multiplicity.

Method: Yields measured via the dielectron decay channel and normalized by average values in inelastic collisions, with multiplicity divided into three azimuthal regions.

Result: A stronger-than-linear increase in self-normalized yields for both prompt and non-prompt J/$ψ$, with similar trends observed in the toward region, while weaker in transverse and away regions.

Conclusion: Prompt and non-prompt J/$ψ$ production shows multiplicity-dependent behavior, highlighting differences in emission angles.
...
### [30] [Recent Neutrino Oscillation and Cross-Section Results from the T2K Experiment](https://arxiv.org/abs/2604.08259)
*Nick Latham*

Main category: hep-ex

TL;DR: The T2K experiment advances the search for leptonic charge-parity violation with new oscillation results and cross-section measurements, particularly in the T2K-II phase involving a gadolinium-loaded detector.


<details>
  <summary>Details</summary>
Motivation: To explore leptonic charge-parity violation and enhance precision in neutrino oscillation measurements.

Method: Utilization of a near detector complex for high-statistics neutrino-nucleus interaction cross-section measurements and the incorporation of a gadolinium-loaded far detector.

Result: Latest T2K oscillation results and world-first measurements of rare neutrino interaction channels are presented.

Conclusion: The results emphasize the crucial role of interaction modeling and oscillation analysis in addressing charge-parity violation in the T2K-II era.
...
### [31] [Search for the lepton-flavour violating decays $B^+ \to π^+ μ^\pm e^\mp$](https://arxiv.org/abs/2604.08396)
...
TL;DR: This paper reports the first search for lepton-flavour violating decays $B^+ \to π^+ μ^{±} e^{∓}$ using LHC data, setting an upper limit on the branching fraction.


<details>
  <summary>Details</summary>
Motivation: To investigate lepton-flavour violation in $B$ meson decays, which may indicate physics beyond the Standard Model.

Method: Analysis of data from LHCb experiment from 2011-2018, totaling 9 fb$^{-1}$ of integrated luminosity, to search for specific decay signatures.

Result: No significant signal observed; upper limit on branching fraction set at $	ext{B}(B^+ \to π^+ μ^{±} e^{∓}) < 1.8 \times 10^{-9}$ (90% C.L.), two orders of magnitude more restrictive than current values.

Conclusion: This work provides the first constraint on lepton-flavour violating $b \to d$ transitions and the most stringent limits on $b \to d μ^{±} e^{∓}$ processes, along with constraints on new physics scenarios.
...
### [32] [Finite Volume Effects on Transverse Momentum Spectra at LHC and RHIC Using a Blast-Wave Model with Planck Transformed Temperatures](https://arxiv.org/abs/2604.07410)
*A. S. Parvan,A. A. Aparin,E. V. Nedorezov*

Main category: hep-ph

TL;DR: This study examines finite volume effects on charged pions in heavy-ion collisions, using a Lorentz-covariant model to improve accuracy in parameter extraction compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of infinite volume models in accurately describing the dynamics of heavy-ion collisions at various energy scales.

Method: A finite volume Boltzmann-Gibbs blast-wave model with Planck transformations is used to derive laboratory frame parameters from the local rest frame, comparing it with the conventional infinite volume model.

Result: The finite volume model yields realistic temperatures and fire cylinder volumes, maintaining consistency with relativistic thermodynamics, while the infinite volume model results in unphysical values.

Conclusion: Accurate modeling of finite size and Lorentz transformations is crucial for reliable extraction of freeze-out parameters in heavy-ion collisions.
...
### [33] [Probing Majoron Dark Matter with Gravitational Wave Detectors](https://arxiv.org/abs/2604.08193)
*Ippei Obata,Tsutomu T. Yanagida*

Main category: hep-ph

TL;DR: The paper explores the detection of Majoron dark matter using linear optical cavities in gravitational wave detectors, focusing on its photonic interactions and potential measurement capabilities.


<details>
  <summary>Details</summary>
Motivation: To investigate Majoron dark matter as a viable candidate for dark matter in the universe and explore its interaction with photons.

Method: The study involves analyzing Majoron dark matter's coupling to photons through a QED anomaly, leading to detectable oscillatory photon birefringence in laser interferometers.

Result: The birefringence induced by dark matter has potential measurements within the sensitivity range of existing and future optical interferometers, like Advanced LIGO and KAGRA.

Conclusion: Ground-based gravitational wave detectors could effectively probe Majoron dark matter, enhancing our understanding of its properties.
...
### [34] [Scalars at the Cosmological Collider: Full Shapes of Tree Diagrams and Bispectrum Searches using Planck Data](https://arxiv.org/abs/2604.07434)
*Soubhik Kumar,Qianshu Lu,Zhong-Zhi Xianyu,Yisong Zhang*

Main category: hep-ph

TL;DR: The paper investigates primordial non-Gaussianity (NG) in the context of high-energy particle interactions at the Cosmological Collider, analyzing contributions to the bispectrum from massive particle decays and their implications when examined against Planck data.


<details>
  <summary>Details</summary>
Motivation: To explore the potential signatures of primordial non-Gaussianity due to massive particle interactions during inflation, particularly focusing on high-energy particle physics and its cosmological effects.

Method: The authors derive shape functions for the bispectrum based on three process classes ('single exchange', 'double exchange', 'triple exchange') and perform data analysis against the Planck dataset.

Result: The analysis did not find evidence for primordial non-Gaussianity but identified a slight deviation indicating evidence for non-zero NG in a specific parameter space with global 1.5σ significance.

Conclusion: The findings suggest possibilities for extending minimal models to explain observed phenomena, with particular insight into the role of scalar chemical potential in enhancing non-Gaussianity.
...
### [35] [Gauged Q-balls in flat potentials](https://arxiv.org/abs/2604.07454)
*Julian Heeck,Yu Zhi*

Main category: hep-ph

TL;DR: This paper investigates gauged Q-balls in flat potentials, contrasting them with Coleman's Q-balls while providing analytic approximations and numerical comparisons.


<details>
  <summary>Details</summary>
Motivation: To explore the properties and behavior of gauged Q-balls in flat potentials, particularly in the context of supersymmetric models.

Method: The authors analyze gauged Q-balls using analytic approximations and numerical solutions, comparing these to global Q-balls in flat potentials and studying Proca Q-balls with massive gauge bosons.

Result: The study finds that despite the qualitative differences in global Q-balls in flat potentials, the gauged versions share remarkable similarities, and Proca Q-balls act as a bridge between global and gauged cases.

Conclusion: Gauged Q-balls in flat potentials exhibit behaviors akin to their global counterparts, enhancing understanding of these systems in theoretical frameworks.
...
### [36] [GOOFy fermions](https://arxiv.org/abs/2604.07458)
*P. M. Ferreira*

Main category: hep-ph

TL;DR: This paper explores newly discovered symmetries in two Higgs doublet models, establishing fermion field transformations and identifying regions of parameter space invariant under renormalization.


<details>
  <summary>Details</summary>
Motivation: To investigate a newly found class of symmetries in two Higgs doublet models and their implications for fermion interactions.

Method: The authors derive transformations for fermion fields that respect the newly established symmetries in the context of the two Higgs doublet model framework.

Result: New Yukawa matrix textures that maintain symmetry up to two-loops are identified, alongside previously unexplored regions of parameter space that are invariant under full perturbative renormalization, including aspects of scalar-fermion interactions.

Conclusion: The discovery of these symmetries and transformations could enhance the understanding of the dynamics in two Higgs doublet models and their implications for particle physics.
...
### [37] [Axion-like Particles and Lepton Flavor Violation in Muonic Atoms](https://arxiv.org/abs/2604.07481)
*Girish Kumar,Alexey A. Petrov*

Main category: hep-ph

TL;DR: The Mu2e experiment investigates lepton-flavor violation in a simplified ALP framework, computing the transition rate for the process $μ^- e^- 	o e^- e^-$ with stringent constraints affecting its observability.


<details>
  <summary>Details</summary>
Motivation: To explore lepton-flavor-violating processes in muonic atoms and assess the impact of axion-like particles on these transitions.

Method: Calculated the ALP-mediated transition rate, assessed how parameters affect branching ratios, and compared findings against existing experimental constraints.

Result: The branching ratio for $μ^- e^- 	o e^- e^-$ drops to at most $	ext{O}(10^{-20})$ after applying laboratory constraints, with a more suppressed resonant region for certain masses.

Conclusion: Existing bounds, particularly from $Δa_e$ and other muon decay processes, severely limit the observable rates, indicating that the upcoming Mu3e experiment will be critical in probing meaningful parameter space.
...
### [38] [Excitation function for global Λpolarization in relativistic heavy ion collisions with the Core Corona model](https://arxiv.org/abs/2604.07501)
*Alejandro Ayala,José Jorge Medina Serna,Isabel Domínguez,María Elena Tejeda-Yeomans*

Main category: hep-ph

TL;DR: The study computes the global polarization of $Λ$ in semicentral heavy-ion collisions using a Core-Corona model, revealing significant contributions from the corona region and predicting a robust maximum in excitation function.


<details>
  <summary>Details</summary>
Motivation: To better understand the behavior of $Λ$ polarization in heavy-ion collisions and its dependence on various physical parameters, particularly at low energies.

Method: The Core-Corona framework models the interaction region with intrinsic polarization derived from a field-theoretical approach. Vortical motion is incorporated via an effective fermion propagator, using gluons and $σ$-mesons as mediators in the core and corona, respectively.

Result: The model successfully describes the excitation function across experimental data, indicating that the corona contributes dominantly at the energies studied, particularly due to larger lifetime and volume effects at lower energies.

Conclusion: The study predicts a stable maximum for $Λ$ polarization near $	ext{s}_{NN} 	ext{= 3 GeV}$, which holds under variations in freeze-out conditions and accounts for subthreshold production.
...
### [39] [Forward trijet production in proton-nucleus collisions: gluon initiated channel](https://arxiv.org/abs/2604.07509)
*Paul Caucal,Marcos Guerrero Morales,Farid Salazar*

Main category: hep-ph

TL;DR: This paper reports on a calculation of forward trijet production in proton-nucleus collisions, using the Color Glass Condensate effective theory. It highlights contributions from gluon and quark interactions and provides novel insights into the four-gluon vertex structure.


<details>
  <summary>Details</summary>
Motivation: To investigate forward trijet production in proton-nucleus collisions and understand the dynamics in the gluon initiated channel using effective field theories.

Method: Calculations are performed in the Color Glass Condensate framework under the dilute-dense approximation, utilizing covariant perturbation theory to analyze amplitudes as a combination of regular and instantaneous contributions.

Result: The study identifies contributions from $q\bar{q}g$ and $ggg$ final states, revealing that the four-gluon vertex has an instantaneous contribution structure. It also highlights the role of rapidity and collinear divergences in the evolution of parton distribution functions.

Conclusion: The results confirm the dilute-dense formalism at one-loop order and lay the groundwork for next-to-leading order calculations in dijet/dihadron production.
...
TL;DR: This paper presents a framework for global statistical fits in high-energy physics using Machine Learning (ML) methods.


<details>
  <summary>Details</summary>
Motivation: To address the high computational costs of traditional statistical methods in physics model fitting.

Method: Utilizes Boosted Decision Trees for likelihood approximation, ML workflows for data generation and hyperparameter optimization, and MCMC for posterior distribution sampling.

Result: Demonstrates efficient exploration of the parameter space for Axion-Like Particles (ALPs) related to a specific anomaly at Belle II.

Conclusion: The developed ML framework enables robust and efficient statistical analysis in high-energy physics experiments.
...
TL;DR: This paper presents a method for calculating the production fraction ratio $R^{	ext{±}0}$ using $ar B	o D^{(*)}ightarrow 	ext{decays}$, reveals a value indicating isospin violation, and corrects previous branching fraction measures, addressing related puzzles.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve understanding of the production fraction ratios in B-meson decays, particularly focusing on isospin violation effects in the context of experimental analyses.

Method: Utilizing data from $ar B	o D^{(*)}	ext{and other decays}$, the authors derive correlations for branching fractions, aiming for precision and addressing prior inconsistencies and biases in existing literature.

Result: The most precise measurement of $R^{	ext{±}0}$ at $1.062(19)$ is obtained, indicating a substantial deviation from unity, coupled with larger branching fractions than previously recorded.

Conclusion: The findings highlight isospin violation in $Υ(4S)$ decays, emphasizing its importance in future experimental and phenomenological studies, and contribute to resolving the $V_{cb}$ puzzle.
...
### [42] [Transverse energy-momentum tensor distributions in polarized nucleons](https://arxiv.org/abs/2604.07616)
*Ho-Yeon Won,Cédric Lorcé*

Main category: hep-ph

TL;DR: The study analyzes the energy-momentum tensor distributions in polarized nucleons using quantum phase-space formalism, focusing on transverse components and multipole structures, demonstrating consistency with established light-front distributions in the infinite-momentum frame.


<details>
  <summary>Details</summary>
Motivation: To enhance the understanding of energy-momentum distributions in polarized nucleons and the implications of their transverse components.

Method: Utilizing quantum phase-space formalism to investigate the spatial distributions of the energy-momentum tensor, particularly focusing on transverse indices and their multipole structure.

Result: Derivation of standard light-front distributions from the formalism, while providing insights into the 'bad' components and their structural origins.

Conclusion: The study successfully bridges quantum phase-space formalism with established distributions, offering deeper insights into the energy-momentum structure in polarized nucleons.
...
TL;DR: This study examines how memory effects, modeled via a generalized Langevin equation, impact heavy quark dynamics using time-correlated thermal noise.


<details>
  <summary>Details</summary>
Motivation: Understanding the dynamics of heavy quarks in quark-gluon plasma is crucial for insights into high-energy nuclear physics.

Method: The study employs a generalized Langevin equation incorporating time-correlated thermal noise generated through a Caputo fractional derivative.

Result: The analysis reveals that memory effects significantly affect momentum correlations, average squared momentum, displacement, and kinetic energy metrics of heavy quarks.

Conclusion: The findings demonstrate that time correlated thermal noise plays a substantial role in the dynamics of heavy quarks within quark-gluon plasma.
...
TL;DR: The study investigates the potential detection of ultralight dark matter (ULDM) by examining its influence on S-star orbits near the supermassive black hole in the Galactic Center.


<details>
  <summary>Details</summary>
Motivation: The dense environment of the Galactic Center serves as an ideal setting to explore the properties of ultralight dark matter, particularly through its gravitational effects.

Method: The authors analyze the orbital dynamics of S-stars under different coupling scenarios of a scalar ULDM field, focusing on linear and quadratic couplings and two ULDM structures: scalar gravitational atom and spherical soliton.

Result: The quadratic coupling leads to significant alterations in orbital dynamics, enabling the authors to derive stringent constraints on ULDM mass and coupling constants utilizing observed periastron precession rates of the S2 star.

Conclusion: The obtained limits on the mass ratio of ULDM to the black hole and on the quadratic coupling constant exceed existing constraints, offering new insights into ULDM properties in the mass range of $10^{-20} 	ext{eV} - 10^{-18} 	ext{eV}.
...
### [45] [Fermion Multiplicities at the GUT Scale: A Statistical Study of Unification and Proton Decay](https://arxiv.org/abs/2604.08237)
*Akifumi Chitose,Ko Hirooka,Masahiro Ibe,Satoshi Shirai*

Main category: hep-ph

TL;DR: The paper explores how multiple vector-like fermions in SU(5) GUT influence gauge coupling unification and proton decay characteristics, leading to longer proton lifetimes and relaxing Yukawa relations.


<details>
  <summary>Details</summary>
Motivation: To investigate the role of multiple vector-like fermions in enhancing gauge coupling unification and addressing issues in SU(5) GUTs, such as proton decay and flavor structures.

Method: The study employs theoretical analysis to examine the threshold effects of additional fermions on gauge coupling matching and their implications for proton decay and Yukawa relations.

Result: Findings indicate that extra fermions enable a single unified gauge coupling at an elevated unification scale and suppress nucleon decay operator coefficients, which increase proton lifetimes.

Conclusion: The framework of multi-fermion SU(5) GUTs offers a compelling approach to reconciling gauge unification and flavor structures while ensuring proton stability, warranting further experimental investigation.
...
### [46] [Kinetic and canonical momentum broadening in the Glasma](https://arxiv.org/abs/2604.08520)
*Dana Avramescu,Carlos Lamas,Tuomas Lappi,Meijian Li,Carlos A. Salgado*

Main category: hep-ph

TL;DR: The paper presents a quantum framework for real-time particle dynamics in the Glasma phase of heavy-ion collisions, emphasizing gauge invariance in momentum definitions.


<details>
  <summary>Details</summary>
Motivation: To understand particle evolution in heavy-ion collisions and derive accurate equations of motion considering gauge invariance.

Method: Establish a correspondence between Wong's classical equations and Heisenberg motion equations, leading to kinetic and canonical momentum equations in a non-Abelian background.

Result: The kinetic momentum broadening shows significant contributions from transverse fields, even under the eikonal approximation; a transverse Coulomb gauge minimizes numerical errors in simulations.

Conclusion: The developed framework enhances precision for future quantum implementations in the study of particle dynamics in heavy-ion collisions.
...
### [47] [Borns Rule from Reversible Evolution and Irreversible Outcomes](https://arxiv.org/abs/2604.07418)
*Oskar Axelsson*

Main category: quant-ph

TL;DR: This paper derives the quadratic measure (Born rule) from the interplay between linear reversible evolution and the formation of persistent records in physical processes.


<details>
  <summary>Details</summary>
Motivation: To illustrate that the quadratic measure can arise from fundamental structural features of physical processes rather than needing to be assumed.

Method: By analyzing the compatibility of linear reversible evolution and multiplicative outcome weight composition, the paper shows how these elements interact and lead to constraints on weight assignments.

Result: The analysis reveals that the only weight assignment consistent with the two regimes is quadratic, thus deriving the Born rule.

Conclusion: The Born rule can be uniquely derived from the combination of reversible evolution and record formation, without necessitating prior probabilistic assumptions.
...
### [48] [Comment on "Quantum theory based on real numbers cannot be experimentally falsified": On the compatibility of physical principles with information theory for fermions](https://arxiv.org/abs/2604.07425)
*Fatemeh Moradi Kalarde,Xiangling Xu,Marc-Olivier Renou*

Main category: quant-ph

TL;DR: The paper critiques a postulate from arXiv:2603.19208 for formulating quantum theory, arguing it fails within Fermionic Information Theory, which is essential for understanding information from identical fermions. This calls for a reevaluation of foundational principles in quantum theories.


<details>
  <summary>Details</summary>
Motivation: To critically assess a proposed physical postulate that aims to determine suitable formulations of quantum theory, specifically in the context of Fermionic Information Theory.

Method: The authors analyze the adherence of the selected postulate to Fermionic Information Theory and illustrate its shortcomings when applied in this scenario.

Result: The postulate fails to satisfy the requirements of Fermionic Information Theory, indicating it isn't universally applicable as initially claimed.

Conclusion: The findings emphasize the necessity of scrutinizing fundamental principles in quantum theories through the lens of Fermionic Information Theory, suggesting avenues for further research.
...
### [49] [Observation of genuine $2+1$D string dynamics in a U$(1)$ lattice gauge theory with a tunable plaquette term on a trapped-ion quantum computer](https://arxiv.org/abs/2604.07436)
*Rohan Joshi,Yizhuo Tian,Kevin Hemery,N. S. Srivatsa,Jesse J. Osborne,Henrik Dreyer,Enrico Rinaldi,Jad C. Halimeh*

Main category: quant-ph

TL;DR: This paper reports on a quantum simulation of string dynamics in 2+1D physics using a quantum computer, highlighting the importance of a plaquette term for genuine dynamics and demonstrating significant results in matter creation and dynamics propagation.


<details>
  <summary>Details</summary>
Motivation: To explore dynamical phenomena in 2+1D high-energy physics that are unattainable in lower dimensions, particularly focusing on string dynamics related to hadronization.

Method: Implementation of a U(1) quantum link model on a quantinuum quantum computer, utilizing a 5x4 lattice and 51 qubits, with a shallow circuit design and a specific plaquette term to enable gauge field dynamics.

Result: The simulation reveals genuine 2+1D dynamics including string propagation and annihilation of string segments with electron-positron pair production, evidencing that the inclusion of the plaquette term is crucial for observable dynamics across the lattice.

Conclusion: The findings underscore the significance of dynamical gauge fields in 2D and suggest a pathway for simulating photon-like propagation in quantum gauge theories using quantum computers.
...
### [50] [Ten-Second Electron-Spin Coherence in Isotopically Engineered Diamond](https://arxiv.org/abs/2604.07439)
...
TL;DR: This study enhances spin coherence times and optical transitions in NV centers in diamond, advancing quantum technology.


<details>
  <summary>Details</summary>
Motivation: To improve spin-photon entanglement in quantum networks by achieving long spin-coherence times and coherent optical transitions.

Method: Utilized isotopically engineered diamond with controlled carbon and nitrogen concentrations, implemented a real-time feedforward scheme and tailored decoupling sequences.

Result: Achieved record defect-electron-spin coherence times of T2 = 6.8 ms and T2^DD = 11.2 s, and observed optical transitions with a linewidth of 16.9 MHz.

Conclusion: Findings open new research directions for diamond impurities and enhance spin-qubit control for quantum applications.
...
### [51] [Locked Subharmonic Oscillations in the Entanglement Spectrum of a Periodically Driven Topological Chain](https://arxiv.org/abs/2604.07442)
*Rishabh Jha*

Main category: quant-ph

TL;DR: This study reveals that subharmonic responses in periodically driven quantum systems can be detected through the entanglement spectrum in a free-fermion model.


<details>
  <summary>Details</summary>
Motivation: To investigate the subharmonic response in periodically driven quantum systems, focusing on the entanglement spectrum instead of conventional observables.

Method: The authors study a driven Su-Schrieffer-Heeger chain with symmetry-protected edge modes and analyze subsystem correlation matrices and entanglement levels under coherent superpositions of edge modes.

Result: The entanglement spectrum indicates a robust period-doubling response; stroboscopic correlation structures alternate, while edge densities remain unchanged due to symmetry.

Conclusion: Entanglement spectroscopy is a useful tool for probing Floquet topological coherence, with zero-$C0$ Floquet topology being necessary alongside coherent preparation.
...
### [52] [When is randomization advantageous in quantum simulation?](https://arxiv.org/abs/2604.07448)
*Francesco Paganelli,Michele Grossi,Andrea Giachero,Thomas E. O'Brien,Oriel Kiss*

Main category: quant-ph

TL;DR: The paper explores Hamiltonian simulation benefits from randomization, introducing a sparse-QSVT method and analyzing error propagation. It finds that randomized methods significantly reduce gate counts for complex Hamiltonians, but only within certain precision limits, after which deterministic methods are preferred.


<details>
  <summary>Details</summary>
Motivation: To understand when randomization improves Hamiltonian simulation efficiency, particularly in controlled Hamiltonian ensembles.

Method: A sparse-QSVT construction utilizing composite stochastic decompositions, analyzing stochastic and approximation error propagation through block-encoding and QSVT procedures.

Result: For complex Hamiltonians with inhomogeneous coefficients, randomization can reduce gate counts by up to an order of magnitude, but only in moderate-precision regimes.

Conclusion: While randomization has advantages for certain Hamiltonians, deterministic methods ultimately become more efficient as precision increases, especially given unmodeled structures in real-world systems.
...
### [53] [Operational criteria for quantum advantage in latency-constrained nonlocal games](https://arxiv.org/abs/2604.07451)
*Changhao Li,Seigo Kikura,Akihisa Goban,Hayata Yamasaki,Shinichi Sunami*

Main category: quant-ph

TL;DR: This paper establishes a framework to analyze quantum advantages in latency-constrained tacit coordination (LCTC), accounting for real-world limitations and proposing practical implementations for enhanced decision-making.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the inadequacies in existing analyses of quantum-classical performance gaps in time-sensitive coordination scenarios by considering realistic operational constraints.

Method: The authors develop a framework incorporating finite-duration operations, limited entanglement generation rates, and generalized utility structures, leveraging statistical certification methods from nonlocal games.

Result: They propose time-multiplexed operations for trapped-atom quantum nodes that can deliver a sufficient stream of entangled qubits, achieving microsecond decision latencies and high decision rates suitable for dynamic environments.

Conclusion: This work connects theoretical quantum advantages with practical implementations, enabling robust decision-making capabilities in complex, real-world environments.
...
### [54] [Quantum Simulation of Collective Neutrino Oscillations using Dicke States](https://arxiv.org/abs/2604.07452)
*Katarina Bleau,Nikolina Ilic,Joachim Kopp,Ushak Rahaman,Xin Yue Yu*

Main category: quant-ph

TL;DR: New algorithms for simulating dense neutrino gases on quantum computers utilize Dicke states and $su(2)$ spin algebra for improved efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the simulation of entangled neutrino flavour states in dense gases, such as those found in supernovae, inefficiencies in existing quantum simulations highlight the need for better methods.

Method: Introduction of qubit-efficient algorithms leveraging Dicke states and the $su(2)$ spin algebra to enhance simulation efficacy.

Result: The new algorithms demonstrate superior performance in simulations when tested on both classical and quantum hardware.

Conclusion: The proposed algorithms show promise for efficient and effective simulations of complex quantum systems like neutrino gases.
...
### [55] [Optimal Quantum State Testing Even with Limited Entanglement](https://arxiv.org/abs/2604.07460)
*Chirag Wadhwa,Sitan Chen*

Main category: quant-ph

TL;DR: This paper presents an efficient method for certifying quantum states using fewer copies, improving on traditional fully entangled measurement protocols by focusing on a manageable number of copies at a time.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in certifying high-dimensional quantum states efficiently while managing measurement complexity, which is intractable with the usual methods that require joint measurements on many copies.

Method: The authors introduce a new approach that allows for measurements on only a limited number of copies at a time, achieving a tradeoff that optimally maintains the certification rates.

Result: They establish a smooth copy complexity upper bound for state certification, showcasing a significant improvement for high-precision scenarios, and develop related algorithms for mixedness testing and purity estimation.

Conclusion: The paper concludes that optimal certification rates can be achieved using measurements on a limited number of copies, and that substantial reductions in the need for fully entangled measurements can enhance efficiency in practice.
...
### [56] [On Lorentzian symmetries of quantum information](https://arxiv.org/abs/2604.07471)
*James Fullwood,Vlatko Vedral,Edgar Guzmán-González*

Main category: quant-ph

TL;DR: This paper shows that Lorentz symmetries in quantum information can arise from an information-theoretic principle, demonstrating a link between quantum mechanics and relativistic spacetime structure.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the non-Lorentz invariance of von Neumann entropy and explore how quantum information principles can lead to Lorentzian symmetries in a pre-spacetime context.

Method: It derives the action of the restricted Lorentz group on a single qubit's internal degrees of freedom based on the principle of preservation of linear entropy, demonstrating general invariance in quantum mutual information and related measures.

Result: It establishes that linear entropy is Lorentz invariant and shows that correlation functions for qubits yield the Minkowski metric, reflecting the emergent nature of relativistic spacetime from quantum information properties.

Conclusion: The findings support the 'It from Qubit' paradigm, indicating that the structure of relativistic spacetime can be directly linked to the intrinsic properties of quantum information.
...
### [57] [Fermionic entanglement and quantum correlation measures in molecules](https://arxiv.org/abs/2604.07633)
*J. Garcia,J. A. Cianciulli,R. Rossignoli*

Main category: quant-ph

TL;DR: The paper investigates fermionic entanglement in water molecules using full configuration interaction, focusing on various entanglement measures over internuclear distances and states.


<details>
  <summary>Details</summary>
Motivation: To characterize electronic eigenstates of the water molecule via entanglement measures.

Method: Analysis of fermionic entanglement through Schmidt decomposition and reduced density matrices, introducing new entanglement measures.

Result: Identification of total spin-up/down entanglement, one- and two-body entanglement metrics, and new measures like mutual information and two-body negativities.

Conclusion: Insights into dissociation limits and the nature of ground and thermal states at low temperatures are achieved.
...
### [58] [Exponential quantum advantage in processing massive classical data](https://arxiv.org/abs/2604.07639)
*Haimeng Zhao,Alexander Zlokapa,Hartmut Neven,Ryan Babbush,John Preskill,Jarrod R. McClean,Hsin-Yuan Huang*

Main category: quant-ph

TL;DR: A small quantum computer can achieve significant advantages over classical systems in processing large-scale data for machine learning tasks.


<details>
  <summary>Details</summary>
Motivation: To demonstrate quantum advantage in classical data processing and machine learning, addressing a fundamental open problem in the field.

Method: A quantum algorithm called quantum oracle sketching processes classical data samples on-the-fly, allowing for efficient classification and dimension reduction.

Result: The quantum machine shows a four to six orders of magnitude reduction in required size and sample complexity compared to classical counterparts, validated in real-world applications.

Conclusion: These findings indicate a broad domain of quantum advantage in machine learning on classical data, providing a non-trivial test of quantum mechanics' correctness.
...
### [59] [A Thermodynamic SU(1,1) Witness Framework for Double-Quantum NMR Signals in Neural Tissue](https://arxiv.org/abs/2604.07641)
*Christian Kerskens*

Main category: quant-ph

TL;DR: This paper develops a thermodynamic framework to analyze classical fluctuations in non-compact spin systems, specifically addressing double-quantum observables.


<details>
  <summary>Details</summary>
Motivation: To extend existing entanglement criteria from compact to non-compact spin algebras and handle the challenges posed by DQ observables in SU(1,1) structures.

Method: A thermodynamic witness framework is developed that imposes constraints using detailed-balance conditions and sequence-transfer limits, analyzing quantum dynamics of spin-bath interactions.

Result: Spontaneous transient pair correlations are shown to be limited to around amplitude of \(10^{-9}\) and classical coherent sequence amplification bounded to \(\mathcal{O}(10^{-2})\).

Conclusion: A theoretically derived bounding framework enables the classification of macroscopic DQ anomalies as classically inexplicable under conditions of structural stability.
...
### [60] [Control-centric quantum noise spectroscopy of time-ordered polyspectra](https://arxiv.org/abs/2604.07682)
*Kaiah Steven,Elliot Coupe,Qi Yu,Gerardo A. Paz-Silva*

Main category: quant-ph

TL;DR: This paper presents a control-centric approach to non-parametric quantum noise spectroscopy, enhancing the estimation of environmental noise in quantum systems under realistic experimental constraints.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve quantum control and decoherence suppression in noise-sensitive quantum computing and sensing applications by accurately characterizing environmental noise.

Method: The authors adopt a control-centric viewpoint, reformulating the noise spectroscopy problem to focus on time-ordered polyspectra and adjust control filter functions to eliminate issues associated with time-ordering.

Result: The method allows for the generalization of frequency-comb quantum noise spectroscopy to various control scenarios, achieving effective noise estimation in challenging environments without introducing unnecessary control symmetries.

Conclusion: The proposed approach facilitates the targeted reconstruction of time-ordered polyspectra, demonstrating effectiveness in classical Gaussian and non-Gaussian quantum environments through simulations.
...
### [61] [Trotterization with Many-body Coulomb Interactions: Convergence for General Initial Conditions and State-Dependent Improvements](https://arxiv.org/abs/2604.07704)
*Di Fang,Xiaoxu Wu*

Main category: quant-ph

TL;DR: This paper establishes rigorous error bounds for Trotter formulas applied to many-body quantum systems with Coulomb interactions, showing a sharp convergence rate that remains efficient with respect to particle number.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges in simulating many-body quantum systems with Coulomb interactions, which have unique complexities such as unbounded Hamiltonians and exponentially growing Hilbert space dimensions.

Method: The authors derive rigorous error bounds for second-order Trotter formulas in many-body quantum systems, demonstrating a $1/4$ convergence rate with polynomial error dependence on the particle number and identifying conditions for improved convergence rates.

Result: The second-order Trotter method achieves a convergence rate of $1/4$ in general conditions, maintaining quantum efficiency without regularization of the Coulomb singularity, and identifies improved convergence under specific initial state conditions.

Conclusion: The results have significant implications for simulating hydrogenic systems and are consistent with earlier numerical findings, suggesting practical applicability in quantum computing.
...
### [62] [Critical Entanglement Dynamics at Dynamical Quantum Phase Transitions](https://arxiv.org/abs/2604.07714)
*Kaiyuan Cao,Mingzhi Li,Xiang-Ping Jiang,Shu Chen,Jian Wang*

Main category: quant-ph

TL;DR: This paper investigates momentum-space entanglement entropy at dynamical quantum phase transitions (DQPTs), revealing critical structures and their dependence on system dimensionality and bipartition selection.


<details>
  <summary>Details</summary>
Motivation: To explore how momentum-space entanglement entropy behaves during dynamical quantum phase transitions in translationally invariant systems.

Method: Analysis of various models (Su-Schrieffer-Heeger, quantum XY chain, Haldane model) and examination of the geometric DQPT condition leading to entanglement spectrum degeneracies.

Result: At the DQPT conditions, exact degeneracies in the entanglement spectrum correspond to maximal entropy of ln 2; dimensional dependence of critical momenta is established.

Conclusion: Momentum-space entanglement entropy serves as a robust diagnostic for DQPTs, linking entanglement and topology in a unified framework.
...
### [63] [Ghost imaging with zero photons](https://arxiv.org/abs/2604.07782)
*Meixue Chen,Yiqi Song,Yu Gu,Huafan Zhang,Huaibin Zheng,Yuchen He,Hui Chen,Yu Zhou,Fuli Li,Zhuo Xu,Jianbin Liu*

Main category: quant-ph

TL;DR: This study introduces a novel ghost imaging method that retrieves images using zero photons from both signal and reference beams, challenging conventional imaging principles.


<details>
  <summary>Details</summary>
Motivation: To explore unconventional properties of ghost imaging that allow for image reconstruction even when no photons interact with the object or are present in the signal and reference beams.

Method: Conducted an experiment referred to as 'ghost imaging with zero photons' where only time bins with zero photons were utilized to retrieve images, employing photon-number projection measurement and analyzing thermal light statistics.

Result: Successfully demonstrated that images can be reconstructed despite the absence of interacting photons or photon presence in both beams, highlighting new correlations in ghost imaging.

Conclusion: The findings contribute to the ongoing discussion on ghost imaging's physics, deepening the understanding of the relationship between quantum and classical correlations.
...
### [64] [Inverse Laplace and Mellin integral transforms modified for use in quantum communications](https://arxiv.org/abs/2604.07787)
*Gustavo Alvarez,Igor Kondrashuk*

Main category: quant-ph

TL;DR: This paper reviews and modifies integral transformations for applications in quantum computing security, drawing from concepts in quantum field theory.


<details>
  <summary>Details</summary>
Motivation: To enhance the application of integral transformations in quantum computing, particularly in security protocols.

Method: The paper modifies standard integral transformations and the inverse integral transformation to accommodate solutions from quantum chromodynamics and optics.

Result: The proposed modifications allow for the effective application of these transformations in quantum security protocols.

Conclusion: The study provides a foundational review of integral transforms and presents novel modifications that can improve their utility in extended domains such as quantum computing.
...
### [65] [Complexity phase transition for continuous-variable cluster state](https://arxiv.org/abs/2604.07804)
*Byeongseon Go,Hyunseok Jeong,Changhun Oh*

Main category: quant-ph

TL;DR: The paper explores how finite squeezing impacts the computational power of continuous-variable cluster states in measurement-based quantum computation, identifying thresholds for computational complexity based on squeezing levels.


<details>
  <summary>Details</summary>
Motivation: To understand the intrinsic computational limitations of CV cluster states in MBQC due to finite squeezing and the experimental challenges of incorporating non-Gaussian resources.

Method: An explicit framework for measurement-based linear optics (MBLO) was developed to analyze the relationship between squeezing levels and the computational complexity of quantum output states.

Result: The study identifies thresholds corresponding to classically tractable and intractable computational regimes, revealing a squeezing-driven complexity phase transition.

Conclusion: Advancements in squeezing levels or error-correction integration are essential for achieving reliable, large-scale quantum computation with CV cluster states.
...
### [66] [Optimal noisy quantum phase estimation with finite-dimensional states](https://arxiv.org/abs/2604.07828)
*Jin-Feng Qin,Jing Liu*

Main category: quant-ph

TL;DR: This paper investigates optimal finite-dimensional probe states for phase estimation in quantum interferometry under noise, specifically particle loss, and proposes a measurement strategy to achieve high precision.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to understand optimal probe states in practical noisy conditions, as previous results were under ideal, noise-free conditions.

Method: The study uses numerical algorithms, particularly constrained optimization by linear approximation, to explore the true optimal states in the presence of particle loss.

Result: A two-step measurement strategy is developed and confirmed through numerical simulations, showing it can reach the ultimate precision limit despite noise.

Conclusion: The paper concludes that the proposed strategy effectively addresses the challenges posed by particle loss, thereby enhancing phase estimation precision.
...
### [67] [Quantum Simulation of Hyperbolic Equations and the Nonexistence of a Dirac Path Measure](https://arxiv.org/abs/2604.07847)
*Sumita Datta*

Main category: quant-ph

TL;DR: The paper analyzes why a classical probability measure for the Dirac equation's path integral does not exist, linking insights from distributional properties and Minkowski metric's signature.


<details>
  <summary>Details</summary>
Motivation: The need to understand the limitations in representing the Dirac equation with a probability measure in Minkowski space has prompted this reevaluation.

Method: The authors compare two perspectives: Zastawniak's insights on the Dirac propagator's distributional nature and the implications of Minkowski's indefinite metric.

Result: The perspectives are unified, revealing a singular mathematical obstruction that prevents the construction of a suitable measure.

Conclusion: Understanding this obstruction has implications for stochastic representations of relativistic equations.
...
### [68] [Analysis of State Teleportation using Noisy Quantum Gates](https://arxiv.org/abs/2604.07849)
*Imama Tul Birrah Khan,Muhammad Faryad*

Main category: quant-ph

TL;DR: The study analyzes how various noise processes impact quantum state teleportation, finding that fidelity decreases polynomially with noise but shows linear decline at low noise levels, indicating robustness of the protocol.


<details>
  <summary>Details</summary>
Motivation: To understand the effects of noise on quantum computing protocols, particularly quantum state teleportation, and to characterize error sources in realistic scenarios.

Method: The study models different noise processes (depolarization, bit flip, phase flip) as quantum channels, evaluating the fidelity of teleported states by comparing ideal and noisy outcomes.

Result: Fidelity decreases polynomially with increasing noise strength across all types, but shows a linear decrease at low noise levels, demonstrating the teleportation protocol's robustness under certain conditions.

Conclusion: The findings enhance understanding of error characteristics and could guide noise mitigation strategies in quantum computing.
...
### [69] [Hardware-Aware Quantum Support Vector Machines](https://arxiv.org/abs/2604.07856)
*Adil Mubashir Chaudhry,Ali Raza Haider,Hanzla Khan,Muhammad Faryad*

Main category: quant-ph

TL;DR: The paper presents a hardware-aware Neural Architecture Search (NAS) that optimizes quantum feature maps for IBM quantum processors, achieving high accuracy with native gate compatibility and zero transpilation overhead.


<details>
  <summary>Details</summary>
Motivation: To design quantum machine learning algorithms that efficiently utilize near-term quantum hardware while ensuring compatibility with specific gate sets and minimizing errors from transpilation.

Method: Utilizing genetic algorithms for NAS to create quantum Support Vector Machine (QSVM) feature maps constrained to IBM Torino native gates, leading to circuits directly executable on IBM quantum hardware.

Result: The NAS method discovered a 12-gate circuit achieving 91.23% accuracy on the UCI Breast Cancer dataset, outperforming hand-crafted feature maps by 27 percentage points and coming close to the classical RBF SVM baseline.

Conclusion: This research demonstrates that hardware-aware NAS enables the practical deployment of quantum kernel methods on current NISQ devices, facilitating immediate circuit execution without the need for modifications.
...
### [70] [Fast and Coherent Transfer of Atomic Qubits in Optical Tweezers using Fiber Array Architecture](https://arxiv.org/abs/2604.07862)
*Jia-Chao Wang,Zai-Zheng Zhang,Xiao Li,Guang-Wei Wang,Xiao-Dong He,Min Liu,Peng Xu*

Main category: quant-ph

TL;DR: This paper presents a fiber array neutral-atom quantum computing architecture enabling fast and low-heating qubit transfer, enhancing scalability and performance.


<details>
  <summary>Details</summary>
Motivation: There is a need to address the bottlenecks of transfer speed and motional heating in quantum circuits to facilitate scalable quantum computing.

Method: The authors utilize a fiber array architecture with site-resolved control over trap depths to facilitate smooth amplitude exchange between static and moving traps, enabling quick and coherent qubit transfers.

Result: They achieve a transfer time of 10 μs with minimal heating (0.156(9) μK) and high fidelity (0.99992(5)) over 500 cycles, with inter-site transfers taking 120 μs at a heating rate of 0.783(17) μK, maintaining fidelity of 0.9998(1) over 100 cycles.

Conclusion: This work presents a scalable and efficient method for coherent qubit transfer that significantly reduces heating, improving speed and fidelity in quantum computing.
...
### [71] [Hybrid Quantum--Classical k-Means Clustering via Quantum Feature Maps](https://arxiv.org/abs/2604.07873)
*Syed M. Abdullah,Alisha Baba,Muhammad Siddique,Muhammad Faryad*

Main category: quant-ph

TL;DR: This study introduces a quantum-enhanced variant of the k-means clustering algorithm, utilizing quantum kernels for improved clustering performance over classical methods.


<details>
  <summary>Details</summary>
Motivation: The k-means algorithm is widely used but has limitations such as sensitivity to initialization and poor performance in high-dimensional spaces.

Method: The proposed quantum k-means replaces Euclidean distance with a quantum kernel from inner products of quantum states, employing multiple quantum feature maps to embed data in a higher-dimensional space.

Result: Achieving clustering accuracy of 88.6% on the Iris dataset and 91.0% on the breast cancer dataset, with improved stability and performance over classical k-means.

Conclusion: Quantum kernels present a promising approach for enhanced unsupervised learning, particularly in the era of NISQ computing.
...
### [72] [Quantum Thermal Field Effect Transistor](https://arxiv.org/abs/2604.07893)
*Abhijeet Kumar,Soniya Malik,P. Arumugam*

Main category: quant-ph

TL;DR: The paper presents a quantum thermal field-effect transistor (qtFET) utilizing a qubit-qutrit-qubit architecture, demonstrating its potential analogies to traditional electronic field-effect transistors and its capacity to modulate thermal currents.


<details>
  <summary>Details</summary>
Motivation: To explore novel quantum devices that can effectively manage thermal currents while leveraging quantum properties for enhanced performance over traditional electronics.

Method: The qtFET is designed with left and right qubits coupled to a middle qutrit, each interacting with individual thermal baths; the middle qutrit acts as a modulator for thermal current.

Result: The study reveals that the qtFET functions similarly to an eFET, effectively modulating thermal currents and showing promise for future quantum thermal devices and amplifiers.

Conclusion: The qtFET presents a new architecture that can serve as a key component in quantum thermal technologies, suggesting significant advancements over conventional transistor designs.
...
### [73] [Non-variational supervised quantum kernel methods: a review](https://arxiv.org/abs/2604.07896)
*John Tanner,Chon-Fai Kam,Jingbo Wang*

Main category: quant-ph

TL;DR: The paper reviews non-variational quantum kernel methods (QKMs) in supervised quantum machine learning, focusing on their theoretical foundations, practical implementations, and challenges, and aims to identify conditions for achieving quantum advantages.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of non-variational QKMs while addressing limitations of variational methods and to provide clarity on their practical application in quantum machine learning.

Method: The review covers classical kernel theory, quantum feature map constructions, estimation methods, and frameworks for evaluating quantum advantage while discussing challenges like exponential concentration and spectral properties of operators.

Result: Insights are synthesized on the effectiveness of QKMs, challenges in their application, and conditions under which they may outperform classical models.

Conclusion: The paper highlights the promise of QKMs in supervised learning and outlines key obstacles that need resolution to realize quantum-enhanced performance.
...
### [74] [A Review of Variational Quantum Algorithms: Insights into Fault-Tolerant Quantum Computing](https://arxiv.org/abs/2604.07909)
...
TL;DR: The paper reviews variational quantum algorithms (VQAs) in the context of transitioning to fault-tolerant quantum computing, analyzing their core framework, challenges, and applications.


<details>
  <summary>Details</summary>
Motivation: As quantum architectures evolve toward fault-tolerant regimes, it's crucial to reassess the viability of VQAs under new operational constraints.

Method: The review deconstructs the algorithmic framework of VQAs, examining ansatz design, optimization strategies, training bottlenecks, and their adaptation to error-corrected environments.

Result: Identifies critical challenges like barren plateaus and evaluates techniques for maintaining performance in early fault-tolerant phases, while discussing diverse applications of VQAs.

Conclusion: Provides a theoretical roadmap for refining variational principles to ensure the continued relevance of VQAs in future quantum hardware generations.
...
### [75] [Informational Mpemba Effect for Fast State Purification in Non-Hermitian System](https://arxiv.org/abs/2604.07926)
*C. G. Feyisa,Huan-YuKu,J. -S. You,H. H. Jen*

Main category: quant-ph

TL;DR: The paper presents a method for rapid state purification in quantum systems using collective reservoir engineering and highlights a phenomenon where mixed states achieve higher purity faster than expected.


<details>
  <summary>Details</summary>
Motivation: The motivation lies in overcoming the fragility of quantum systems to decoherence, which hinders their practical use in quantum applications.

Method: The authors employ collective reservoir engineering in driven non-Hermitian qubit systems to rapidly purify mixed states and generate multipartite entanglement.

Result: Efficient entanglement generation during purification is found to depend on the degeneracy of collective subradiant modes, exhibiting an informational Mpemba effect, where initially mixed states achieve full purity faster than expected.

Conclusion: The findings suggest that driven non-Hermitian systems with engineered dissipation provide enhanced purification efficiency, paving the way for advancements in quantum engineering.
...
### [76] [Investigation of Automated Design of Quantum Circuits for Imaginary Time Evolution Methods Using Deep Reinforcement Learning](https://arxiv.org/abs/2604.07951)
*Ryo Suzuki,Shohei Watabe*

Main category: quant-ph

TL;DR: The paper presents an automated framework using DDQN for VITE circuit design, improving circuit efficiency for quantum computations.


<details>
  <summary>Details</summary>
Motivation: To address limitations of current VITE methods on NISQ devices due to high gate counts and circuit depth.

Method: An automated framework using Double Deep-Q Networks (DDQN) to treat circuit design as a multi-objective optimization problem.

Result: Achieved 37% fewer gates and 43% less depth in Max-Cut problems; reached Full-CI limit for H2 with shallower circuits.

Conclusion: Deep reinforcement learning is effective in discovering optimal circuit structures, advancing efficient quantum algorithm design.
...
### [77] [Quantum Property Testing for Bounded-Degree Directed Graphs](https://arxiv.org/abs/2604.07954)
*Pan Peng,Jingyu Wu*

Main category: quant-ph

TL;DR: The study demonstrates a quantum speedup for testing properties of directed graphs by showing that certain properties tested classically require significantly fewer queries in a quantum unidirectional model.


<details>
  <summary>Details</summary>
Motivation: To explore the advantages of quantum algorithms in property testing for directed graphs with bounded degrees, focusing on the trade-offs between classical and quantum query complexities.

Method: The authors provide a quantum property testing framework for directed graphs, analyzing the queries needed for both classical bidirectional and quantum unidirectional models.

Result: They establish that properties testable with $O_{	ext{ε,d}}(1)$ classical queries can be tested in the quantum model using $n^{1/2 - Ω_{	ext{ε,d}}(1)}$ queries, achieving almost quadratic speedup. A specific property is shown to necessitate more queries in the quantum model, demonstrating the tightness of their results.

Conclusion: The findings highlight significant quantum advantages in property testing and suggest efficient approximations of subgraph occurrences in unidirectional settings, paving the way for further explorations in quantum property testing.
...
### [78] [Simultaneous ground-state cooling of six mechanical modes of two levitated nanoparticles](https://arxiv.org/abs/2604.07971)
*Qian Zhang,Yi Xu,Jie-Qiao Liao*

Main category: quant-ph

TL;DR: The paper discusses a system for simultaneously cooling multiple modes of levitated nanoparticles using a coupled cavity setup.


<details>
  <summary>Details</summary>
Motivation: To explore macroscopic quantum effects in mechanical motion, ground-state cooling is essential.

Method: A polarization-angle-controllable cavity-nanoparticle system is developed, where two nanoparticles are trapped and coupled to a cavity field, allowing for simultaneous ground-state cooling of six mechanical modes.

Result: A linearized seven-mode Hamiltonian is derived, revealing the cooling mechanism and confirming conditions for dark modes that impede cooling.

Conclusion: The ability to control polarization angles allows for effective manipulation of quantum states, facilitating the study of collective quantum effects in multiple nanoparticles.
...
### [79] [Belief Propagation Convergence Prediction for Bivariate Bicycle Quantum Error Correction Codes](https://arxiv.org/abs/2604.07995)
*Anton Pakhunov*

Main category: quant-ph

TL;DR: This paper presents a method to predict the convergence of Belief Propagation in Bivariate Bicycle quantum error correction codes, using a simple modulo operation.


<details>
  <summary>Details</summary>
Motivation: Accurately predicting the convergence of BP decoding can optimize quantum error correction processes and improve performance.

Method: The authors propose that if the syndrome defect count is divisible by the code's column weight w, BP will likely converge, requiring only a simple modulo check.

Result: The method shows high accuracy in predicting BP convergence (100% at p <= 0.001, degrading to 87% at p = 0.01) and validated through extensive testing on five BB codes, achieving AUC = 0.995.

Conclusion: The findings boost the effectiveness of BP decoding in quantum error correction, confirming the method's reliability across various decoder strategies and its implications for future implementations.
...
### [80] [Harnessing dark states: coherent control in coupled cavity-Rydberg-atom systems](https://arxiv.org/abs/2604.08023)
*Ying-Zhi Li,Xuan Zhao,Le-Man Kuang,Jie-Qiao Liao*

Main category: quant-ph

TL;DR: Study of dark-state effects in Rydberg atoms and cavity systems using the arrowhead-matrix method, highlighting implications for quantum information.


<details>
  <summary>Details</summary>
Motivation: The dark-state effect is crucial for advancing fundamental understanding in atomic physics and has practical applications in quantum technologies.

Method: Utilized the arrowhead-matrix method to analyze dark states in a system of coupled Rydberg atoms and cavity field, exploring cases for multiple atoms and excitation number subspaces.

Result: Identified the forms of dark states for various atom configurations and proposed a method for experimental detection based on quantum state populations.

Conclusion: Findings enhance the understanding of dark-state physics and suggest new avenues for experimentation in quantum information applications.
...
### [81] [Fixing semi-classical physics from first principles: how to derive effective classical-quantum dynamics from open quantum theory](https://arxiv.org/abs/2604.08024)
*Isaac Layton*

Main category: quant-ph

TL;DR: The paper critiques standard mean-field semi-classical methods and demonstrates that incorporating environmental decoherence provides accurate semi-classical descriptions of quantum dynamics.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of mean-field semi-classical approaches in accurately representing quantum systems.

Method: Utilizes a toy model to analyze the standard semi-classical approach and includes environmental decoherence.

Result: Demonstrates that models including decoherence improve semi-classical approaches and accurately reflect the original quantum dynamics.

Conclusion: Consistent classical-quantum dynamics can serve as effective descriptions of open quantum systems.
...
### [82] [Local Marking of Locally Implementable Unitary Operations](https://arxiv.org/abs/2604.08054)
*Adil Imam,Satyaki Manna*

Main category: quant-ph

TL;DR: The paper explores local marking of multipartite quantum unitary operations, showing that local marking doesn't equate to distinguishing those operations. It presents examples of globally distinguishable operations that are not locally markable and examines the implications for entanglement and discrimination.


<details>
  <summary>Details</summary>
Motivation: The study aims to clarify the concepts of local marking versus distinguishing quantum operations, particularly in scenarios without entanglement.

Method: The authors analyze multipartite unitary channels using local operations and classical communication (LOCC) to investigate their local distinguishability and marking properties.

Result: They found that while local marking can be achieved, it does not imply global distinguishability. They provided examples of globally distinguishable unitary operations that cannot be locally marked.

Conclusion: The investigation reveals critical differences between local marking and discrimination, highlighting a hierarchy in how entangled and product operations can be probed and marked.
...
### [83] [Orthogonalised Self-Guided Quantum Tomography: Insights from Single-Pixel Imaging](https://arxiv.org/abs/2604.08057)
...
TL;DR: The paper introduces self-guided imaging (SGI), a linear approach akin to self-guided quantum tomography (SGQT), demonstrating its mathematical equivalence to single-pixel imaging (SPI) and showcasing improved performance through orthogonalisation techniques.


<details>
  <summary>Details</summary>
Motivation: To enhance imaging techniques using concepts from quantum tomography while ensuring efficiency and accuracy in measurements.

Method: The authors developed self-guided imaging (SGI) and orthogonalised SGQT, leveraging advancements from single-pixel imaging, resulting in improved convergence without extra experimental costs.

Result: Numerically, fidelity increased from 95.2% to 99.17%, and experimentally from 92.1% to 95.3%, showcasing significant performance enhancement.

Conclusion: The findings indicate that integrating methods from SPI and SGQT can optimize imaging measurements and convergence in various applications.
...
### [84] [Divide et impera: hybrid multinomial classifiers from quantum binary models](https://arxiv.org/abs/2604.08094)
*Simone Roncallo,Angela Rosy Morgillo,Seth Lloyd,Chiara Macchiavello,Lorenzo Maccone*

Main category: quant-ph

TL;DR: This paper explores integrating quantum binary models into a multinomial classifier using various strategies, highlighting the efficiency of decision trees.


<details>
  <summary>Details</summary>
Motivation: To understand how to effectively combine quantum binary models for improved classification performance.

Method: A hybrid approach using one-vs-one, one-vs-rest strategies, and a binary decision tree, with benchmarking against a classical binary model.

Result: The decision tree method is shown to be cost-effective, achieving similar accuracies while maintaining logarithmic computational overhead relative to the number of classes.

Conclusion: The decision tree offers a viable solution for integrating quantum binary models into multinomial classification.
...
### [85] [Photon pairs, squeezed light and the quantum wave mixing effect in a cascaded qubit system](https://arxiv.org/abs/2604.08139)
*R. D. Ivanovskikh,W. V. Pogosov,A. A. Elistratov,S. V. Remizov,A. Yu. Dmitriev,T. R. Sabirov,A. V. Vasenin,S. A. Gunin,O. V. Astafiev*

Main category: quant-ph

TL;DR: The paper presents a theoretical framework for quantum wave mixing in a system with two superconducting qubits, identifying conditions for anomalous correlations and a selection rule for QWM spectrum peaks.


<details>
  <summary>Details</summary>
Motivation: To explore quantum wave mixing in cascaded waveguide-QED systems and the influence of photon correlations on output spectra.

Method: Derived an effective master equation from field correlation functions of a source emission and analyzed the resulting qubit dynamics under specific conditions.

Result: Identified a regime with anomalous correlations and established a selection rule that suppresses specific sidebands in the QWM spectrum; confirmed findings through numerical simulations.

Conclusion: The analysis of peak amplitudes provides a means to investigate photon statistics in nonclassical fields impacting quantum technologies.
...
### [86] [Charging Quantum Batteries via Dissipative Quenches](https://arxiv.org/abs/2604.08151)
*Riccardo Grazi,Donato Farina,Niccolò Traverso Ziani,Dario Ferraro*

Main category: quant-ph

TL;DR: This paper explores work extraction in open quantum batteries with interacting spin chains, focusing on how dissipation and dephasing affect ergotropy during different states and dynamics.


<details>
  <summary>Details</summary>
Motivation: To understand how environmental factors influence the efficiency of work extraction in quantum batteries and to identify conditions under which ergotropy is generated.

Method: Analyzed two- and four-qubit XX models in thermal Gibbs states while varying dissipation and dephasing types; characterized the work extractability through environmental effects.

Result: Showed that dissipative dynamics can generate ergotropy from passive states, with hotter states outperforming colder ones temporarily, and identified how collective dissipation affects steady states based on temperature and system size.

Conclusion: Dissipation enhances work extraction capabilities, while dephasing diminishes it, underscoring the divergent impacts of these environmental factors on quantum battery efficiency.
...
### [87] [Evaluating the performance of a weak-field homodyne receiver in quadrature phase-shift keying optical communication](https://arxiv.org/abs/2604.08241)
*Silvia Cassina,Alex Pozzoli,Michele N. Notarnicola,Marco Lamperti,Stefano Olivares,Alessia Allevi*

Main category: quant-ph

TL;DR: The paper presents a new weak-field receiver for quantum communication that enhances information transfer rates, showing promise with quaternary communication based on coherent states.


<details>
  <summary>Details</summary>
Motivation: Efficient detection schemes are critical for improving quantum communication protocols' information transfer rates.

Method: A weak-field receiver utilizing wave-particle duality was developed, focusing on quaternary communication using coherent states with controlled phase encoding via feedback mechanisms.

Result: The receiver demonstrated improved mutual information and secret key generation rates, indicating its effectiveness over traditional receivers.

Conclusion: The findings support advancing to continuous phase modulation for further enhancing communication efficiency.
...
### [88] [Optimized Gottesman-Kitaev-Preskill Error Correction via Tunable Preprocessing](https://arxiv.org/abs/2604.08247)
*Xiang-Jiang Chen,Hao-Miao Jiang,Liu-Jun Wang,Qing Chen*

Main category: quant-ph

TL;DR: The paper introduces a preprocessing-based Steane-type scheme (P-Steane) for the GKP code, enhancing error correction by reshaping noise propagation through tunable parameters.


<details>
  <summary>Details</summary>
Motivation: To improve the performance of GKP code error-correction protocols limited by the noise propagation of the canonical Steane-type scheme.

Method: The P-Steane scheme employs a preprocessing stage with adjustable squeezing parameters $a$ and $b$, creating a parameter framework that includes variations of existing methods.

Result: The P-Steane scheme reproduces the performance of the ME-Steane and teleportation-based schemes and achieves the minimum noise variances in specific conditions, outperforming the ME-Steane scheme over a defined parameter range.

Conclusion: By allowing adjustable noise propagation, the P-Steane scheme enhances error correction in the GKP code, particularly in small-noise regimes.
...
### [89] [QARIMA: A Quantum Approach To Classical Time Series Analysis](https://arxiv.org/abs/2604.08277)
*Nishikanta Mohanty,Bikash K. Behera,Badshah Mukherjee,Pravat Dash*

Main category: quant-ph

TL;DR: The paper introduces a quantum-inspired ARIMA methodology that utilizes quantum techniques for identifying lags and estimating parameters, demonstrating improved model efficiency on various datasets.


<details>
  <summary>Details</summary>
Motivation: To enhance the ARIMA modeling process using quantum assistance, thereby improving efficiency in lag discovery and parameter estimation without significant hyperparameter tuning.

Method: The methodology integrates quantum-assisted lag discovery (QACF and QPACF) with fixed-configuration variational quantum circuits for autoregressive and moving-average parameter estimation, including a weak-lag refinement step that does not alter the determined orders.

Result: The approach is empirically validated on environmental and industrial datasets, showing reduced mean squared error and mean absolute percentage error compared to classical ARIMA models, with clear contributions from quantum techniques in optimization.

Conclusion: The integration of quantum methods in the ARIMA framework not only decreases meta-optimization effort but also clarifies the role of quantum effects in model parameterization.
...
### [90] [Kirkwood-Dirac distributions in classical optics](https://arxiv.org/abs/2604.08325)
*Alfredo Luis,Lorena Ballesteros Ferraz*

Main category: quant-ph

TL;DR: The paper analyzes Kirkwood-Dirac distributions in classical optics, linking them to optical coherence and providing methods for their experimental determination.


<details>
  <summary>Details</summary>
Motivation: To explore the connection between Kirkwood-Dirac distributions and optical coherence, revealing the significance of complex and negative values in coherence analysis.

Method: The paper defines Kirkwood-Dirac distributions as generalized mutual coherence functions across various field variables and proposes experimental methods for their determination using interference.

Result: The authors demonstrate that the interpretation of anomalous values relates directly to coherence and applies uniformly across all field variables, including polarization and wave propagation.

Conclusion: The findings unify the understanding of Kirkwood-Dirac distributions and coherence in optics, suggesting coherent interpretations of complex and negative values.
...
### [91] [Thermal Time and Irreversibility from Non-Commuting Observables in Accelerated Quantum Systems](https://arxiv.org/abs/2604.08349)
*Marcello Rotondo*

Main category: quant-ph

TL;DR: This paper explores the conditions under which temporal ordering becomes significant in relativistic quantum field theory through localized detector models, showing the importance of the KMS condition and non-commuting observables in achieving distinguishability.


<details>
  <summary>Details</summary>
Motivation: To understand how temporal ordering affects operations in relativistic quantum field theory and to identify the circumstances necessitating this distinguishability.

Method: The study employs localized detector models, focusing on two-level detectors interacting with a quantum field in the Minkowski vacuum, analyzing the influence of KMS condition and sequential couplings through non-commuting observables on the detector states.

Result: Distinguishability in interactions is achieved when the state meets the KMS condition, revealing that the detector's reduced state is sensitive to the order of operations even at second order, influenced by quantum relative entropy.

Conclusion: The research highlights a relationship between the ordering of interactions and the thermal response of detectors, leading to a closed-form expression that captures the dependence on temperature and energy scales.
...
### [92] [Scalable Neural Decoders for Practical Fault-Tolerant Quantum Computation](https://arxiv.org/abs/2604.08358)
*Andi Gu,J. Pablo Bonilla Ataides,Mikhail D. Lukin,Susanne F. Yelin*

Main category: quant-ph

TL;DR: This paper presents a convolutional neural network decoder that enhances quantum error correction for scalable quantum computing, achieving significantly lower logical error rates and higher throughput compared to existing decoders.


<details>
  <summary>Details</summary>
Motivation: The need for fast and accurate classical decoders in quantum error correction to match the pace of quantum hardware advancements.

Method: Introduction of a convolutional neural network decoder that leverages the geometric structure of quantum error correction codes.

Result: The decoder achieves logical error rates up to 17 times lower than current decoders, with logical error rates around 10^-10 at physical error rates of 0.1%, and significantly higher throughput (3-5 orders of magnitude).

Conclusion: The study indicates that the costs associated with fault-tolerant quantum computation may be lower than previously thought, facilitating the practical application of such systems.
...
### [93] [Per-Shot Evaluation of QAOA on Max-Cut: A Black-Box Implementation Comparison with Goemans-Williamson](https://arxiv.org/abs/2604.08367)
*Evgenii Dolzhkov,Franz G. Fuchs,Dirk Oliver Theis*

Main category: quant-ph

TL;DR: This study empirically evaluates the Quantum Approximate Optimization Algorithm (QAOA) for the Max-Cut problem, treating it as a black-box optimizer, and comparing its performance against the classical Goemans-Williamson algorithm.


<details>
  <summary>Details</summary>
Motivation: To assess the realistic performance of QAOA on practical graph structures without the influence of parameter optimization, highlighting its potential limitations for end users.

Method: Conducted empirical evaluations using off-the-shelf QAOA implementations as black-box optimizers and benchmarked against the classical GW algorithm, employing a per-shot statistical framework for analysis.

Result: The evaluation indicates specific performance thresholds for QAOA compared to the classical baseline, revealing circumstances under which QAOA can outperform GW as well as its limitations.

Conclusion: QAOA shows potential for the Max-Cut problem, but current results also suggest significant constraints that need addressing for practical, resource-limited users.
...
### [94] [Sufficiency and Petz recovery for positive maps](https://arxiv.org/abs/2604.08380)
*Lauritz van Luijk,Henrik Wilming*

Main category: quant-ph

TL;DR: This paper explores the relationship between quantum states and positive, trace-preserving maps, characterizing it using minimal sufficient Jordan algebras, and extending previous results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to clarify the mathematical framework surrounding the interconversion of quantum states through positive, trace-preserving maps, particularly in relation to sufficient statistics.

Method: The authors utilize minimal sufficient Jordan algebras to generalize the Koashi-Imoto decomposition, showing that Neyman-Pearson tests generate these algebras and deriving results pertaining to the data-processing inequality and interconversion of dichotomies.

Result: The paper establishes conditions under which recovery maps exist in the PTP setting and derives criteria for the interconversion of dichotomies via PTP maps, contributing to the understanding of quantum statistical experiments.

Conclusion: Overall, it advances the mathematical theory of quantum states and their transformations by providing new insights through minimal sufficient Jordan algebras and extending existing results to approximately finite-dimensional settings.
...
### [95] [Rapid mixing for high-temperature Gibbs states with arbitrary external fields](https://arxiv.org/abs/2604.08408)
*Ainesh Bakshi,Xinyu Tan*

Main category: quant-ph

TL;DR: The paper investigates how external fields affect the entanglement and computational complexity of high-temperature Gibbs states, demonstrating that they can induce entanglement in otherwise separable states and identifying conditions under which sampling from these states is computationally hard.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of external fields on the entanglement structure and computational complexity of high-temperature Gibbs states, which are essential in quantum matter models.

Method: A quasi-local Lindbladian is introduced to describe the dynamics of the system, and its ability to mix to the Gibbs state is analyzed. The complexity of sampling from corresponding Gibbs states with large external fields is also examined under standard complexity assumptions.

Result: External fields can induce entanglement in Gibbs states that would otherwise be separable. The study identifies a scalability crossover for entanglement triggered by the external field and demonstrates that certain configurations result in classically hard sampling problems.

Conclusion: High-temperature Gibbs states affected by external fields exhibit significant entanglement and classical hardness, while also allowing efficient quantum sampling, indicating promising avenues for achieving quantum advantage.
...
### [96] [Multivariate quantum reservoir computing with discrete and continuous variable systems](https://arxiv.org/abs/2604.08427)
*Tobias Fellner,Jonas Merklinger,Christian Holm*

Main category: quant-ph

TL;DR: This paper develops a framework for processing multivariate data in quantum reservoir computing, evaluating three encoding schemes through a new metric and experiments on the Lorenz-63 system.


<details>
  <summary>Details</summary>
Motivation: To expand quantum reservoir computing from univariate to multivariate data processing, addressing real-world complexities.

Method: Three multivariate encoding schemes are proposed and assessed, introducing a mixing capacity metric. Computational performance is evaluated on chaotic system prediction tasks using different quantum reservoirs.

Result: The study shows that optimal encoding methods depend on the reservoir system and task, with peak performance associated with non-classical quantum effects.

Conclusion: The findings emphasize the significance of task-specific input design in quantum reservoir computing for effective multivariate data processing.
...
### [97] [Time evolution of impurity models and their universality for quantum computation](https://arxiv.org/abs/2604.08466)
*N. C. Mai Pham,Raul A. Santos*

Main category: quant-ph

TL;DR: This paper proves that time-independent impurity Hamiltonians can perform universal quantum computation on $N$ qubits given certain conditions, specifically with fermionic product states.


<details>
  <summary>Details</summary>
Motivation: To determine if time-independent evolution of impurity Hamiltonians can achieve universal quantum computation, in contrast to the well-studied time-dependent case.

Method: The authors examine time-independent impurity Hamiltonians acting on $O(N)$ qubits, showing that if the input is a product state of fermions, the system can compute universally.

Result: The proof demonstrates that the size of the impurity required for universal computation scales as $O(S \log S)$, where $S$ is the computation depth.

Conclusion: This establishes a new avenue for understanding quantum computation capabilities of time-independent systems, expanding the known computational resources of impurity Hamiltonians.
...
### [98] [Accelerating Quantum Tensor Network Simulations with Unified Path Variations and Non-Degenerate Batched Sampling](https://arxiv.org/abs/2604.08467)
*Taylor Lee Patti,Paavai Pari,Yang Gao,Azzam Haidar,Thien Nguyen,Tom Lubowe,Daniel Lowell,Brucek Khailany*

Main category: quant-ph

TL;DR: This paper enhances the PTSBE method for tensor networks, achieving over $10^8\times$ speedup in data collection rates for quantum simulations.


<details>
  <summary>Details</summary>
Motivation: To improve the data collection efficiency of tensor network simulations compared to statevector PTSBE, which had limitations leading to modest speedups.

Method: The authors develop three key improvements: (1) error-independent unified path variation, (2) non-degenerate tensor network sampling, and (3) a flexible/optimized contraction framework.

Result: Achieved a data collection rate greater than $10^8\times$ compared to traditional methods, and over $1000\times$ for general quantum simulations.

Conclusion: The presented methods significantly enhance the efficiency of tensor network simulations, making them more competitive with statevector approaches.
...
### [99] [Measurement-induced state transitions across the fluxonium qubit landscape](https://arxiv.org/abs/2604.08515)
*Alex A. Chapple,Boris M. Varbanov,Alexander McDonald,Alexandre Blais*

Main category: quant-ph

TL;DR: The paper analyzes how multi-photon resonances affect measurement fidelity in fluxonium qubits, finding lighter fluxoniums are more robust against state transitions due to their properties.


<details>
  <summary>Details</summary>
Motivation: To optimize high-fidelity readout in circuit quantum electrodynamics, understanding measurement-induced state transitions is crucial.

Method: The study involves a theoretical analysis of measurement-induced state transitions in fluxonium qubits across a wide parameter range and time-dependent readout simulations.

Result: Lighter fluxonium qubits exhibit reduced susceptibility to measurement-induced state transitions compared to heavier ones, attributed to lower multi-photon resonance density and other properties.

Conclusion: The findings suggest that structural characteristics of fluxonium qubits significantly influence their performance in high-fidelity readout contexts.
